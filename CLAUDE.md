...
8. **テーブル設定ページ（/admin/dataset/edit/:id）はテストで使わない**。102フィールドの描画に時間がかかり不安定。フィールドの設定確認は**レコード操作画面で動作テスト**に置き換える。例: 表示条件のUI確認→レコードフォームで表示条件の動作確認、必須設定のUI確認→空保存でエラー確認。テスト観点（何をテストしているか）を維持し、フローだけ変える。スキップは絶対禁止。
9. **テスト修正時はまず元のテスト観点を確認する**。specs/*.yaml の description/expected を読み、「何を確認すべきか」を理解してからフローを設計する。テスト観点を変えずにフローだけ最適化するのが正しいアプローチ。
10. **create-trial API に `with_all_type_table: true` を渡す**。ALLテストテーブルを別途作成する必要がない。テーブルID=7固定。`create-all-type-table` API + ポーリング（60秒）は不要。
11. **全specをcreateTestEnv（自己完結型）に移行する**。global-setupのテスト環境に依存せず、各specのbeforeAllで`createTestEnv(browser)`を呼んで専用環境を作成する。これにより: (a)他のspecのテストデータ変更の影響を受けない (b)並列実行が安全 (c)global-setupの失敗でcascade failしない。
12. **afterAllのリセット処理は不要**。createTestEnvの使い捨て環境では設定のリセットやテーブル削除が不要。afterAllはログインタイムアウトの主要原因でもあったため、削除して高速化。
13. **create-trial API の競合状態**: 同時実行時にUNIQUE KEY違反で`CLIENT ADD ERROR`が発生する。対策: createTestEnvでリトライロジックを追加、または同時実行数を制限。
14. **ALLテストテーブルのフィールドラベル**: debug APIが作成するテーブルの「テキスト_デフォルト値」にはデフォルト値が未設定。デフォルト値テストは「日付」「日時」フィールドの現在日時自動入力で代替する。
15. **ALLテストテーブルの表示条件は未設定**: `add-all-type-table`スクリプトは「ラジオ_表示条件テキスト」フィールドを作成するが、表示条件（ラジオ=ラジオAの時のみ表示）は設定しない。表示条件の動作テストはai-test環境の手動設定済みテーブルでのみ可能。新規作成テーブルではフィールドの存在と操作可能性の確認に留める。
16. **create-trial + with_all_type_table の504対策**: ALBのidle timeout=60秒のため、テーブル作成が60秒を超えると504が返る。domain省略（PHP側自動生成）の方が成功率が高い。504時はwith_all_type_tableなしでリトライ→debug/create-all-type-tableでフォールバック。
