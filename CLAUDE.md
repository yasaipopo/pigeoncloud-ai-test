...
実行/判定チェーン（環境ごと並列・チェーン内直列） → 集計 → レポート + カタログ更新提案
```
...
- **FAIL は必ず切り分けてから報告**: ①本物のプロダクトバグ ②カタログ不備（期待値が実仕様とズレ） ③タイミング（非同期反映ラグ・再実行で再現するか） ④環境起因。プロダクトバグは `.claude/product-bugs.md` に記録し、カタログは緩めない
