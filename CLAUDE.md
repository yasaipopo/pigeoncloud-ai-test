...
- 🔴 **検証は「ブラウザからの通常ユーザー動線」に限定する**（2026-06-22 ユーザー指示）。テスト値は必ずブラウザ操作の結果から取る。**ブラウザで再現できない範囲はテストしない**。
  - 範囲外（E2Eでやらない）の例: 認可バイパス・攻撃可否（raw API で `auto:true` を直送する等のフロントを経由しない検証）・並行性/競合・型不正・境界などのエッジ。これらは **product 側の Unit/Integration テストの責務**
  - バグ修正PR・障害が来ても、E2E が足すのは「その機能が**通常動線として正しく動くか**」だけ（例: 承認WFのセキュリティ修正 #3696 → E2Eは「申請→承認→承認済み」が通るかを見る。「第三者が攻撃で取り下げできるか」は見ない＝integration test の領域）
  - データ準備（debug API でのテーブル/ユーザー/前提state 作成）は従来どおり可。**禁止なのは「検証の主操作をブラウザでなく API 直叩きで代替する」こと**
...
- 🔴 **マニュアル準拠で網羅する**（2026-06-22 ユーザー指示）: シナリオは**公式ヘルプ/マニュアル（help.pigeon-cloud.com 等・repo `pigeon-help-2026`）に書かれた手順を正**とし、「マニュアル通りに基本動作が動くか」を機能ごとに網羅的にカバーする。期待値（observations）はマニュアルの記載が根拠。`source:` にヘルプ記事URL/IDを記録する。Claudeの推測仕様でなくマニュアルが正
- 🔴 **複数ロールを要する動線は observations に「別人性」を必ず入れる**（2026-06-22 wf-001 教訓）: 申請者≠承認者 等、別ユーザーの関与が要件なら「承認者の表示名が申請者と異なる」を観測点にする。同一ユーザーでの簡略化（master が自己申請・自己承認）を構造的に弾く
...
- データ干渉防止: 各シナリオは `{id}-{run-id}-` プレフィックスの自リソースのみ作成・検証＋実行前クリーン＋冪等 setup/teardown（再実行・並列でも件数/完全一致アサートが汚染されない）。件数アサートは「自分が作成した分のみ対象」。ALLテストテーブル不使用（重い・干渉源）。プレフィックスは必ず**そのジャーニー/シナリオ自身のid**に揃える（番号ドリッフト禁止・2026-06-28 codexf指摘）

### 🔴 ジャーニー型の追加規約（2026-06-28 codexfレビュー反映）
- **1認証=多観点（ジャーニー）**: 大=ジャーニー（1ログインで多観点）/ 中=フェーズ / 小=観点(checkpoint)。ログインやり直しを廃す
- 🔴 **多ロール切替規約**: master が UI で1回ログインする主動線を基本とし、補助ロール（申請者/承認者/制限ユーザー等）は**MCP/debugで事前作成＋別 context/storageState を事前生成**しておき、**別人性チェックの時だけ切替**える。毎回ログインし直さない
- 🔴 **MCP準備×UI操作の責務境界**: setup:mcp は土台（テーブル/レコード/grant/WF定義のベースライン）まで。**被検証機能の作成操作（項目定義/書式オプション/通知設定/WF設定/帳票テンプレ登録）は必ず action:ui でUI操作**＝MCPに吸わせない（生成動線の空洞化防止）。各機能エリアに最低1本のUI生成動線を残す
- 🔴 **E2E観測限界の縮退**: ファイル/CSV/Excel/帳票の**中身**・xlsxセル置換・送信元IP変更を要する検証はUI外＝**integration責務**。E2Eは「DL発火＋ファイル名」「設定保存＋一覧表示」等のブラウザ観測点に縮退する。時間依存（自動ログアウト/強制変更間隔/ロック自動解除）は P3隔離 or 設定反映表示へ縮退（非決定回避）
- 🔴 **flake対策**: D&D・ファイルUP/ZIP・メール受信・非同期反映は固定sleep禁止。web-first待機（expect.poll/反映を待ってから）＋一意データ＋遷移後アサート
...
- 🔴 **テスト結果・動作手順は gemcli または別エージェントで必ずチェックする**（2026-06-22 ユーザー指示・wf-001 実害より）: PASS確定・完了報告の前に、(a) テスト結果（判定）と (b) executor の実際の操作手順（`work/{id}/` の最終成功スクリプト）を gemcli（`~/.gemini/gemini-tracked`）か別の Claude サブエージェントに渡してレビューさせ、下記を検出する:
  - **要件のすり替え・簡略化**: 例「申請者≠承認者」が要件なのに master が自己申請・自己承認していないか（wf-001 で実際に発生・判定がスルーした穴）
  - **観測の取り違え・緩和**: observations の対象を別の表示で代替していないか、マニュアルの期待と実際の検証対象がズレていないか
  - チェックで問題が出たら PASS にせず TEST_ISSUE として差し戻す。gemcli 不可時は別サブエージェントにフォールバック（無チェックで完了報告しない）
