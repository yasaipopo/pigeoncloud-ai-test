...
## 🟢 自律マージ承認 (恒久ルール、2026-04-26 ユーザー指示)

**このリポジトリ (yasaipopo/pigeoncloud-ai-test) は E2E テストコード専用のため、
PR の main マージはユーザー承認なしで実行してよい。**

- 適用範囲: このリポジトリ内のすべての PR (テスト追加・修正・知見 md・coverage map 等)
- 流れ:
  1. ブランチで作業 → コミット → push → `gh pr create --draft`
  2. 単体テスト pass 確認 (黄金ルートの [7] 相当) → Gemini bot レビューがあれば対応
  3. **`gh pr ready N` → `gh pr merge N --squash --delete-branch` を続けて自動実行可**
  4. main 同期 → 必要なら次の PR に着手
- 例外: 機密情報を含む可能性がある PR、プロダクトリポジトリ (Loftal/pigeon_cloud) への変更はこのルールの対象外
- グローバル CLAUDE.md の「main マージは必ずユーザー確認」ルールに対する **このリポジトリ限定の上書き**

なぜ: テストコード変更は本番影響なし、また連続 PR 着手時にユーザー承認待ちが
ボトルネックになっていたため (2026-04-26 ユーザー明示指示)。

---

## 🟢 gemcli 方向性レビュー (恒久ルール、2026-04-26 ユーザー指示)

**自律進行中、進む方向が "変な方" に逸れないよう gemcli にレビューさせる。**

タイミング (どれかに該当したら実行):
- **大きな方針転換時**: 別カテゴリのテスト群へピボットする時 (例: CRUD → enforcement)
- **PR 3 件単位**: PR 3 件続けてマージしたら、次の方針を gemcli に検証させる
- **不確実性が高いとき**: CRUD で十分か enforcement まで踏み込むかなど判断に迷ったら即実行
- ユーザーが "レビューさせて" と明示指示したとき

レビュー内容に含めること:
- これまでやったこと (PR 一覧 + 件数 + 結果)
- 進む方向 (次の PR で何をやる予定か)
- 検証してほしい観点 (a) 方向性、b) 粒度、c) 優先度、d) 代替施策)

実行コマンド:
```bash
~/.gemini/gemini-tracked -y -p "<状況サマリー + 検証観点>"
```

判定種別と動作:
- **妥当 (このまま進めてよい)**: そのまま続行、ユーザー報告に "Gemini 妥当判定" を含める
- **再考 (方針転換すべき)**: 提言を踏まえて方針修正、ユーザーに変更点を報告
- **改善案あり**: 提言を取り入れた upgrade 版で進行

なぜ: 自律進行中に "変な方向" に走るリスクを Gemini で早期検出するため
(2026-04-26 ユーザー指示「変な方に進んでないか毎回レビューさせて」)。
