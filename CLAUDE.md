...
4. **テーブル設定ページ（/admin/dataset/edit/:id）はテストで使わない**。102フィールドの描画に時間がかかり不安定。フィールドの設定確認は**レコード操作画面で動作テスト**に置き換える。テスト観点を維持しフローだけ変える。スキップは絶対禁止。
...
10. **workflow.spec.js は全面書き直しが必要**（2026-04-03時点）。ワークフローの設定・申請UIがビジュアルエディタに一新されたため、既存のセレクター・フローが全て無効。現状のテストは実行しても意味がないので、新UIに合わせて再実装する。
