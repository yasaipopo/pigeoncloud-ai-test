...
【2】実行・判定オーケストレーション（環境チェーン並列 × チェーン内直列）
  ...
  │            → 再判定 → それでもNGなら確定                │
  │         │ PASS のとき → 第三者チェック(別エージェント):  │
  │         └→ シナリオ要件 vs 実際の操作手順(work/script)を │
  │            突合。要件すり替え・簡略化(例:申請者と承認者を │
  │            同一userで代替)を検出したら FAIL/TEST_ISSUE降格│
  │         │ FAIL / STUCK のとき:                          │
...
RUN_ID=$(date +%Y%m%d-%H%M)-run
RUN_DIR=$(node v2/lib/paths.js "$RUN_ID")  # ~/pigeon-e2e-archive/YYYY-MM/{runId}
node v2/provision-envs.js --count 2 --run-dir "$RUN_DIR"   # レジストリ再利用 + 不足分のみ新規作成
# ローカル実行時は .env.staging を自動ロード（--env-file で変更可）
# 成果物（report.md / evidence / video）は全て RUN_DIR 配下に月ごと集約される
```
