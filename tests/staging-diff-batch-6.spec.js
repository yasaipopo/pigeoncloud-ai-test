...
test.describe.serial('staging 差分 第 7 弾 (10 件 認可ネガティブ + API + パフォーマンス)', () => {
    ...
    /**
     * neg-010: 一般ユーザーで /api/admin/edit/admin/1 POST 更新 → 401/403 (権限不足)
     * (Angular SPA は route を render するため、認可は API レイヤで検証する)
     * @requirements.txt(R-360)
     */
    test('neg-010: 一般ユーザーが master(id=1) の admin 情報更新 API → 拒否', async ({ page }) => {
        test.skip(fileBeforeAllFailed || !generalUserEmail, 'beforeAll失敗 or 一般ユーザー作成失敗');
        test.setTimeout(60000);
        const _testStart = Date.now();

        await loginAs(page, generalUserEmail, generalUserPassword);
        ...
        // 一般ユーザーは master(id=1) を更新できない (400/401/403 のいずれか)
        expect([400, 401, 403], `一般ユーザー master 更新は拒否 (actual=${result.status})`).toContain(result.status);

        await autoScreenshot(page, 'SD7-01', 'neg-010', _testStart);
    });

    /**
     * neg-020: 一般ユーザーで master-settings 画面ロード → ISE/500 が出ない (構造的安全性)
    ...
    test('neg-020: 一般ユーザーで /admin/master-settings → ISE/500 出ない', async ({ page }) => {
        ...
        await loginAs(page, generalUserEmail, generalUserPassword);
        await page.goto(BASE_URL + '/admin/master-settings', { waitUntil: 'domcontentloaded', timeout: 15000 }).catch(() => {});
        ...
        await autoScreenshot(page, 'SD7-02', 'neg-020', _testStart);
    });

    /**
     * neg-030: 一般ユーザーで kintone-migration 画面ロード → ISE/500 出ない
    ...
    test('neg-030: 一般ユーザーで /admin/kintone-migration → ISE/500 出ない', async ({ page }) => {
        ...
        await loginAs(page, generalUserEmail, generalUserPassword);
        await page.goto(BASE_URL + '/admin/kintone-migration', { waitUntil: 'domcontentloaded', timeout: 15000 }).catch(() => {});
        await page.waitForTimeout(2000);
        ...
        await autoScreenshot(page, 'SD7-03', 'neg-030', _testStart);
    });

    /**
     * neg-040: 一般ユーザーで debug API → 403 または認可エラー
     ...
     */
    test('neg-050: 認証なしで /api/admin/edit/admin/1 POST 更新 → 拒否 (4xx)', async ({ page }) => {
        test.skip(fileBeforeAllFailed, 'beforeAll失敗のためスキップ');
        test.setTimeout(60000);
        const _testStart = Date.now();

        // クッキーをクリアしてから login ページに行く (origin を揃えてから fetch)
        await page.context().clearCookies().catch(() => {});
        ...
        // 未認証で書き込み API は 400/401/403 のいずれかで拒否
        expect([400, 401, 403], `未認証 write は拒否 (actual=${result.status})`).toContain(result.status);

        await autoScreenshot(page, 'SD7-05', 'neg-050', _testStart);
    });
