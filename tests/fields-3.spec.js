...
test.describe('日時フィールド種類変更・バリデーション（19, 47, 97, 101系）', () => {
    ...
    test('19-1: 日時フィールドのフィールド設定ページが正常に表示されること（種類変更確認用）', async ({ page }) => {
        ...
        // 日時のラジオボタンが存在すること
        await expect(modal.locator('input[type="radio"][value="datetime"]')).toBeVisible({ timeout: 10000 });
        // 日付のみのラジオボタンが存在すること
        await expect(modal.locator('input[type="radio"][value="date"]')).toBeVisible({ timeout: 10000 });
        // 時刻のみのラジオボタンが存在すること
        await expect(modal.locator('input[type="radio"][value="time"]')).toBeVisible({ timeout: 10000 });
        // 年月のラジオボタンが存在すること
        await expect(modal.locator('input[type="radio"][value="year_month"]')).toBeVisible({ timeout: 10000 });
        // 現在の種類が「日時」であることを確認
    ...
    test('97-1: 日時フィールドのフィールド設定ページが正常に表示されること（フォーマット「Y/m/d H:i:s」）', async ({ page }) => {
        ...
        await expect(formatLabel.first()).toBeVisible({ timeout: 10000 });
        ...
        if (await formatToggle.count() > 0) {
            ...
            await expect(formatInput.first()).toBeVisible({ timeout: 10000 });
    ...
    test('101-4: 日時フィールドのフィールド設定ページが正常に表示されること（デフォルト現在日時OFF設定用）', async ({ page }) => {
        ...
        await expect(defaultCheckLabel.first()).toBeVisible({ timeout: 10000 });
...
test.describe('項目設定（63, 77系）- 画像/動画URL・計算フィールド', () => {
    ...
    test('63-1: テーブルのヘッダー画像を設定できること', async ({ page }) => {
        ...
        await expect(iconTypeLabel.first()).toBeVisible({ timeout: 10000 });
        // アイコン設定エリアが存在すること
        const iconLabel = menuContent.locator('label').filter({ hasText: 'アイコン' });
        await expect(iconLabel.first()).toBeVisible({ timeout: 10000 });
        // ISEチェック
        const pageText = await page.innerText('body');
        expect(pageText).not.toContain('Internal Server Error');
    });
    ...
    test('63-2: テーブルの動画URLを設定できること', async ({ page }) => {
        ...
        await expect(memoLabel.first()).toBeVisible({ timeout: 10000 });
        // ISEチェック
        const pageText = await page.innerText('body');
        expect(pageText).not.toContain('Internal Server Error');
    });
