...
test.describe('日時フィールド種類変更・バリデーション（19, 47, 97, 101系）', () => {
    ...
    // -------------------------------------------------------------------------
    // 19-1: 日時フィールドの種類変更
    // 日時フィールドの設定モーダルで種類ラジオ（日時/年月/日付のみ/時刻のみ）が表示され、
    // 種類を変更できることを確認する
    // -------------------------------------------------------------------------
    test('19-1: 日時フィールドのフィールド設定ページが正常に表示されること（種類変更確認用）', async ({ page }) => {
        const modal = await openDateTimeFieldModal(page, tableId, '日時');
        // 項目名入力欄が表示されること
        await expect(modal.locator('input[type="text"][name="label"]')).toBeVisible({ timeout: 30000 });
        // 種類ラジオボタンが4つ表示されること（日時/年月/日付のみ/時刻のみ）
        const typeRadios = modal.locator('input[type="radio"][name="_form_data"], input[type="radio"][value="datetime"], input[type="radio"][value="year_month"], input[type="radio"][value="date"], input[type="radio"][value="time"]');
        // 日時のラジオボタンが存在すること
        await expect(modal.locator('input[type="radio"][value="datetime"]')).toBeVisible({ timeout: 30000 });
        // 日付のみのラジオボタンが存在すること
        await expect(modal.locator('input[type="radio"][value="date"]')).toBeVisible({ timeout: 30000 });
        // 時刻のみのラジオボタンが存在すること
        await expect(modal.locator('input[type="radio"][value="time"]')).toBeVisible({ timeout: 30000 });
        // 年月のラジオボタンが存在すること
        await expect(modal.locator('input[type="radio"][value="year_month"]')).toBeVisible({ timeout: 30000 });
        // 現在の種類が「日時」であることを確認
        const datetimeRadio = modal.locator('input[type="radio"][value="datetime"]');
        await expect(datetimeRadio).toBeChecked();
        // 種類を「日付のみ」に変更できることを確認
        await modal.locator('input[type="radio"][value="date"]').click();
        await waitForAngular(page);
        await expect(modal.locator('input[type="radio"][value="date"]')).toBeChecked();
        // 元に戻す（日時に戻す）
        await modal.locator('input[type="radio"][value="datetime"]').click();
        await waitForAngular(page);
        // キャンセルで閉じる（変更を保存しない）
    ...
    // -------------------------------------------------------------------------
    // 47-1: 日時フィールドの項目名未入力バリデーションエラー
    // 日時フィールドの設定モーダルで項目名を空にして保存ボタンを押すと、
    // バリデーションエラー（アラートまたはエラー表示）が発生することを確認する
    // -------------------------------------------------------------------------
    test('47-1: 日時フィールドのフィールド設定ページが正常に表示されること（項目名未入力エラー確認用）', async ({ page }) => {
        const modal = await openDateTimeFieldModal(page, tableId, '日時');
        // 項目名入力欄が表示されること
        const nameInput = modal.locator('input[type="text"][name="label"]');
        ...
        await waitForAngular(page);
        // ダイアログを監視（PigeonCloudはアラートでエラーを表示する場合がある）
        let dialogMessage = '';
        ...
        // 保存ボタンをクリック
        const saveBtn = modal.locator('[data-testid="field-save-btn"], button.btn-primary:has-text("保存"), button.btn-primary:has-text("更新")');
        await saveBtn.first().click();
        await page.waitForTimeout(2000); // バリデーション結果待ち
        // バリデーションエラーが発生すること（アラート or モーダル内エラー表示 or モーダルが閉じない）
        const modalStillVisible = await modal.isVisible();
        // モーダルが開いたまま（=保存に失敗した）か、アラートが出たことを確認
        expect(modalStillVisible || dialogMessage.length > 0).toBeTruthy();
        // 元の項目名を復元してキャンセル
    ...
    // -------------------------------------------------------------------------
    // 49-1: ファイルフィールドの項目名未入力バリデーションエラー
    // ファイルフィールドの設定モーダルで項目名を空にして保存ボタンを押すと、
    // バリデーションエラーが発生することを確認する
    // -------------------------------------------------------------------------
    ...
    // -------------------------------------------------------------------------
    // 97-1: 日時フィールドの表示フォーマット設定（Y/m/d H:i:s）
    // 日時フィールドの設定モーダルで「表示フォーマットを指定する」チェックボックスをONにし、
    // フォーマット入力欄に「Y/m/d H:i:s」を入力できることを確認する
    // -------------------------------------------------------------------------
    test('97-1: 日時フィールドのフィールド設定ページが正常に表示されること（フォーマット「Y/m/d H:i:s」）', async ({ page }) => {
        const modal = await openDateTimeFieldModal(page, tableId, '日時');
        // 「表示フォーマットを指定する」チェックボックスが存在すること
        const formatCheckbox = modal.locator('input[type="checkbox"]').filter({ has: page.locator('xpath=..') }).locator('xpath=ancestor::label').filter({ hasText: '表示フォーマットを指定する' }).locator('input[type="checkbox"]');
        // フォーマット関連のUIを確認（チェックボックスまたはラベル）
        const formatLabel = modal.locator('label').filter({ hasText: 'フォーマット' });
        await expect(formatLabel.first()).toBeVisible({ timeout: 30000 });
        // 「表示フォーマットを指定する」チェックボックスをONにする
        const formatToggle = modal.locator('label').filter({ hasText: '表示フォーマットを指定する' }).locator('input[type="checkbox"]');
        if (await formatToggle.count() > 0) {
            const isChecked = await formatToggle.isChecked();
            if (!isChecked) {
                await formatToggle.click();
                await waitForAngular(page);
            }
            // フォーマット入力欄が表示されること
            const formatInput = modal.locator('input.form-control[placeholder*="Y"]');
            await expect(formatInput.first()).toBeVisible({ timeout: 30000 });
            // フォーマットに「Y/m/d H:i:s」を入力できること
            await formatInput.first().fill('Y/m/d H:i:s');
            const inputValue = await formatInput.first().inputValue();
            expect(inputValue).toBe('Y/m/d H:i:s');
        }
        // キャンセルで閉じる
        await modal.locator('button:has-text("キャンセル")').click();
    });
    ...
    test('97-2: 日時フィールドのフィールド設定ページが正常に表示されること（フォーマットパターン2）', async ({ page }) => {
        ...
        if (await formatToggle.count() > 0) {
            ...
            // フォーマット入力欄に「Y年m月d日 H:i」を入力できること
            const formatInput = modal.locator('input.form-control[placeholder*="Y"]');
            await expect(formatInput.first()).toBeVisible({ timeout: 30000 });
            await formatInput.first().fill('Y年m月d日 H:i');
            const inputValue = await formatInput.first().inputValue();
            expect(inputValue).toBe('Y年m月d日 H:i');
        }
        await modal.locator('button:has-text("キャンセル")').click();
    });

    // -------------------------------------------------------------------------
    // 97-3: 日時フィールドの表示フォーマット設定（パターン3: m/d H:i:s）
    // -------------------------------------------------------------------------
    test('97-3: 日時フィールドのフィールド設定ページが正常に表示されること（フォーマットパターン3）', async ({ page }) => {
        ...
        if (await formatToggle.count() > 0) {
            ...
            const formatInput = modal.locator('input.form-control[placeholder*="Y"]');
            await expect(formatInput.first()).toBeVisible({ timeout: 30000 });
            await formatInput.first().fill('m/d H:i:s');
            const inputValue = await formatInput.first().inputValue();
            expect(inputValue).toBe('m/d H:i:s');
        }
        await modal.locator('button:has-text("キャンセル")').click();
    });

    // -------------------------------------------------------------------------
    // 97-4: 日時フィールドの表示フォーマット設定（パターン4: Y-m-d）
    // -------------------------------------------------------------------------
    test('97-4: 日時フィールドのフィールド設定ページが正常に表示されること（フォーマットパターン4）', async ({ page }) => {
        ...
        if (await formatToggle.count() > 0) {
            ...
            const formatInput = modal.locator('input.form-control[placeholder*="Y"]');
            await expect(formatInput.first()).toBeVisible({ timeout: 30000 });
            await formatInput.first().fill('Y-m-d');
            const inputValue = await formatInput.first().inputValue();
            expect(inputValue).toBe('Y-m-d');
        }
        await modal.locator('button:has-text("キャンセル")').click();
    });

    // -------------------------------------------------------------------------
    // 97-5: 日時フィールドの表示フォーマット設定（パターン5: n/d H:i）
    // -------------------------------------------------------------------------
    test('97-5: 日時フィールドのフィールド設定ページが正常に表示されること（フォーマットパターン5）', async ({ page }) => {
        ...
        if (await formatToggle.count() > 0) {
            ...
            const formatInput = modal.locator('input.form-control[placeholder*="Y"]');
            await expect(formatInput.first()).toBeVisible({ timeout: 30000 });
            await formatInput.first().fill('n/d H:i');
            const inputValue = await formatInput.first().inputValue();
            expect(inputValue).toBe('n/d H:i');
        }
        await modal.locator('button:has-text("キャンセル")').click();
    });
    ...
    // 日時フィールドの設定モーダルで「デフォルトで現在日時(時刻)をセット」チェックボックスが
    // 存在し、OFF操作ができることを確認する
    // -------------------------------------------------------------------------
    test('101-4: 日時フィールドのフィールド設定ページが正常に表示されること（デフォルト現在日時OFF設定用）', async ({ page }) => {
        const modal = await openDateTimeFieldModal(page, tableId, '日時');
        // 種類が「日時」であることを確認
        await expect(modal.locator('input[type="radio"][value="datetime"]')).toBeChecked();
        // 「デフォルトで現在日時(時刻)をセット」チェックボックスが存在すること
        const defaultCheckLabel = modal.locator('label').filter({ hasText: 'デフォルトで現在日時' });
        await expect(defaultCheckLabel.first()).toBeVisible({ timeout: 30000 });
        // チェックボックスの操作確認（OFF→ON→OFF）
        const defaultCheck = modal.locator('label').filter({ hasText: /デフォルトで現在日時/ }).locator('input[type="checkbox"]');
        if (await defaultCheck.count() > 0) {
            const wasChecked = await defaultCheck.isChecked();
            // トグル操作
            await defaultCheck.click();
            await waitForAngular(page);
            const afterToggle = await defaultCheck.isChecked();
    ...
    // 日時フィールドの設定モーダルで種類を「日付のみ」に変更し、
    // デフォルト現在日付のチェックボックスをOFF操作できることを確認する
    // -------------------------------------------------------------------------
    test('101-5: 日付のみフィールドのフィールド設定ページが正常に表示されること（デフォルト現在日付OFF設定用）', async ({ page }) => {
        ...
        const defaultCheckLabel = modal.locator('label').filter({ hasText: /デフォルトで現在日時/ });
        await expect(defaultCheckLabel.first()).toBeVisible({ timeout: 30000 });
        const defaultCheck = defaultCheckLabel.locator('input[type="checkbox"]');
        if (await defaultCheck.count() > 0) {
            const wasChecked = await defaultCheck.isChecked();
            await defaultCheck.click();
            await waitForAngular(page);
            expect(await defaultCheck.isChecked()).toBe(!wasChecked);
            // 元に戻す
            await defaultCheck.click();
        }
        // 種類を元の「日時」に戻す
    ...
    // 日時フィールドの設定モーダルで種類を「時刻のみ」に変更し、
    // デフォルト現在時刻のチェックボックスをOFF操作できることを確認する
    // -------------------------------------------------------------------------
    test('101-6: 時刻のみフィールドのフィールド設定ページが正常に表示されること（デフォルト現在時刻OFF設定用）', async ({ page }) => {
        const modal = await openDateTimeFieldModal(page, tableId, '日時');
        // 種類を「時刻のみ」に変更
        await modal.locator('input[type="radio"][value="time"]').click();
        await waitForAngular(page);
        await expect(modal.locator('input[type="radio"][value="time"]')).toBeChecked();
        // デフォルトチェックボックスが存在すること
        const defaultCheckLabel = modal.locator('label').filter({ hasText: /デフォルトで現在日時/ });
        await expect(defaultCheckLabel.first()).toBeVisible({ timeout: 30000 });
        const defaultCheck = defaultCheckLabel.locator('input[type="checkbox"]');
        if (await defaultCheck.count() > 0) {
    ...
    // 日時フィールドの設定モーダルで種類を「年月」に変更し、
    // デフォルト現在年月のチェックボックスをOFF操作できることを確認する
    // -------------------------------------------------------------------------
    test('101-8: 年月フィールドのフィールド設定ページが正常に表示されること（デフォルト現在年月OFF設定用）', async ({ page }) => {
        const modal = await openDateTimeFieldModal(page, tableId, '日時');
        // 種類を「年月」に変更
        await modal.locator('input[type="radio"][value="year_month"]').click();
        await waitForAngular(page);
        await expect(modal.locator('input[type="radio"][value="year_month"]')).toBeChecked();
        // デフォルトチェックボックスが存在すること
        const defaultCheckLabel = modal.locator('label').filter({ hasText: /デフォルトで現在日時/ });
        await expect(defaultCheckLabel.first()).toBeVisible({ timeout: 30000 });
        const defaultCheck = defaultCheckLabel.locator('input[type="checkbox"]');
...
test.describe('項目設定（63, 77系）- 画像/動画URL・計算フィールド', () => {
    ...
    // -------------------------------------------------------------------------
    // 63-1: テーブルのヘッダー画像設定
    // テーブル設定ページのメニュータブでアイコン画像の設定UIが表示されることを確認する
    // -------------------------------------------------------------------------
    test('63-1: テーブルのヘッダー画像を設定できること', async ({ page }) => {
        await navigateToFieldPage(page, tableId);
        await assertFieldPageLoaded(page, tableId);
        // テーブル設定エリアのタブが表示されること
        const tabs = page.locator('.dataset-tabs [ngbnavlink], .dataset-tabs a');
        await expect(tabs.first()).toBeVisible({ timeout: 15000 });
        // メニュータブをクリック
        await clickSettingTab(page, 'メニュー');
        // メニュータブのコンテンツが表示されること
        const menuContent = page.locator('.dataset-tab-content');
        await expect(menuContent).toBeVisible({ timeout: 30000 });
        // アイコンタイプの設定UIが存在すること
        const iconTypeLabel = menuContent.locator('label').filter({ hasText: 'アイコンタイプ' });
        await expect(iconTypeLabel.first()).toBeVisible({ timeout: 30000 });
        // アイコン設定エリアが存在すること
        const iconLabel = menuContent.locator('label').filter({ hasText: 'アイコン' });
        await expect(iconLabel.first()).toBeVisible({ timeout: 30000 });
        // ISEチェック
        const pageText = await page.innerText('body');
        expect(pageText).not.toContain('Internal Server Error');
    });
    ...
    // -------------------------------------------------------------------------
    // 63-2: テーブルの動画URL設定
    // テーブル設定ページの基本設定タブで「上部にメモを表示する」等の設定UIが
    // 正常に表示されることを確認する（動画URLは上部メモのHTMLエリアで設定可能）
    // -------------------------------------------------------------------------
    test('63-2: テーブルの動画URLを設定できること', async ({ page }) => {
        ...
        await expect(tabs.first()).toBeVisible({ timeout: 15000 });
        // 基本設定タブをクリック
        await clickSettingTab(page, '基本設定');
        const tabContent = page.locator('.dataset-tab-content');
        await expect(tabContent).toBeVisible({ timeout: 30000 });
        // テーブル名の設定UIが存在すること
        const tableNameInput = tabContent.locator('#table_name, input[type="text"]').first();
        await expect(tableNameInput).toBeVisible({ timeout: 30000 });
        // 「上部にメモを表示する」トグルが存在すること
        const memoLabel = tabContent.locator('label').filter({ hasText: '上部にメモを表示する' });
        await expect(memoLabel.first()).toBeVisible({ timeout: 30000 });
        // ISEチェック
        const pageText = await page.innerText('body');
        expect(pageText).not.toContain('Internal Server Error');
    });
    ...
    // -------------------------------------------------------------------------
    // 63-3: 項目設定（テキストフィールドの設定モーダル）
    // テキストフィールドの歯車をクリックして設定モーダルが開き、
    // 種類ラジオ（テキスト/メールアドレス/URL）が表示されることを確認する
    // -------------------------------------------------------------------------
    test('63-3: 項目設定ページが正常に表示されること（パターン3）', async ({ page }) => {
        await navigateToFieldPage(page, tableId);
        await assertFieldPageLoaded(page, tableId);
        // テキストフィールドを探す
        const textField = page.locator('.pc-field-block').filter({ hasText: 'テキスト' });
        await expect(textField.first()).toBeVisible({ timeout: 15000 });
        // 歯車アイコンをクリック
        await textField.first().hover();
        await textField.first().locator('.overSetting .fa-gear').click();
        await waitForAngular(page);
        const modal = page.locator('.settingModal .modal-content');
        await expect(modal).toBeVisible({ timeout: 15000 });
        // 項目名入力欄が表示されること
        await expect(modal.locator('input[type="text"][name="label"]')).toBeVisible({ timeout: 30000 });
        // テキスト種類のラジオボタンが表示されること（テキスト/メールアドレス/URL）
    ...
    test('63-4: 固定テキストフィールドでリスト（箇条書き）を挿入できること', async ({ page }) => {
        await navigateToFieldPage(page, tableId);
        await assertFieldPageLoaded(page, tableId);
        // 固定テキストフィールドを探してモーダルを開く
        ...
        if (await fixedField.count() > 0) {
            ...
            await expect(modal).toBeVisible({ timeout: 15000 });
            // Froalaエディタが表示されること
            const froala = modal.locator('.fr-box, [froalaeditor]');
            await expect(froala.first()).toBeVisible({ timeout: 15000 });
            // ツールバーにリスト挿入ボタンが存在すること（箇条書き=unordered list）
            const listBtn = modal.locator('.fr-toolbar button[data-cmd="formatUL"], .fr-toolbar button[title*="リスト"], .fr-toolbar button.fr-command[id*="list"]');
            // ツールバーボタンの存在を確認（Froalaツールバー自体は表示されている）
            const toolbar = modal.locator('.fr-toolbar');
            ...
            // 固定テキストフィールドがない場合はフィールド追加メニューで存在を確認
            const addBtn = page.locator('button:has-text("項目追加"), .add-field-btn').first();
            await expect(addBtn).toBeVisible({ timeout: 15000 });
            throw new Error('固定テキストフィールドがALLテストテーブルに存在しません');
        }
    });

    test('63-5: 固定テキストフィールドでオーダーリスト（番号リスト）を挿入できること', async ({ page }) => {
        await navigateToFieldPage(page, tableId);
        await assertFieldPageLoaded(page, tableId);
        const fixedField = page.locator('.pc-field-block').filter({ hasText: '固定テキスト' });
        if (await fixedField.count() > 0) {
            ...
            const modal = page.locator('.settingModal .modal-content');
            await expect(modal).toBeVisible({ timeout: 15000 });
            // Froalaエディタのツールバーが表示されること
            const toolbar = modal.locator('.fr-toolbar');
            await expect(toolbar.first()).toBeVisible({ timeout: 10000 });
            // 番号付きリストボタンが存在すること
            const olBtn = modal.locator('.fr-toolbar button[data-cmd="formatOL"]');
            if (await olBtn.count() > 0) {
                await expect(olBtn.first()).toBeVisible();
            }
            // ISEチェック
            const pageText = await page.innerText('body');
            expect(pageText).not.toContain('Internal Server Error');
    ...
    test('63-6: 固定テキストフィールドでライン（水平線）を挿入できること', async ({ page }) => {
        await navigateToFieldPage(page, tableId);
        await assertFieldPageLoaded(page, tableId);
        const fixedField = page.locator('.pc-field-block').filter({ hasText: '固定テキスト' });
        if (await fixedField.count() > 0) {
            ...
            await expect(modal).toBeVisible({ timeout: 15000 });
            // Froalaエディタが表示されること
            const froalaEditor = modal.locator('.fr-element[contenteditable="true"]');
            await expect(froalaEditor.first()).toBeVisible({ timeout: 15000 });
            // ツールバーに水平線ボタンが存在すること
            const hrBtn = modal.locator('.fr-toolbar button[data-cmd="insertHR"]');
            if (await hrBtn.count() > 0) {
                await expect(hrBtn.first()).toBeVisible();
            }
            const pageText = await page.innerText('body');
            expect(pageText).not.toContain('Internal Server Error');
            await modal.locator('button:has-text("キャンセル")').click().catch(() => {});
    ...
    test('63-7: 固定テキストフィールドにテキストを入力して保存できること', async ({ page }) => {
        await navigateToFieldPage(page, tableId);
        await assertFieldPageLoaded(page, tableId);
        const fixedField = page.locator('.pc-field-block').filter({ hasText: '固定テキスト' });
        if (await fixedField.count() > 0) {
            ...
            await expect(froalaEditor.first()).toBeVisible({ timeout: 15000 });
            // テキスト入力が可能なことを確認
            await froalaEditor.first().click();
            await froalaEditor.first().type('テスト固定テキスト63-7');
            const editorText = await froalaEditor.first().innerText();
            expect(editorText).toContain('テスト固定テキスト63-7');
            const pageText = await page.innerText('body');
            expect(pageText).not.toContain('Internal Server Error');
            await modal.locator('button:has-text("キャンセル")').click().catch(() => {});
        } else {
    ...
    test('63-8: 固定テキストフィールドの画像挿入でテキストファイル選択時にエラーとならないこと（異常系）', async ({ page }) => {
        await navigateToFieldPage(page, tableId);
        await assertFieldPageLoaded(page, tableId);
        const fixedField = page.locator('.pc-field-block').filter({ hasText: '固定テキスト' });
        if (await fixedField.count() > 0) {
            ...
            const froalaEditor = modal.locator('.fr-element[contenteditable="true"]');
            await expect(froalaEditor.first()).toBeVisible({ timeout: 15000 });
            // ツールバーに画像挿入ボタンが存在すること
            const imgBtn = modal.locator('.fr-toolbar button[data-cmd="insertImage"]');
            if (await imgBtn.count() > 0) {
                await expect(imgBtn.first()).toBeVisible();
            }
            // ISEが発生しないこと
            const pageText = await page.innerText('body');
            expect(pageText).not.toContain('Internal Server Error');
    ...
    test('63-9: 固定テキストフィールドの動画挿入でテキストファイル選択時にエラーとならないこと（異常系）', async ({ page }) => {
        await navigateToFieldPage(page, tableId);
        await assertFieldPageLoaded(page, tableId);
        const fixedField = page.locator('.pc-field-block').filter({ hasText: '固定テキスト' });
        if (await fixedField.count() > 0) {
            ...
            const froalaEditor = modal.locator('.fr-element[contenteditable="true"]');
            await expect(froalaEditor.first()).toBeVisible({ timeout: 15000 });
            // ツールバーに動画挿入ボタンが存在すること
            const videoBtn = modal.locator('.fr-toolbar button[data-cmd="insertVideo"]');
            if (await videoBtn.count() > 0) {
                await expect(videoBtn.first()).toBeVisible();
            }
            // ISEが発生しないこと
            const pageText = await page.innerText('body');
            expect(pageText).not.toContain('Internal Server Error');
    ...
    // 77-1: 計算フィールド（IF関数）
    // -------------------------------------------------------------------------
    test('77-1: 計算フィールドの設定モーダルでIF関数を設定できること', async ({ page }) => {
        await navigateToFieldPage(page, tableId);
        await assertFieldPageLoaded(page, tableId);
        // 計算フィールドを探してモーダルを開く
        const calcField = page.locator('.pc-field-block').filter({ hasText: '計算' });
        await expect(calcField.first()).toBeVisible({ timeout: 15000 });
        await calcField.first().hover();
        await calcField.first().locator('.overSetting .fa-gear').click();
        await waitForAngular(page);
        const modal = page.locator('.settingModal .modal-content');
        await expect(modal).toBeVisible({ timeout: 15000 });
        // 計算式の入力エリア（textarea/input）が表示されること
        const formulaInput = modal.locator('textarea, input[type="text"]').filter({ hasText: '' });
        const formulaArea = modal.locator('.calc-formula, .formula-editor, textarea, .CodeMirror');
        expect(await formulaArea.count()).toBeGreaterThan(0);
        // 関数選択ドロップダウンまたはIF関数が入力可能なことを確認
        const modalText = await modal.innerText();
        // 計算フィールドの設定であることを確認（「計算」「関数」「式」のいずれかが含まれること）
        const hasCalcUI = modalText.includes('計算') || modalText.includes('関数') || modalText.includes('式') || modalText.includes('項目');
        expect(hasCalcUI).toBeTruthy();
        const pageText = await page.innerText('body');
        expect(pageText).not.toContain('Internal Server Error');
        await modal.locator('button:has-text("キャンセル")').click().catch(() => {});
    });
    ...
    // -------------------------------------------------------------------------
    test('77-2: 計算フィールドの設定モーダルで別の関数を設定できること', async ({ page }) => {
        await navigateToFieldPage(page, tableId);
        await assertFieldPageLoaded(page, tableId);
        // 計算フィールドを探す（2つ目がある場合はそちらを使う）
        const calcFields = page.locator('.pc-field-block').filter({ hasText: '計算' });
        const calcIdx = (await calcFields.count()) > 1 ? 1 : 0;
        await calcFields.nth(calcIdx).hover();
        await calcFields.nth(calcIdx).locator('.overSetting .fa-gear').click();
        await waitForAngular(page);
        const modal = page.locator('.settingModal .modal-content');
        await expect(modal).toBeVisible({ timeout: 15000 });
        // 設定モーダルに項目名入力欄が存在すること
        const labelInput = modal.locator('input[name="label"], input[type="text"]').first();
...
test.describe('項目設定 追加ケース（115〜149系）', () => {
    ...
    // -------------------------------------------------------------------------
    test('134-01: 選択肢(複数選択)チェックボックスの選択肢制限（最小/最大）を設定できること', async ({ page }) => {
        await navigateToFieldPage(page, tableId);
        await assertFieldPageLoaded(page, tableId);
        // 選択肢（複数選択）フィールドを探してモーダルを開く
        const multiField = page.locator('.pc-field-block').filter({ hasText: '選択肢(複数選択)' });
        if (await multiField.count() === 0) {
            // 別の表記で探す
            const altField = page.locator('.pc-field-block').filter({ hasText: '複数選択' });
            expect(await altField.count()).toBeGreaterThan(0);
        }
        const target = (await multiField.count() > 0) ? multiField : page.locator('.pc-field-block').filter({ hasText: '複数選択' });
        await target.first().hover();
        await target.first().locator('.overSetting .fa-gear').click();
        await waitForAngular(page);
        const modal = page.locator('.settingModal .modal-content');
        await expect(modal).toBeVisible({ timeout: 15000 });
        // 種類ラジオにチェックボックスが存在すること
        const modalText = await modal.innerText();
        expect(modalText).toContain('チェックボックス');
        // 選択肢の制限（最小/最大）UIが存在すること
        const hasLimitUI = modalText.includes('選択肢の制限') || modalText.includes('最小') || modalText.includes('最大');
    ...
    test('134-02: 選択肢(複数選択)プルダウンの選択肢制限（最小/最大）を設定できること', async ({ page }) => {
        await navigateToFieldPage(page, tableId);
        await assertFieldPageLoaded(page, tableId);
        const multiField = page.locator('.pc-field-block').filter({ hasText: '選択肢(複数選択)' });
        const target = (await multiField.count() > 0) ? multiField : page.locator('.pc-field-block').filter({ hasText: '複数選択' });
        ...
        const modal = page.locator('.settingModal .modal-content');
        await expect(modal).toBeVisible({ timeout: 15000 });
        // モーダル内にプルダウン種類の選択肢が存在すること
        const modalText = await modal.innerText();
        expect(modalText).toContain('プルダウン');
        // 選択肢の制限UIが存在すること
        const hasLimitUI = modalText.includes('選択肢の制限') || modalText.includes('最小') || modalText.includes('最大');
        expect(hasLimitUI).toBeTruthy();
        const pageText = await page.innerText('body');
    ...
    test('134-03: 選択肢(複数選択)チェックボックスの選択肢制限を空で保存できること', async ({ page }) => {
        await navigateToFieldPage(page, tableId);
        await assertFieldPageLoaded(page, tableId);
        const multiField = page.locator('.pc-field-block').filter({ hasText: '選択肢(複数選択)' });
        const target = (await multiField.count() > 0) ? multiField : page.locator('.pc-field-block').filter({ hasText: '複数選択' });
        ...
        const modal = page.locator('.settingModal .modal-content');
        await expect(modal).toBeVisible({ timeout: 15000 });
        // チェックボックス種類ラジオが表示されること
        const checkboxRadio = modal.locator('input[type="radio"]').filter({ has: page.locator('..') });
        const modalText = await modal.innerText();
        expect(modalText).toContain('チェックボックス');
        // 制限の最小/最大フィールドが空であることを確認（制限なし状態）
        const limitInputs = modal.locator('input[type="number"]');
        if (await limitInputs.count() > 0) {
            // 数値入力欄が存在する場合、空でもエラーにならないことを確認
            const firstVal = await limitInputs.first().inputValue();
            // 空文字列または0であること（制限なし状態）
            expect(firstVal === '' || firstVal === '0' || firstVal === null).toBeTruthy();
        }
        const pageText = await page.innerText('body');
        expect(pageText).not.toContain('Internal Server Error');
        await modal.locator('button:has-text("キャンセル")').click().catch(() => {});
    ...
    test('134-04: 選択肢(複数選択)プルダウンの選択肢制限を空で保存できること', async ({ page }) => {
        await navigateToFieldPage(page, tableId);
        await assertFieldPageLoaded(page, tableId);
        const multiField = page.locator('.pc-field-block').filter({ hasText: '選択肢(複数選択)' });
        const target = (await multiField.count() > 0) ? multiField : page.locator('.pc-field-block').filter({ hasText: '複数選択' });
    ...
    test('147-01: 文字列一行フィールドに10000文字入力してエラーなく保存できること', async ({ page }) => {
        ...
        await waitForAngular(page);
        // レコード追加画面が表示されること
        await expect(page.locator('.card-footer, .form-group')).toBeVisible({ timeout: 30000 });
        // 文字列一行フィールドに10000文字入力
        ...
        expect(inputVal.length).toBe(10000);
    });

    // -------------------------------------------------------------------------
    // 149-1〜149-18: 項目設定（各種）
    // -------------------------------------------------------------------------
    test('149-1: 項目権限設定で組織のみ・閲覧ON/編集OFFの設定画面が表示されること', async ({ page }) => {
        await navigateToFieldPage(page, tableId);
        await assertFieldPageLoaded(page, tableId);
        // 権限設定タブに切り替え
        await clickSettingTab(page, '権限設定');
        await waitForAngular(page);
        const tabContent = page.locator('.dataset-tab-content, .tab-content, .tab-pane.active');
        const bodyText = await page.innerText('body');
        expect(bodyText).not.toContain('Internal Server Error');
        // 権限設定タブが表示されていること
        expect(bodyText).toContain('権限');
        // 「高度な設定」「項目権限設定」のリンクまたはセクションが存在すること
        const advancedLink = page.locator('text=高度な設定, text=項目権限, text=項目ごとの権限');
        if (await advancedLink.count() > 0) {
            await advancedLink.first().click().catch(() => {});
            await waitForAngular(page);
            const bodyText2 = await page.innerText('body');
            // 閲覧権限/編集権限の設定UIが存在すること
            const hasPermUI = bodyText2.includes('閲覧') || bodyText2.includes('編集') || bodyText2.includes('権限');
            expect(hasPermUI).toBeTruthy();
        }
    });

    test('149-2: 項目権限設定で組織ユーザーのみ・閲覧ON/編集OFFの設定画面が表示されること', async ({ page }) => {
        await navigateToFieldPage(page, tableId);
        await assertFieldPageLoaded(page, tableId);
        // 権限設定タブに切り替え
        await clickSettingTab(page, '権限設定');
        ...
        // 権限設定画面が表示されていること
        expect(bodyText).toContain('権限');
        // 権限グループ設定UIが表示されていること
        const hasGroupUI = bodyText.includes('権限グループ') || bodyText.includes('ユーザー') || bodyText.includes('組織');
        expect(hasGroupUI).toBeTruthy();
    });
