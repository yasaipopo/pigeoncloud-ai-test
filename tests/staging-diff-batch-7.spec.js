// @ts-check
/**
 * staging 差分 第 8 弾: 10 件 — 品質横断観点 (a11y + キーボード + バリデーション + エラー UI)
 *
 * batch-1〜6 で staging 差分の主要 PR 観点はほぼカバー完了。
 * 本弾は staging 差分から離れて、これまで未カバーだった
 * 「**品質ガードレール**」観点 5 種で構造的回帰テストを 10 件追加:
 *
 * - a11y      : アクセシビリティ (見出し / label 紐付け)
 * - keyboard  : キーボード操作 (Tab / Escape)
 * - validation: フォームバリデーション境界値 (必須 / 数値 / 文字数)
 * - error-ui  : 不正 URL/不正 ID で ISE が出ない
 * - resp      : レスポンシブ (リサイズで UI 維持)
 *
 * テスト ID:
 * - a11y-010   ダッシュボード画面に主要見出し (h1/h2) が存在
 * - a11y-020   ログイン画面 input に label/aria-label 紐付け
 * - keyb-010   ログイン画面 Tab キーフォーカス順序 (ID→PW→submit)
 * - keyb-020   ALLテーブル編集 → Escape でモーダル/画面が閉じる
...
const autoScreenshot = createAutoScreenshot('staging-diff-batch-7');
...
test.describe.serial('staging 差分 第 8 弾 (10 件 品質横断: a11y + キーボード + バリデーション + エラー UI)', () => {
    ...
    test.beforeAll(async ({ browser }) => {
        try {
            ...
            console.error('[staging-diff-batch-7 beforeAll]', e.message);
