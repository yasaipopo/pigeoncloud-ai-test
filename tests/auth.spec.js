...
/**
 * 別コンテキストのページ用スクショ（firefoxPage等）
 * autoScreenshotと同じ命名規則でファイルを保存する
 */
async function screenshotAnyPage(targetPage, movie, caseNo, checkIndex, testStartTime) {
    const { getScreenshotId } = require('./helpers/screenshot-mapper-util');
    ...
    const map = JSON.parse(require('fs').readFileSync(require('path').join(__dirname, 'helpers', 'screenshot-map.json'), 'utf8'));
    const movieData = map['auth']?.[movie];
    let stepId;
    if (movieData) {
        const firstCase = movieData.firstCase;
        const checks = movieData.checks?.[caseNo] || [];
        stepId = (checkIndex < checks.length) ? `${firstCase}-s${checks[checkIndex]}` : `${firstCase}-s${caseNo}-${checkIndex}`;
    } else {
        stepId = `${caseNo}-s${checkIndex}`;
    }
    const dir = `${reportsDir}/steps/auth/${movie}`;
    require('fs').mkdirSync(dir, { recursive: true });
    const filePath = `${dir}/${stepId}.jpg`;
    await targetPage.screenshot({ path: filePath, type: 'jpeg', quality: 30, fullPage: false }).catch(() => {});
...
test.describe('認証（ログイン・ログアウト・パスワード変更）', () => {
    ...
    test('AT01: 認証基本フロー', async ({ page }) => {
        ...
        await test.step('144-01: 推奨ブラウザ以外でアクセスすると警告メッセージが表示されること', async () => {
            const stepSec = await autoScreenshot(page, 'AT01', 'auth-010', 0, _testStart);
            ...
            // ステップ2スクショ: ログインページ表示
            await screenshotAnyPage(firefoxPage, 'AT01', 'auth-010', 0, _testStart);
            ...
            // s4: 3+4は同じ画面（警告バー表示）→ 最後の✅の後に1枚
            await screenshotAnyPage(firefoxPage, 'AT01', 'auth-010', 1, _testStart);
        ...
        await test.step('1-1: マスターユーザーでログイン・ログアウトが完了すること', async () => {
            await autoScreenshot(page, 'AT01', 'auth-020', 0, _testStart);
            await page.context().clearCookies();
        ...
        if (!page.url().includes('/login')) {
            ...
            await autoScreenshot(page, 'AT01', 'auth-020', 1, _testStart);
            ...
            await autoScreenshot(page, 'AT01', 'auth-020', 2, _testStart);
        });
        ...
        await test.step('1-2: ユーザータイプ「ユーザー」でログイン・ログアウトが完了すること', async () => {
            await autoScreenshot(page, 'AT01', 'auth-030', 0, _testStart);
            ...
            await expect(page.locator('.navbar')).toBeVisible({ timeout: 15000 });
            // s3: ✅ ダッシュボード遷移確認
            await autoScreenshot(page, 'AT01', 'auth-030', 1, _testStart);
            ...
            await expect(page).toHaveURL(/\/admin\/login/);
            await expect(page.locator('#id')).toBeVisible();
            // s4: ✅ /admin/login に戻った確認
            await autoScreenshot(page, 'AT01', 'auth-030', 2, _testStart);
        });
        ...
        await test.step('38-1: 誤ったパスワードでログインエラーが発生すること', async () => {
            await autoScreenshot(page, 'AT01', 'auth-040', 0, _testStart);
            ...
            await expect(page).toHaveURL(/\/admin\/login/);
            // s2: ✅ URLがログインページのまま
            await autoScreenshot(page, 'AT01', 'auth-040', 1, _testStart);
            ...
            // s3: ✅ エラーメッセージ表示
            await autoScreenshot(page, 'AT01', 'auth-040', 2, _testStart);
        ...
        await test.step('295: パスワード変更機能が想定通りに動作すること', async () => {
            await autoScreenshot(page, 'AT01', 'auth-050', 0, _testStart);
            ...
            // s4: ✅ パスワード変更フォーム表示
            await autoScreenshot(page, 'AT01', 'auth-050', 1, _testStart);
            ...
            // s6: ✅ ダッシュボード遷移（パスワード変更成功）
            await autoScreenshot(page, 'AT01', 'auth-050', 2, _testStart);
    ...
    test('AT02: ログインセッション管理', async ({ page }) => {
        ...
        await test.step('176: Googleログインボタンが表示され、クリックでGoogle認証画面に遷移すること', async () => {
            await autoScreenshot(page, 'AT02', 'auth-060', 0, _testStart);
            ...
            // s2: ✅ Googleログインボタン表示
            await autoScreenshot(page, 'AT02', 'auth-060', 1, _testStart);
            ...
            // s4: ✅ クリック後の状態（Google認証画面 or SSOリクエスト発生）
            await autoScreenshot(page, 'AT02', 'auth-060', 2, _testStart);
        ...
        await test.step('212-1: 同時ログイン制御機能が設定人数まで動作すること', async () => {
            await autoScreenshot(page, 'AT02', 'auth-070', 0, _testStart);
            ...
            // s2: ✅ 1つ目ログイン成功
            await screenshotAnyPage(masterPage, 'AT02', 'auth-070', 0, _testStart);
            // s3: ✅ 2つ目ログイン成功
            await screenshotAnyPage(userPage, 'AT02', 'auth-070', 1, _testStart);
        ...
        await test.step('212-2: マスターユーザーがユーザーを強制ログアウトできること', async () => {
            await autoScreenshot(page, 'AT02', 'auth-080', 0, _testStart);
            const browser = page.context().browser();
            const masterContext = await browser.newContext({ storageState: { cookies: [], origins: [] } });
            const masterPage = await masterContext.newPage();
            ...
            // s4: ✅ テストユーザーのセッション無効確認
            await screenshotAnyPage(userPage, 'AT02', 'auth-080', 0, _testStart);
        ...
        await test.step('212-3: 自動ログアウト時間設定が機能すること', async () => {
            await autoScreenshot(page, 'AT02', 'auth-090', 0, _testStart);
            ...
            // s2: ✅ 自動ログアウト時間が「1」に設定されたこと
            await autoScreenshot(page, 'AT02', 'auth-090', 1, _testStart);
        ...
        await test.step('212-4: 全端末からのログアウトが機能すること', async () => {
            await autoScreenshot(page, 'AT02', 'auth-100', 0, _testStart);
            ...
            // s4: ✅ ログアウト後ログインページにリダイレクト
            await autoScreenshot(page, 'AT02', 'auth-100', 1, _testStart);
        });
    ...
    test('UC01: 2段階認証設定', async ({ page }) => {
        ...
        await test.step('267: ログインIDがメールアドレス形式でない場合に2段階認証設定でエラーが表示されること', async () => {
            await autoScreenshot(page, 'UC01', 'auth-110', 0, _testStart);
