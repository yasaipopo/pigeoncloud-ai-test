...
test.describe('認証（ログイン・ログアウト・パスワード管理）', () => {
    ...
    test.describe('2段階認証詳細', () => {
        ...
        test('auth-120: 2FA（QRコード有効化）', async ({ page }) => {
            ...
            await page.goto(BASE_URL + '/admin/setting/account', { waitUntil: 'domcontentloaded' });
            await waitForAngular(page);
        ...
        test('auth-130: 2FA TOTP 認証成功', async ({ page }) => {
            ...
            // [flow] 130-3. アカウント設定画面を開き、2FA を QR 方式で有効化
            await page.goto(BASE_URL + '/admin/setting/account', { waitUntil: 'domcontentloaded', timeout: 30000 });
            await waitForAngular(page);
            const qrRadio = page.locator('input[value="qr"]');
            if (await qrRadio.count() > 0) {
                await qrRadio.first().click({ force: true }).catch(() => {});
                await waitForAngular(page);
            }
            ...
            expect(secret, 'TOTP シークレットが取得できること（デバッグAPIで）').toBeTruthy();
    ...
    test.describe('セッション管理', () => {
        ...
        test('auth-150: PW変更による他端末の強制ログアウト', async ({ browser }) => {
            ...
            await page1.goto(BASE_URL + '/admin/setting/account');
    ...
    test.describe('パスワードポリシー', () => {
        ...
        test('auth-230: 過去パスワード再利用禁止', async ({ page }) => {
            ...
            await page.fill('#confirm_new_password', PASSWORD);
            await page.click('button:has-text("保存")');
    ...
    test.describe('UC07: SSO・セキュリティ設定', () => {
        ...
        test('auth-260: マルチテナント分離', async ({ browser, page }) => {
            ...
            // [flow] 260-4. テナント A の Cookie を新規コンテキストにコピーし、テナント B の URL にアクセス
            const crossContext = await browser.newContext();
            // テナント A のホスト Cookie を B に付与（ドメイン変換が必要な場合は domain を B のホストに書き換える）
            ...
            await crossPage.goto(envB.baseUrl + '/admin/dashboard', { waitUntil: 'domcontentloaded', timeout: 15000 }).catch(() => {});

            // [check] 260-5. ✅ テナント B の Cookie でないためダッシュボードに入れず、ログイン画面にリダイレクトされること
            const finalUrl = crossPage.url();
            const isRedirectedToLogin = finalUrl.includes('/admin/login') || !finalUrl.includes('/admin/dashboard');
            expect(isRedirectedToLogin, 'テナントA Cookie ではテナントBダッシュボードに入れないこと').toBe(true);
        ...
        test('auth-280: InternalAuthMiddleware 認証', async ({ page }) => {
            ...
            // [flow] 280-1. X-Manage-Key ヘッダなしで /api/internal/* を叩く
            const withoutKey = await page.evaluate(async (baseUrl) => {
                try {
                    const r = await fetch(baseUrl + '/api/internal/status', {
                        method: 'GET',
                        headers: { 'X-Requested-With': 'XMLHttpRequest' },
                    });
                    return { status: r.status };
            ...
            // [check] 280-2. ✅ 403 Forbidden もしくは 401 Unauthorized が返ること
            expect([401, 403]).toContain(withoutKey.status);
            ...
            if (internalKey) {
                ...
                const withKey = await page.evaluate(async ({ baseUrl, key }) => {
                    const r = await fetch(baseUrl + '/api/internal/status', {
                        method: 'GET',
                        headers: { 'X-Requested-With': 'XMLHttpRequest', 'X-Manage-Key': key },
                    });
                    return { status: r.status };
        ...
        test('auth-290: クライアント証明書 UI とエラー', async ({ page }) => {
            ...
            // [flow] 290-2. クライアント証明書設定画面を開く
            await page.goto(BASE_URL + '/admin/setting/client-cert', { waitUntil: 'domcontentloaded', timeout: 30000 });
            await waitForAngular(page);

            // [check] 290-3. ✅ 画面が 500 エラーを出さずに表示されること
            const bodyText = await page.locator('body').innerText();
            expect(bodyText).not.toContain('Internal Server Error');

            // [check] 290-4. ✅ 「発行」ボタン（新規作成）が存在すること
            const issueBtn = page.locator('button:has-text("発行"), button:has-text("新規作成"), button:has-text("追加")').first();
            await expect(issueBtn).toBeVisible({ timeout: 10000 });

            await autoScreenshot(page, 'UC07', 'auth-290', _testStart);

            // [check] 290-5. 🔴 実 mTLS クライアント証明書でのログイン（ALB 経路偽装不可のため test-env-limitations.md 記録済）
        });
    });
