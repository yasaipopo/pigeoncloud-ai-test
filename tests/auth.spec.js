...
test.describe('認証（ログイン・ログアウト・パスワード変更）', () => {
    ...
    test('AT01: 認証基本フロー', async ({ page }) => {
        ...
        await test.step('1-2: ユーザータイプ「ユーザー」でログイン・ログアウトが完了すること', async () => {
            ...
            await expect(page.locator('.navbar')).toBeVisible({ timeout: 15000 });
            // s3: ✅ ダッシュボード遷移確認
            await stepScreenshot(page, 'auth', 'AT01', 'auth-030-s3', _testStart);
            ...
            await stepScreenshot(page, 'auth', 'AT01', 'auth-030-s4', _testStart);
        });
        ...
        await test.step('38-1: 誤ったパスワードでログインエラーが発生すること', async () => {
            ...
            await expect(page).toHaveURL(/\/admin\/login/);
            // s2: ✅ URLがログインページのまま
            await stepScreenshot(page, 'auth', 'AT01', 'auth-040-s2', _testStart);
            ...
            // s3: ✅ エラーメッセージ表示
            await stepScreenshot(page, 'auth', 'AT01', 'auth-040-s3', _testStart);
        ...
        await test.step('295: パスワード変更機能が想定通りに動作すること', async () => {
            ...
            // s4: ✅ パスワード変更フォーム表示
            await stepScreenshot(page, 'auth', 'AT01', 'auth-050-s4', _testStart);
            ...
            await stepScreenshot(page, 'auth', 'AT01', 'auth-050-s6', _testStart);
    ...
    test('AT02: ログインセッション管理', async ({ page }) => {
        ...
        await test.step('176: Googleログインボタンが表示され、クリックでGoogle認証画面に遷移すること', async () => {
            ...
            // s2: ✅ Googleログインボタン表示
            await stepScreenshot(page, 'auth', 'AT02', 'auth-060-s2', _testStart);
            ...
            await stepScreenshot(page, 'auth', 'AT02', 'auth-060-s4', _testStart);
        ...
        await test.step('212-1: 同時ログイン制御機能が設定人数まで動作すること', async () => {
            ...
            // s2: ✅ 1つ目ログイン成功
            await stepScreenshot(masterPage, 'auth', 'AT02', 'auth-070-s2', _testStart);
            // s3: ✅ 2つ目ログイン成功
            await stepScreenshot(userPage, 'auth', 'AT02', 'auth-070-s3', _testStart);
        ...
        await test.step('212-2: マスターユーザーがユーザーを強制ログアウトできること', async () => {
            ...
            await stepScreenshot(userPage, 'auth', 'AT02', 'auth-080-s4', _testStart);
        ...
        await test.step('212-3: 自動ログアウト時間設定が機能すること', async () => {
            ...
            await stepScreenshot(page, 'auth', 'AT02', 'auth-090-s2', _testStart);
        ...
        await test.step('212-4: 全端末からのログアウトが機能すること', async () => {
            ...
            await stepScreenshot(page, 'auth', 'AT02', 'auth-100-s4', _testStart);
        });
