...
test.describe('認証（ログイン・ログアウト・パスワード変更）', () => {
    ...
    test('UC01: 2段階認証設定', async ({ page }) => {
        ...
        await test.step('267: ログインIDがメールアドレス形式でない場合に2段階認証設定でエラーが表示されること', async () => {
            ...
            // 2段階認証の設定画面を開く
            // Angular SPAのフルリロードになるため、networkidleまで待つ
            await page.goto(BASE_URL + '/admin/setting/account', { waitUntil: 'domcontentloaded', timeout: 60000 }).catch(() => {});
            await waitForAngular(page);
            // Angularブートストラップ完了を待つ（navbarが出るまで）
            await page.waitForSelector('.navbar', { timeout: 30000 }).catch(async () => {
                // navbarが出ない場合はリロードして再試行
