...
test.describe('認証（ログイン・ログアウト・パスワード変更）', () => {
    ...
    test('212-4: 全端末からのログアウトが機能すること', async ({ page }) => {
        ...
        if (hasAllDeviceLogout) {
            ...
            // （Angular SPA の状態によってはモーダルなしで直接ログアウトされる場合あり）
            // ログインページへのリダイレクトを最大10秒待機
            await page.waitForURL(/\/admin\/login/, { timeout: 10000 }).catch(async () => {
                // リダイレクト待機が失敗した場合は現在のURLを確認
                const currentUrl = page.url();
                if (!currentUrl.includes('/admin/login')) {
                    throw new Error(`ログアウト後にログインページへリダイレクトされなかった。現在のURL: ${currentUrl}`);
                }
            });
            await expect(page).toHaveURL(/\/admin\/login/);
        }
    ...
    test('295: パスワード変更機能が想定通りに動作すること', async ({ page }) => {
        ...
        await page.waitForLoadState('domcontentloaded');
        // Angular SPAのレンダリング完了を待つ（「読み込み中...」が消えるまで）
        await page.waitForFunction(
            () => !document.body.innerText.includes('読み込み中'),
            { timeout: 30000 }
        ).catch(() => {});
        await waitForAngular(page);
        // ユーザーリストAPIのレスポンス完了を待つ（networkidle）
        ...
        if (!userCreated) {
            ...
            expect(userListText2, `ユーザー管理ページにテストユーザー(${testUserEmail295})が表示されること`).toContain(testUserEmail295);
        }
