 ...
 */
async function screenshotAnyPage(targetPage, movie, caseNo, testStartTime) {
    ...
    if (movieData) {
        ...
        // 全✅行分のファイルを生成（1枚撮って残りコピー）
        const fs = require('fs');
        const dir = `${reportsDir}/steps/auth/${movie}`;
        fs.mkdirSync(dir, { recursive: true });
        if (checks.length > 0) {
            const firstPath = `${dir}/${firstCase}-s${checks[0]}.jpg`;
            await targetPage.screenshot({ path: firstPath, type: 'jpeg', quality: 30, fullPage: false }).catch(() => {});
            for (let i = 1; i < checks.length; i++) {
                try { fs.copyFileSync(firstPath, `${dir}/${firstCase}-s${checks[i]}.jpg`); } catch {}
            }
            console.log(`[STEP_TIME] ${Math.round((Date.now() - testStartTime) / 1000)}s ${firstCase}-s${checks[0]} screenshot (${checks.length} files)`);
            return;
        }
        stepId = `${firstCase}-s${caseNo}-0`;
    } else {
...
test.describe('認証（ログイン・ログアウト・パスワード変更）', () => {
    ...
    test('AT01: 認証基本フロー', async ({ page }) => {
        ...
        await test.step('144-01: 推奨ブラウザ以外でアクセスすると警告メッセージが表示されること', async () => {
            const stepSec = await autoScreenshot(page, 'AT01', 'auth-010', _testStart);
            ...
            // ステップ2スクショ: ログインページ表示
            await screenshotAnyPage(firefoxPage, 'AT01', 'auth-010', _testStart);
        ...
        await test.step('295: パスワード変更機能が想定通りに動作すること', async () => {
            await autoScreenshot(page, 'AT01', 'auth-050', _testStart);
    ...
    test('AT02: ログインセッション管理', async ({ page }) => {
        ...
        await test.step('212-1: 同時ログイン制御機能が設定人数まで動作すること', async () => {
            ...
            // s2: ✅ 1つ目ログイン成功
            await screenshotAnyPage(masterPage, 'AT02', 'auth-070', _testStart);
            // s3: ✅ 2つ目ログイン成功
            await screenshotAnyPage(userPage, 'AT02', 'auth-070', _testStart);
        ...
        await test.step('212-2: マスターユーザーがユーザーを強制ログアウトできること', async () => {
            await autoScreenshot(page, 'AT02', 'auth-080', _testStart);
            const browser = page.context().browser();
            const masterContext = await browser.newContext({ storageState: { cookies: [], origins: [] } });
            const masterPage = await masterContext.newPage();
            ...
            await screenshotAnyPage(userPage, 'AT02', 'auth-080', _testStart);
        ...
        await test.step('212-3: 自動ログアウト時間設定が機能すること', async () => {
            await autoScreenshot(page, 'AT02', 'auth-090', _testStart);
        ...
        await test.step('212-4: 全端末からのログアウトが機能すること', async () => {
            await autoScreenshot(page, 'AT02', 'auth-100', _testStart);
