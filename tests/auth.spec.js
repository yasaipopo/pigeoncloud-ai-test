...
test.describe('認証（ログイン・ログアウト・パスワード管理）', () => {
    ...
    test.describe('UC07: SSO・セキュリティ設定', () => {
        ...
        test('auth-260: マルチテナント分離', async ({ browser, page }) => {
            ...
            // [check] 260-5. ✅ テナント B の API を直接叩くと 401/403 が返ること（セッション分離の最も確実な検証）
            ...
            // tenant mismatch 時は 401 (ApiRequest.php authorize) が期待値。400 "login_error" も session 無効化後の回帰パス
            const isRejected = [401, 403].includes(apiStatus) || (apiStatus === 400 && apiBody.includes('login_error'));
