...
test.describe('認証（ログイン・ログアウト・パスワード変更）', () => {
    ...
    test('AT01: 認証基本フロー', async ({ page }) => {
        ...
        await test.step('144-01: 推奨ブラウザ以外でアクセスすると警告メッセージが表示されること', async () => {
            console.log(`[STEP_TIME] ${Math.round((Date.now() - _testStart) / 1000)}s 144-01`);
            ...
            // 2. ログインページを開く
            await firefoxPage.goto(BASE_URL + '/admin/login', { waitUntil: 'domcontentloaded' });
            ...
            await firefoxPage.waitForSelector('.navbar', { timeout: 15000 }).catch(() => {});
            await waitForAngular(firefoxPage);

            const warningEl = firefoxPage.locator('div.warning');
            await expect(warningEl.first()).toBeVisible({ timeout: 15000 });

            // 5. 警告テキストに「推奨されているブラウザは Edge, chrome, safariの最新版」が含まれることを検証
            // full-layout.component.html L212-214: *ngIf="!recommended_browser"
        ...
        await test.step('1-1: マスターユーザーでログイン・ログアウトが完了すること', async () => {
            console.log(`[STEP_TIME] ${Math.round((Date.now() - _testStart) / 1000)}s 1-1`);
            await page.context().clearCookies();
            ...
            // 7. ユーザーアイコン→ログアウトメニューをクリック
            // full-layout.component.html: a.nav-link.nav-pill.avatar → #logout
        ...
        await test.step('1-2: ユーザータイプ「ユーザー」でログイン・ログアウトが完了すること', async () => {
            console.log(`[STEP_TIME] ${Math.round((Date.now() - _testStart) / 1000)}s 1-2`);
        ...
        await test.step('38-1: 誤ったパスワードでログインエラーが発生すること', async () => {
            console.log(`[STEP_TIME] ${Math.round((Date.now() - _testStart) / 1000)}s 38-1`);
            ...
            await expect(toastError.first()).toBeVisible({ timeout: 15000 });
        ...
        await test.step('295: パスワード変更機能が想定通りに動作すること', async () => {
            console.log(`[STEP_TIME] ${Math.round((Date.now() - _testStart) / 1000)}s 295`);
            ...
            await waitForAngular(page);
            await expect(page.locator('text=パスワードを変更してください')).toBeVisible({ timeout: 60000 });
    ...
    test('AT02: ログインセッション管理', async ({ page }) => {
        ...
        await test.step('176: Googleログインボタンが表示され、クリックでGoogle認証画面に遷移すること', async () => {
            console.log(`[STEP_TIME] ${Math.round((Date.now() - _testStart) / 1000)}s 176`);
            ...
            await page.waitForSelector('#id', { timeout: 30000 }).catch(() => {});
            await waitForAngular(page);
        ...
        await test.step('212-1: 同時ログイン制御機能が設定人数まで動作すること', async () => {
            console.log(`[STEP_TIME] ${Math.round((Date.now() - _testStart) / 1000)}s 212-1`);
            const browser = page.context().browser();
        ...
        await test.step('212-2: マスターユーザーがユーザーを強制ログアウトできること', async () => {
            console.log(`[STEP_TIME] ${Math.round((Date.now() - _testStart) / 1000)}s 212-2`);
            const browser = page.context().browser();
            const masterContext = await browser.newContext({ storageState: { cookies: [], origins: [] } });
            const masterPage = await masterContext.newPage();
        ...
        await test.step('212-3: 自動ログアウト時間設定が機能すること', async () => {
            console.log(`[STEP_TIME] ${Math.round((Date.now() - _testStart) / 1000)}s 212-3`);
        ...
        await test.step('212-4: 全端末からのログアウトが機能すること', async () => {
            console.log(`[STEP_TIME] ${Math.round((Date.now() - _testStart) / 1000)}s 212-4`);
    ...
    test('UC01: 2段階認証設定', async ({ page }) => {
        ...
        await test.step('267: ログインIDがメールアドレス形式でない場合に2段階認証設定でエラーが表示されること', async () => {
            console.log(`[STEP_TIME] ${Math.round((Date.now() - _testStart) / 1000)}s 267`);
            ...
            // Angularの描画完了を確実に待つ
            await page.waitForSelector('.navbar', { timeout: 60000 }).catch(async () => {
            ...
            const twoFactorVisible = await twoFactorLabel.isVisible({ timeout: 15000 }).catch(() => false);
