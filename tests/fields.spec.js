...
test.describe('項目設定（115, 116系）', () => {
    ...
    test('115-02: 自動採番フィールドがCSVアップロード後も正常に採番されること', async ({ page }) => {
        ...
        if (await autoNumField.count() > 0) {
            await expect(autoNumField).toBeVisible({ timeout: 30000 });
        }
...
test.describe('選択肢制限・フィールド追加（FD04）', () => {
    ...
    test('14-12-1: 年月フィールドを必須+デフォルト値付きで追加できること', async ({ page }) => {
        ...
        await expect(dateTypeBtn).toBeVisible({ timeout: 30000 });
    ...
    test('14-13: ファイルフィールドを必須+複数許可で追加できること', async ({ page }) => {
        ...
        await expect(fileTypeBtn).toBeVisible({ timeout: 30000 });
    ...
    test('14-14: 計算フィールドをデフォルト設定で追加できること', async ({ page }) => {
        ...
        await expect(calcTypeBtn).toBeVisible({ timeout: 30000 });
    ...
    test('14-17: 文章(複数行)フィールドを通常テキストで追加できること', async ({ page }) => {
        ...
        await expect(textareaTypeBtn).toBeVisible({ timeout: 30000 });
    ...
    test('14-19: YES/NOフィールドをデフォルト設定で追加できること', async ({ page }) => {
        ...
        await expect(yesnoTypeBtn).toBeVisible({ timeout: 30000 });
...
test.describe('項目名パディング追加（92/93/94系）', () => {
    ...
    async function testFieldNamePadding(page, fieldTypeLabel, paddingChar, fieldName) {
        ...
        await expect(typeBtn).toBeVisible({ timeout: 30000 });
...
test.describe('日時種類変更（19系）', () => {
    ...
    test('19-1: 日時フィールドの種類を変更できること', async ({ page }) => {
        ...
        await expect(editPanel.first()).toBeVisible({ timeout: 30000 });
...
test.describe('日時フォーマット（97系）', () => {
    ...
    async function testDateFormat(page, formatStr) {
        ...
        await expect(optionBtn).toBeVisible({ timeout: 30000 });
        ...
        await expect(dateFieldAfterSave).toBeVisible({ timeout: 30000 });
    ...
    test('97-1: 日時フィールドにdate("Y/m/d H:i:s")フォーマットを設定できること', async ({ page }) => {
        ...
        await expect(page.locator('.field-drag, .cdk-drag').filter({ hasText: '日時' }).first()).toBeVisible({ timeout: 30000 });
    });
...
test.describe('表示条件設定（223-229系）', () => {
    ...
    async function testDisplayCondition(page, fieldTypeLabel) {
        ...
        await expect(conditionRow).toBeVisible({ timeout: 30000 });
...
test.describe('必須条件設定（231-241系）', () => {
    ...
    async function testRequiredCondition(page, fieldTypeLabel) {
        ...
        await expect(requiredCheck).toBeVisible({ timeout: 30000 });
        ...
        await expect(condArea).toBeVisible({ timeout: 30000 });
...
test.describe('項目権限設定（149系）', () => {
    ...
    /**
     * テーブルの権限設定ページにアクセスし、権限設定UIが表示されることを確認
     * @param {import('@playwright/test').Page} page
     * @returns {import('@playwright/test').Locator} 権限設定タブのコンテンツ領域
     */
    async function navigateToFieldPermission(page) {
        ...
        await expect(permTab).toBeVisible({ timeout: 30000 });
    ...
    /**
     * 権限設定画面で閲覧/編集トグルを操作し、期待するON/OFF状態を確認する共通関数
     * @param {import('@playwright/test').Page} page
     * @param {object} opts
     * @param {boolean} opts.viewOn - 閲覧をONにするか
     * @param {boolean} opts.editOn - 編集をONにするか
     * @param {string} opts.label - テストで確認するラベル（例: '閲覧', '編集'等）
     */
    async function testPermissionToggle(page, { viewOn, editOn }) {
        await navigateToFieldPermission(page);

        // 権限設定セクションが表示されていることを確認
        const permSection = page.locator('.permission-section, .permission-settings, .grant-modal-container').first();
        await expect(permSection).toBeVisible({ timeout: 30000 });

        // 閲覧トグルを探す（「閲覧」ラベルに近いswitch-input）
        const viewRow = page.locator('.permission-row').filter({ hasText: /^閲覧/ }).first();
        const viewToggle = viewRow.locator('input.switch-input').first();

        // 閲覧トグルが存在しない場合はフォールバック（権限設定UIの構造が異なる場合）
        if (await viewToggle.count() === 0) {
            // 「閲覧」チェックボックスを直接探す
            const viewCheck = page.locator('label:has-text("閲覧") input[type="checkbox"], input[name*="view"]').first();
            if (await viewCheck.count() > 0) {
                const isChecked = await viewCheck.isChecked();
                if (viewOn && !isChecked) {
                    await viewCheck.click({ force: true });
                    await waitForAngular(page);
                } else if (!viewOn && isChecked) {
                    await viewCheck.click({ force: true });
                    await waitForAngular(page);
                }
            }
            ...
            // トグルスイッチ操作: 閲覧
            const viewChecked = await viewToggle.isChecked();
            if (viewOn && !viewChecked) {
                await viewToggle.click({ force: true });
                await waitForAngular(page);
            } else if (!viewOn && viewChecked) {
                await viewToggle.click({ force: true });
                await waitForAngular(page);
            }
            // 閲覧トグルの状態を確認
            if (viewOn) {
                await expect(viewToggle).toBeChecked();
            } else {
                await expect(viewToggle).not.toBeChecked();
            }
        }

        // 編集トグルを探す
        const editRow = page.locator('.permission-row').filter({ hasText: /^編集/ }).first();
        const editToggle = editRow.locator('input.switch-input').first();

        if (await editToggle.count() === 0) {
            const editCheck = page.locator('label:has-text("編集") input[type="checkbox"], input[name*="edit"]').first();
            if (await editCheck.count() > 0 && viewOn) {
                // 閲覧OFFの場合は編集が無効化されるため操作しない
                const isChecked = await editCheck.isChecked();
                if (editOn && !isChecked) {
                    await editCheck.click({ force: true });
                    await waitForAngular(page);
                } else if (!editOn && isChecked) {
                    await editCheck.click({ force: true });
                    await waitForAngular(page);
                }
            }
            ...
            if (viewOn) {
                // 閲覧ONの場合のみ編集トグルが操作可能
                const editChecked = await editToggle.isChecked();
                if (editOn && !editChecked) {
                    await editToggle.click({ force: true });
                    await waitForAngular(page);
                } else if (!editOn && editChecked) {
                    await editToggle.click({ force: true });
                    await waitForAngular(page);
                }
                if (editOn) {
    ...
    test('149-1: 項目権限設定 - 組織のみ・閲覧ON/編集OFFの設定画面が表示されること', async ({ page }) => {
        await testPermissionToggle(page, { viewOn: true, editOn: false });
        // 権限設定画面で閲覧ONの状態で「条件設定」ボタンが表示されることを確認
        const condBtn = page.locator('button:has-text("条件設定")').first();
        if (await condBtn.count() > 0) {
            await expect(condBtn).toBeVisible();
        }
    });

    test('149-2: 項目権限設定 - 組織ユーザーのみ・閲覧ON/編集OFFの設定画面が表示されること', async ({ page }) => {
        await testPermissionToggle(page, { viewOn: true, editOn: false });
        // 閲覧ONでも編集トグルが非チェック状態であることを追加確認
        const editRow = page.locator('.permission-row').filter({ hasText: /^編集/ }).first();
        const editToggle = editRow.locator('input.switch-input').first();
        if (await editToggle.count() > 0) {
            await expect(editToggle).not.toBeChecked();
    ...
    test('149-5: 項目権限設定 - 組織のみ・閲覧OFF/編集OFFの設定画面が表示されること', async ({ page }) => {
        await testPermissionToggle(page, { viewOn: false, editOn: false });
    });
    ...
    test('149-9: 項目権限設定 - 組織のみ・閲覧ON/編集ONの設定画面が表示されること', async ({ page }) => {
        await testPermissionToggle(page, { viewOn: true, editOn: true });
        // 閲覧・編集ともONなので、両方の「条件設定」ボタンが表示されうる
        const condBtns = page.locator('button:has-text("条件設定")');
        if (await condBtns.count() >= 2) {
            await expect(condBtns.first()).toBeVisible();
            await expect(condBtns.nth(1)).toBeVisible();
        }
    });
    ...
    test('149-16: 項目権限設定 - 全ユーザー・編集権限なしテーブルで閲覧ON/編集ONの設定画面が表示されること', async ({ page }) => {
        await testPermissionToggle(page, { viewOn: true, editOn: true });
        // 編集権限なしテーブルの場合でも、権限設定画面で編集トグルが操作可能であることを確認
        const editRow = page.locator('.permission-row').filter({ hasText: /^編集/ }).first();
        const editToggle = editRow.locator('input.switch-input').first();
        if (await editToggle.count() > 0) {
    ...
    test('149-17: 項目権限設定 - 作成者のみの権限設定画面が表示されること', async ({ page }) => {
        await navigateToFieldPermission(page);
        // 権限設定セクションが表示されていることを確認
        const permSection = page.locator('.permission-section, .permission-settings').first();
        await expect(permSection).toBeVisible({ timeout: 30000 });
        // 「作成者のみ」に関連するUIが存在することを確認（「作成者」テキストまたはラジオボタン）
        const bodyText = await page.innerText('body');
        expect(bodyText).not.toContain('Internal Server Error');
        // 閲覧/編集トグルが存在すること
        const viewRow = page.locator('.permission-row').filter({ hasText: /^閲覧/ }).first();
        await expect(viewRow).toBeVisible({ timeout: 30000 });
    });
...
test.describe('バグ修正確認・機能改善（フィールド関連）', () => {
    ...
    test('247: 選択肢(単一選択)で「0」を選択した場合にブランクにならず値が表示されること', async ({ page }) => {
        ...
        if (await selectField.count() > 0) {
            ...
            if (await optionArea.count() > 0) {
                await expect(optionArea).toBeVisible({ timeout: 30000 });
            }
        }
    ...
    test('171: 他テーブル参照フィールドで選択肢の新規追加表示/非表示を設定できること', async ({ page }) => {
        ...
        if (await refField.count() > 0) {
            ...
            if (await newAddOption.count() > 0) {
                await expect(newAddOption).toBeVisible({ timeout: 30000 });
            }
        }
    ...
    test('174: 計算フィールドの編集中にリアルタイム表示が可能であること', async ({ page }) => {
        ...
        if (await calcField.count() > 0) {
            ...
            const formulaInput = page.locator('input[name*="formula"], textarea[name*="formula"], [class*="formula"] input, [class*="formula"] textarea').first();
            if (await formulaInput.count() > 0) {
                await expect(formulaInput).toBeVisible({ timeout: 30000 });
            }
    ...
    test('195: テーブルの項目並べ替えが保存後も反映されていること', async ({ page }) => {
        ...
        if (await fieldList.count() > 0) {
            await expect(fieldList).toBeVisible({ timeout: 30000 });
    ...
    test('204: 他テーブル参照フィールドで複数項目のルックアップ設定が可能であること', async ({ page }) => {
        ...
        if (await refField.count() > 0) {
            ...
            if (await lookupArea.count() > 0) {
                await expect(lookupArea).toBeVisible({ timeout: 30000 });
            }
        }
        ...
        if (await field.count() > 0) {
            ...
            if (checkDisplayCondition) {
                ...
                if (await displayCondition.count() > 0) {
                    await expect(displayCondition).toBeVisible({ timeout: 30000 });
                }
            }
        }
    ...
    test('265-1: テキストフィールドを必須に設定し空欄保存でエラーになること', async ({ page }) => {
        ...
        if (await textField.count() > 0) {
            ...
            if (await requiredLabel.count() > 0) {
                await expect(requiredLabel).toBeVisible({ timeout: 30000 });
            }
        }
    ...
    test('265-2: フィールドの重複チェック設定UIが存在すること', async ({ page }) => {
        ...
        if (await textField.count() > 0) {
            ...
            if (await uniqueLabel.count() > 0) {
                await expect(uniqueLabel).toBeVisible({ timeout: 30000 });
            }
        }
    ...
    test('267-1: テキストフィールドに初期値を設定しレコード新規作成で自動入力されること', async ({ page }) => {
        ...
        if (await textField.count() > 0) {
            ...
            if (await defaultLabel.count() > 0) {
                await expect(defaultLabel).toBeVisible({ timeout: 30000 });
            }
        }
    ...
    test('14-25: 他テーブル参照フィールドをルックアップ+必須+検索必須+複数値で追加できること', async ({ page }) => {
        ...
        if (await refField.count() > 0) {
            ...
            if (await lookupLabel.count() > 0) {
                await expect(lookupLabel).toBeVisible({ timeout: 30000 });
            }
...
test.describe('計算フィールド拡張テスト', () => {
    ...
    test('380: 計算項目の「計算値の自動更新」OFFが正しく動作すること', async ({ page }) => {
        ...
        if (await autoUpdateLabel.count() > 0) {
            await expect(autoUpdateLabel).toBeVisible({ timeout: 30000 });
        }
...
test.describe('他テーブル参照フィールド拡張テスト', () => {
    ...
    test('183: 権限がない場合、他テーブル参照項目に「新規追加」が非表示であること', async ({ page }) => {
        ...
        if (await newAddLabel.count() > 0) {
            await expect(newAddLabel).toBeVisible({ timeout: 30000 });
        }
    ...
    test('189: 他テーブル参照の一覧テーブルからの検索ボタンが機能すること', async ({ page }) => {
        ...
        if (await optionBtn.count() > 0) {
            ...
            if (await searchBtnLabel.count() > 0) {
                await expect(searchBtnLabel).toBeVisible({ timeout: 30000 });
            }
        }
    ...
    test('746: 他テーブル参照の表示項目に対象テーブルの子テーブルが含まれないこと', async ({ page }) => {
        ...
        if (await displayFieldLabel.count() > 0) {
            await expect(displayFieldLabel).toBeVisible({ timeout: 30000 });
        }
    ...
    test('440: 日時項目のフォーマットにY/y/M/n/d/H/h/i等が使用可能であること', async ({ page }) => {
        ...
        if (await dateField.count() > 0) {
            ...
            if (await formatLabel.count() > 0) {
                await expect(formatLabel).toBeVisible({ timeout: 30000 });
            }
        }
    ...
    test('786: 数値項目で桁区切り表示が入力中も表示されること', async ({ page }) => {
        ...
        if (await numField.count() > 0) {
            ...
            if (await digitLabel.count() > 0) {
                await expect(digitLabel).toBeVisible({ timeout: 30000 });
            }
        }
