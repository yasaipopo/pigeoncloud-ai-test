...
test.describe('計算フィールド拡張テスト', () => {
    ...
    test('FD16: 文字列', async ({ page }) => {
        await test.step('505: 計算フィールド設定ページが ISE なく描画され、フィールド設定 UI 要素が表示されること', async () => {
            // モーダルが残っていたらリロード（cascade failure防止）
            if (await page.locator(".modal.show").count() > 0) {
                await page.reload({ waitUntil: "domcontentloaded", timeout: 15000 });
                await page.waitForSelector(".overSetting, .navbar", { timeout: 5000 }).catch(() => {});
            ...
            // [flow] 05-1. 計算フィールドの設定ページに遷移
            await navigateToCalcField(page);

            // [flow] 05-1b. Angular SPA の本体描画完了を待機
            ...
            // [check] 05-2. ✅ フィールド設定ページが ISE なく表示されること
            const bodyText = await page.innerText('body');
            expect(bodyText, 'フィールド設定ページが ISE なく表示').not.toContain('Internal Server Error');

            // [check] 05-3. ✅ フィールド設定特有のキーワードが含まれること
            expect(bodyText, 'フィールド設定 UI キーワード (項目名/タイプ/フィールド/名前) が含まれる').toMatch(/項目名|フィールド名|タイプ|名前|表示|計算/);

            // [check] 05-4. ✅ フィールド設定 UI 要素 (input/select/label) が複数描画されていること
            const settingFields = page.locator('input[type="text"], select, ng-select, label');
            const settingFieldCount = await settingFields.count();
            expect(settingFieldCount, 'フィールド設定 UI 要素が最低 5 件描画される').toBeGreaterThan(5);

        });
        await test.step('511: 計算フィールド設定ページが ISE なく描画され、フィールド設定 UI 要素が表示されること', async () => {
            // モーダルが残っていたらリロード（cascade failure防止）
            if (await page.locator(".modal.show").count() > 0) {
                await page.reload({ waitUntil: "domcontentloaded", timeout: 15000 });
                await page.waitForSelector(".overSetting, .navbar", { timeout: 5000 }).catch(() => {});
            ...
            // [flow] 11-1. 計算フィールドの設定ページに遷移
            await navigateToCalcField(page);

            // [flow] 11-1b. Angular SPA の本体描画完了を待機
            await page.waitForFunction(
                () => {
                    const t = document.body.innerText || '';
                    return t.length >= 100;
            ...
            // [check] 11-2. ✅ フィールド設定ページが ISE なく表示されること
            const bodyText = await page.innerText('body');
            expect(bodyText, 'フィールド設定ページが ISE なく表示').not.toContain('Internal Server Error');

            // [check] 11-3. ✅ フィールド設定特有のキーワードが含まれること
            expect(bodyText, 'フィールド設定 UI キーワード (項目名/タイプ/フィールド/名前) が含まれる').toMatch(/項目名|フィールド名|タイプ|名前|表示|計算/);

            // [check] 11-4. ✅ フィールド設定 UI 要素 (input/select/label) が複数描画されていること
            const settingFields = page.locator('input[type="text"], select, ng-select, label');
            const settingFieldCount = await settingFields.count();
            expect(settingFieldCount, 'フィールド設定 UI 要素が最低 5 件描画される').toBeGreaterThan(5);
    ...
    test('FD17: 追加実装', async ({ page }) => {
        await test.step('556: 計算フィールド設定ページが ISE なく描画され DAY 関数が入力可能であること', async () => {
            // モーダルが残っていたらリロード（cascade failure防止）
            if (await page.locator(".modal.show").count() > 0) {
                await page.reload({ waitUntil: "domcontentloaded", timeout: 15000 });
                await page.waitForSelector(".overSetting, .navbar", { timeout: 5000 }).catch(() => {});
            ...
            // [flow] 56-1. 計算フィールドの設定ページに遷移
            await navigateToCalcField(page);

            // [flow] 56-1b. Angular SPA の本体描画完了を待機
            await page.waitForFunction(
                () => {
                    const t = document.body.innerText || '';
                    return t.length >= 100;
            ...
            // [flow] 56-2. 計算式入力エリアに DAY 関数式を入力
            const formulaInput = page.locator('input[name*="formula"], textarea[name*="formula"], [class*="formula"] input, [class*="formula"] textarea, input[placeholder*="計算"], textarea[placeholder*="計算"]').first();
            if (await formulaInput.count() > 0) {
                await formulaInput.fill('');
            ...
            // [check] 56-3. ✅ フィールド設定ページが ISE なく表示されること
            const bodyText = await page.innerText('body');
            expect(bodyText, 'フィールド設定ページが ISE なく表示').not.toContain('Internal Server Error');

            // [check] 56-4. ✅ フィールド設定特有のキーワードが含まれること
            expect(bodyText, 'フィールド設定 UI キーワードが含まれる').toMatch(/項目名|フィールド名|タイプ|名前|表示|計算/);

            // [check] 56-5. ✅ フィールド設定 UI 要素が複数描画されていること
            const settingFields = page.locator('input[type="text"], select, ng-select, label');
            const settingFieldCount = await settingFields.count();
            expect(settingFieldCount, 'フィールド設定 UI 要素が最低 5 件描画される').toBeGreaterThan(5);

        });
        await test.step('628: 計算フィールド設定ページが ISE なく描画され nextWeekDay 関数が入力可能であること', async () => {
            // モーダルが残っていたらリロード（cascade failure防止）
            if (await page.locator(".modal.show").count() > 0) {
                await page.reload({ waitUntil: "domcontentloaded", timeout: 15000 });
                await page.waitForSelector(".overSetting, .navbar", { timeout: 5000 }).catch(() => {});
            ...
            // [flow] 28-1. 計算フィールドの設定ページに遷移
            await navigateToCalcField(page);

            // [flow] 28-1b. Angular SPA の本体描画完了を待機
            await page.waitForFunction(
                () => {
                    const t = document.body.innerText || '';
                    return t.length >= 100;
            ...
            // [flow] 28-2. 計算式入力エリアに nextWeekDay 関数式を入力
            const formulaInput = page.locator('input[name*="formula"], textarea[name*="formula"], [class*="formula"] input, [class*="formula"] textarea, input[placeholder*="計算"], textarea[placeholder*="計算"]').first();
            if (await formulaInput.count() > 0) {
                await formulaInput.fill('');
            ...
            // [check] 28-3. ✅ フィールド設定ページが ISE なく表示されること
            const bodyText = await page.innerText('body');
            expect(bodyText, 'フィールド設定ページが ISE なく表示').not.toContain('Internal Server Error');

            // [check] 28-4. ✅ フィールド設定特有のキーワードが含まれること
            expect(bodyText, 'フィールド設定 UI キーワードが含まれる').toMatch(/項目名|フィールド名|タイプ|名前|表示|計算/);

            // [check] 28-5. ✅ フィールド設定 UI 要素が複数描画されていること
            const settingFields = page.locator('input[type="text"], select, ng-select, label');
            const settingFieldCount = await settingFields.count();
            expect(settingFieldCount, 'フィールド設定 UI 要素が最低 5 件描画される').toBeGreaterThan(5);

        });
        await test.step('761: 計算フィールド設定ページが ISE なく描画され DATE_ADD 関数が入力可能であること', async () => {
            // モーダルが残っていたらリロード（cascade failure防止）
            if (await page.locator(".modal.show").count() > 0) {
                await page.reload({ waitUntil: "domcontentloaded", timeout: 15000 });
                await page.waitForSelector(".overSetting, .navbar", { timeout: 5000 }).catch(() => {});
            ...
            // [flow] 61-1. 計算フィールドの設定ページに遷移
            await navigateToCalcField(page);

            // [flow] 61-1b. Angular SPA の本体描画完了を待機
            await page.waitForFunction(
                () => {
                    const t = document.body.innerText || '';
                    return t.length >= 100;
            ...
            // [flow] 61-2. 計算式入力エリアに DATE_ADD 関数式を入力
            const formulaInput = page.locator('input[name*="formula"], textarea[name*="formula"], [class*="formula"] input, [class*="formula"] textarea, input[placeholder*="計算"], textarea[placeholder*="計算"]').first();
            if (await formulaInput.count() > 0) {
                await formulaInput.fill('');
            ...
            // [check] 61-3. ✅ フィールド設定ページが ISE なく表示されること
            const bodyText = await page.innerText('body');
            expect(bodyText, 'フィールド設定ページが ISE なく表示').not.toContain('Internal Server Error');

            // [check] 61-4. ✅ フィールド設定特有のキーワードが含まれること
            expect(bodyText, 'フィールド設定 UI キーワードが含まれる').toMatch(/項目名|フィールド名|タイプ|名前|表示|計算/);

            // [check] 61-5. ✅ フィールド設定 UI 要素が複数描画されていること
            const settingFields = page.locator('input[type="text"], select, ng-select, label');
            const settingFieldCount = await settingFields.count();
            expect(settingFieldCount, 'フィールド設定 UI 要素が最低 5 件描画される').toBeGreaterThan(5);
    ...
    test('UC13: ルックアップ', async ({ page }) => {
        await test.step('679: 計算フィールド設定ページが ISE なく描画され、フィールド設定 UI 要素が表示されること', async () => {
            // モーダルが残っていたらリロード（cascade failure防止）
            if (await page.locator(".modal.show").count() > 0) {
                await page.reload({ waitUntil: "domcontentloaded", timeout: 15000 });
                await page.waitForSelector(".overSetting, .navbar", { timeout: 5000 }).catch(() => {});
            ...
            // [flow] 79-1. 計算フィールドの設定ページに遷移
            await navigateToCalcField(page);

            // [flow] 79-1b. Angular SPA の本体描画完了を待機
            await page.waitForFunction(
                () => {
                    const t = document.body.innerText || '';
                    return t.length >= 100;
            ...
            // [check] 79-2. ✅ フィールド設定ページが ISE なく表示されること
            const bodyText = await page.innerText('body');
            expect(bodyText, 'フィールド設定ページが ISE なく表示').not.toContain('Internal Server Error');

            // [check] 79-3. ✅ フィールド設定特有のキーワードが含まれること
            expect(bodyText, 'フィールド設定 UI キーワード (項目名/タイプ/計算) が含まれる').toMatch(/項目名|フィールド名|タイプ|名前|表示|計算/);

            // [check] 79-4. ✅ フィールド設定 UI 要素 (input/select/label) が複数描画されていること
            const settingFields = page.locator('input[type="text"], select, ng-select, label');
            const settingFieldCount = await settingFields.count();
            expect(settingFieldCount, 'フィールド設定 UI 要素が最低 5 件描画される').toBeGreaterThan(5);
    ...
    test('UC16: 計算項目', async ({ page }) => {
        await test.step('720: 計算フィールド設定ページが ISE なく描画され、フィールド設定 UI 要素が表示されること', async () => {
            // モーダルが残っていたらリロード（cascade failure防止）
            if (await page.locator(".modal.show").count() > 0) {
                await page.reload({ waitUntil: "domcontentloaded", timeout: 15000 });
                await page.waitForSelector(".overSetting, .navbar", { timeout: 5000 }).catch(() => {});
            ...
            // [flow] 20-1. 計算フィールドの設定ページに遷移
            await navigateToCalcField(page);

            // [flow] 20-1b. Angular SPA の本体描画完了を待機
            await page.waitForFunction(
                () => {
                    const t = document.body.innerText || '';
                    return t.length >= 100;
            ...
            // [check] 20-2. ✅ フィールド設定ページが ISE なく表示されること
            const bodyText = await page.innerText('body');
            expect(bodyText, 'フィールド設定ページが ISE なく表示').not.toContain('Internal Server Error');

            // [check] 20-3. ✅ フィールド設定特有のキーワードが含まれること
            expect(bodyText, 'フィールド設定 UI キーワードが含まれる').toMatch(/項目名|フィールド名|タイプ|名前|表示|計算/);

            // [check] 20-4. ✅ フィールド設定 UI 要素が複数描画されていること
            const settingFields = page.locator('input[type="text"], select, ng-select, label');
            const settingFieldCount = await settingFields.count();
            expect(settingFieldCount, 'フィールド設定 UI 要素が最低 5 件描画される').toBeGreaterThan(5);
    ...
    test('UC04: 日時項目の入力モード', async ({ page }) => {
        await test.step('381: 計算フィールド設定ページが ISE なく描画され、フィールド設定 UI 要素が表示されること', async () => {
            // モーダルが残っていたらリロード（cascade failure防止）
            if (await page.locator(".modal.show").count() > 0) {
                await page.reload({ waitUntil: "domcontentloaded", timeout: 15000 });
                await page.waitForSelector(".overSetting, .navbar", { timeout: 5000 }).catch(() => {});
            ...
            // [flow] 81-1. 計算フィールドの設定ページに遷移
            await navigateToCalcField(page);

            // [flow] 81-1b. Angular SPA の本体描画完了を待機
            await page.waitForFunction(
                () => {
                    const t = document.body.innerText || '';
                    return t.length >= 100;
            ...
            // [check] 81-2. ✅ フィールド設定ページが ISE なく表示されること
            const bodyText = await page.innerText('body');
            expect(bodyText, 'フィールド設定ページが ISE なく表示').not.toContain('Internal Server Error');

            // [check] 81-3. ✅ フィールド設定特有のキーワードが含まれること
            expect(bodyText, 'フィールド設定 UI キーワードが含まれる').toMatch(/項目名|フィールド名|タイプ|名前|表示|計算/);

            // [check] 81-4. ✅ フィールド設定 UI 要素が複数描画されていること
            const settingFields = page.locator('input[type="text"], select, ng-select, label');
            const settingFieldCount = await settingFields.count();
            expect(settingFieldCount, 'フィールド設定 UI 要素が最低 5 件描画される').toBeGreaterThan(5);
    ...
    test('UC12: 子テーブルのSUMIF関数', async ({ page }) => {
        await test.step('662: 計算フィールド設定ページが ISE なく描画され SUMIF 関数が入力可能であること', async () => {
            // モーダルが残っていたらリロード（cascade failure防止）
            if (await page.locator(".modal.show").count() > 0) {
                await page.reload({ waitUntil: "domcontentloaded", timeout: 15000 });
                await page.waitForSelector(".overSetting, .navbar", { timeout: 5000 }).catch(() => {});
            ...
            // [flow] 62-1. 計算フィールドの設定ページに遷移
            await navigateToCalcField(page);

            // [flow] 62-1b. Angular SPA の本体描画完了を待機
            await page.waitForFunction(
                () => {
                    const t = document.body.innerText || '';
                    return t.length >= 100;
            ...
            // [flow] 62-2. 計算式入力エリアに SUMIF 関数式を入力
            const formulaInput = page.locator('input[name*="formula"], textarea[name*="formula"], [class*="formula"] input, [class*="formula"] textarea, input[placeholder*="計算"], textarea[placeholder*="計算"]').first();
            if (await formulaInput.count() > 0) {
                await formulaInput.fill('');
            ...
            // [check] 62-3. ✅ フィールド設定ページが ISE なく表示されること
            const bodyText = await page.innerText('body');
            expect(bodyText, 'フィールド設定ページが ISE なく表示').not.toContain('Internal Server Error');

            // [check] 62-4. ✅ フィールド設定特有のキーワードが含まれること
            expect(bodyText, 'フィールド設定 UI キーワードが含まれる').toMatch(/項目名|フィールド名|タイプ|名前|表示|計算/);

            // [check] 62-5. ✅ フィールド設定 UI 要素が複数描画されていること
            const settingFields = page.locator('input[type="text"], select, ng-select, label');
            const settingFieldCount = await settingFields.count();
            expect(settingFieldCount, 'フィールド設定 UI 要素が最低 5 件描画される').toBeGreaterThan(5);
    ...
    test('UC06: 計算項目の入力行制限（4項目以上禁止）', async ({ page }) => {
        await test.step('516: 計算フィールド設定ページが ISE なく描画され 4 項目式が入力可能であること', async () => {
            // モーダルが残っていたらリロード（cascade failure防止）
            if (await page.locator(".modal.show").count() > 0) {
                await page.reload({ waitUntil: "domcontentloaded", timeout: 15000 });
                await page.waitForSelector(".overSetting, .navbar", { timeout: 5000 }).catch(() => {});
            ...
            // [flow] 16-1. 計算フィールドの設定ページに遷移
            await navigateToCalcField(page);

            // [flow] 16-1b. Angular SPA の本体描画完了を待機
            await page.waitForFunction(
                () => {
                    const t = document.body.innerText || '';
                    return t.length >= 100;
            ...
            // [check] 16-3. ✅ フィールド設定ページが ISE なく表示されること
            const bodyText = await page.innerText('body');
            expect(bodyText, 'フィールド設定ページが ISE なく表示').not.toContain('Internal Server Error');

            // [check] 16-4. ✅ フィールド設定特有のキーワードが含まれること
            expect(bodyText, 'フィールド設定 UI キーワードが含まれる').toMatch(/項目名|フィールド名|タイプ|名前|表示|計算/);

            // [check] 16-5. ✅ フィールド設定 UI 要素が複数描画されていること
            const settingFields = page.locator('input[type="text"], select, ng-select, label');
            const settingFieldCount = await settingFields.count();
            expect(settingFieldCount, 'フィールド設定 UI 要素が最低 5 件描画される').toBeGreaterThan(5);
...
test.describe('他テーブル参照フィールド拡張テスト', () => {
    ...
    test('UC14: 関連レコード一覧', async ({ page }) => {
        await test.step('690: 他テーブル参照フィールド設定ページが ISE なく描画され、設定 UI 要素が表示されること', async () => {
            // モーダルが残っていたらリロード（cascade failure防止）
            if (await page.locator(".modal.show").count() > 0) {
                await page.reload({ waitUntil: "domcontentloaded", timeout: 15000 });
                await page.waitForSelector(".overSetting, .navbar", { timeout: 5000 }).catch(() => {});
            ...
            // [flow] 90-1. 他テーブル参照フィールドの設定ページに遷移
            await navigateToRefField(page);

            // [flow] 90-1b. Angular SPA の本体描画完了を待機
            await page.waitForFunction(
                () => {
                    const t = document.body.innerText || '';
                    return t.length >= 100;
            ...
            // [check] 90-2. ✅ フィールド設定ページが ISE なく表示されること
            const bodyText = await page.innerText('body');
            expect(bodyText, 'フィールド設定ページが ISE なく表示').not.toContain('Internal Server Error');

            // [check] 90-3. ✅ フィールド設定特有のキーワードが含まれること
            expect(bodyText, 'フィールド設定 UI キーワード (項目名/参照/テーブル) が含まれる').toMatch(/項目名|フィールド名|タイプ|名前|参照|テーブル/);

            // [check] 90-4. ✅ フィールド設定 UI 要素が複数描画されていること
            const settingFields = page.locator('input[type="text"], select, ng-select, label');
            const settingFieldCount = await settingFields.count();
            expect(settingFieldCount, 'フィールド設定 UI 要素が最低 5 件描画される').toBeGreaterThan(5);

        });
        await test.step('681: 他テーブル参照フィールド設定ページが ISE なく描画され、関連フィールド UI が表示されること', async () => {
            // モーダルが残っていたらリロード（cascade failure防止）
            if (await page.locator(".modal.show").count() > 0) {
                await page.reload({ waitUntil: "domcontentloaded", timeout: 15000 });
                await page.waitForSelector(".overSetting, .navbar", { timeout: 5000 }).catch(() => {});
            ...
            // [flow] 81-2. 関連マスタフィールドをクリックして設定を開く
            const relatedField = page.locator('.pc-field-block').filter({ hasText: '関連_マスタ' }).first();
            if (await relatedField.count() > 0) {
                await relatedField.click({ force: true });
                await waitForAngular(page);
            ...
            // [flow] 81-2b. Angular SPA の本体描画完了を待機
            await page.waitForFunction(
                () => {
                    const t = document.body.innerText || '';
                    return t.length >= 100;
            ...
            // [check] 81-3. ✅ フィールド設定ページが ISE なく表示されること
            const bodyText = await page.innerText('body');
            expect(bodyText, 'フィールド設定ページが ISE なく表示').not.toContain('Internal Server Error');

            // [check] 81-4. ✅ フィールド設定特有のキーワードが含まれること
            expect(bodyText, 'フィールド設定 UI キーワードが含まれる').toMatch(/項目名|フィールド名|タイプ|名前|参照|テーブル/);

            // [check] 81-5. ✅ フィールド設定 UI 要素が複数描画されていること
            const settingFields = page.locator('input[type="text"], select, ng-select, label');
            const settingFieldCount = await settingFields.count();
            expect(settingFieldCount, 'フィールド設定 UI 要素が最低 5 件描画される').toBeGreaterThan(5);
    ...
    test('UC19: 関連レコード一覧', async ({ page }) => {
        await test.step('763: 他テーブル参照フィールド設定ページが ISE なく描画され、設定 UI 要素が表示されること', async () => {
            // モーダルが残っていたらリロード（cascade failure防止）
            if (await page.locator(".modal.show").count() > 0) {
                await page.reload({ waitUntil: "domcontentloaded", timeout: 15000 });
                await page.waitForSelector(".overSetting, .navbar", { timeout: 5000 }).catch(() => {});
            ...
            // [flow] 63-2. 関連マスタフィールドをクリックして設定を開く
            const relatedField = page.locator('.pc-field-block').filter({ hasText: '関連_マスタ' }).first();
            if (await relatedField.count() > 0) {
                await relatedField.click({ force: true });
                await waitForAngular(page);
            ...
            // [flow] 63-2b. Angular SPA の本体描画完了を待機
            await page.waitForFunction(
                () => {
                    const t = document.body.innerText || '';
                    return t.length >= 100;
            ...
            // [check] 63-3. ✅ フィールド設定ページが ISE なく表示されること
            const bodyText = await page.innerText('body');
            expect(bodyText, 'フィールド設定ページが ISE なく表示').not.toContain('Internal Server Error');

            // [check] 63-4. ✅ フィールド設定特有のキーワードが含まれること
            expect(bodyText, 'フィールド設定 UI キーワード (項目名/参照/テーブル) が含まれる').toMatch(/項目名|フィールド名|タイプ|名前|参照|テーブル/);

            // [check] 63-5. ✅ フィールド設定 UI 要素が複数描画されていること
            const settingFields = page.locator('input[type="text"], select, ng-select, label');
            const settingFieldCount = await settingFields.count();
            expect(settingFieldCount, 'フィールド設定 UI 要素が最低 5 件描画される').toBeGreaterThan(5);
...
test.describe('日時フィールド拡張テスト', () => {
    ...
    test('FD15: 日時', async ({ page }) => {
        await test.step('387: 日時フィールド設定ページが ISE なく描画され、フィールド設定 UI 要素が表示されること', async () => {
            // モーダルが残っていたらリロード（cascade failure防止）
            if (await page.locator(".modal.show").count() > 0) {
                await page.reload({ waitUntil: "domcontentloaded", timeout: 15000 });
                await page.waitForSelector(".overSetting, .navbar", { timeout: 5000 }).catch(() => {});
            ...
            // [flow] 87-2b. Angular SPA の本体描画完了を待機
            await page.waitForFunction(
                () => {
                    const t = document.body.innerText || '';
                    return t.length >= 100;
            ...
            // [check] 87-3. ✅ フィールド設定ページが ISE なく表示されること
            const bodyText = await page.innerText('body');
            expect(bodyText, 'フィールド設定ページが ISE なく表示').not.toContain('Internal Server Error');

            // [check] 87-4. ✅ フィールド設定特有のキーワードが含まれること
            expect(bodyText, 'フィールド設定 UI キーワード (項目名/日時/タイプ) が含まれる').toMatch(/項目名|フィールド名|タイプ|名前|日時|表示/);

            // [check] 87-5. ✅ フィールド設定 UI 要素が複数描画されていること
            const settingFields = page.locator('input[type="text"], select, ng-select, label');
            const settingFieldCount = await settingFields.count();
            expect(settingFieldCount, 'フィールド設定 UI 要素が最低 5 件描画される').toBeGreaterThan(5);
    ...
    test('UC10: 日時（時刻のみ）のクリア', async ({ page }) => {
        await test.step('599: 日時フィールド設定ページが ISE なく描画され、フィールド設定 UI 要素が表示されること', async () => {
            // モーダルが残っていたらリロード（cascade failure防止）
            if (await page.locator(".modal.show").count() > 0) {
                await page.reload({ waitUntil: "domcontentloaded", timeout: 15000 });
                await page.waitForSelector(".overSetting, .navbar", { timeout: 5000 }).catch(() => {});
            ...
            // [flow] 99-2b. Angular SPA の本体描画完了を待機
            await page.waitForFunction(
                () => {
                    const t = document.body.innerText || '';
                    return t.length >= 100;
            ...
            // [check] 99-3. ✅ フィールド設定ページが ISE なく表示されること
            const bodyText = await page.innerText('body');
            expect(bodyText, 'フィールド設定ページが ISE なく表示').not.toContain('Internal Server Error');

            // [check] 99-4. ✅ フィールド設定特有のキーワードが含まれること
            expect(bodyText, 'フィールド設定 UI キーワード (項目名/日時/タイプ) が含まれる').toMatch(/項目名|フィールド名|タイプ|名前|日時|表示/);

            // [check] 99-5. ✅ フィールド設定 UI 要素が複数描画されていること
            const settingFields = page.locator('input[type="text"], select, ng-select, label');
            const settingFieldCount = await settingFields.count();
            expect(settingFieldCount, 'フィールド設定 UI 要素が最低 5 件描画される').toBeGreaterThan(5);
...
test.describe('ルックアップ・テーブル表示テスト', () => {
    ...
    test('FD17: 追加実装', async ({ page }) => {
        await test.step('575: 他テーブル参照フィールド設定ページが ISE なく描画され、設定 UI 要素が表示されること', async () => {
            // モーダルが残っていたらリロード（cascade failure防止）
            if (await page.locator(".modal.show").count() > 0) {
                await page.reload({ waitUntil: "domcontentloaded", timeout: 15000 });
                await page.waitForSelector(".overSetting, .navbar", { timeout: 5000 }).catch(() => {});
            ...
            // [flow] 75-2b. Angular SPA の本体描画完了を待機
            await page.waitForFunction(
                () => {
                    const t = document.body.innerText || '';
                    return t.length >= 100;
            ...
            // [check] 75-3. ✅ フィールド設定ページが ISE なく表示されること
            const bodyText = await page.innerText('body');
            expect(bodyText, 'フィールド設定ページが ISE なく表示').not.toContain('Internal Server Error');

            // [check] 75-4. ✅ フィールド設定特有のキーワードが含まれること
            expect(bodyText, 'フィールド設定 UI キーワード (項目名/参照/テーブル) が含まれる').toMatch(/項目名|フィールド名|タイプ|名前|参照|テーブル/);

            // [check] 75-5. ✅ フィールド設定 UI 要素が複数描画されていること
            const settingFields = page.locator('input[type="text"], select, ng-select, label');
            const settingFieldCount = await settingFields.count();
            expect(settingFieldCount, 'フィールド設定 UI 要素が最低 5 件描画される').toBeGreaterThan(5);
    ...
    test('UC13: ルックアップ', async ({ page }) => {
        await test.step('664: 他テーブル参照フィールド設定ページが ISE なく描画され、設定 UI 要素が表示されること', async () => {
            // モーダルが残っていたらリロード（cascade failure防止）
            if (await page.locator(".modal.show").count() > 0) {
                await page.reload({ waitUntil: "domcontentloaded", timeout: 15000 });
                await page.waitForSelector(".overSetting, .navbar", { timeout: 5000 }).catch(() => {});
            ...
            // [flow] 64-2b. Angular SPA の本体描画完了を待機
            await page.waitForFunction(
                () => {
                    const t = document.body.innerText || '';
                    return t.length >= 100;
            ...
            // [check] 64-3. ✅ フィールド設定ページが ISE なく表示されること
            const bodyText = await page.innerText('body');
            expect(bodyText, 'フィールド設定ページが ISE なく表示').not.toContain('Internal Server Error');

            // [check] 64-4. ✅ フィールド設定特有のキーワードが含まれること
            expect(bodyText, 'フィールド設定 UI キーワード (項目名/参照/テーブル) が含まれる').toMatch(/項目名|フィールド名|タイプ|名前|参照|テーブル/);

            // [check] 64-5. ✅ フィールド設定 UI 要素が複数描画されていること
            const settingFields = page.locator('input[type="text"], select, ng-select, label');
            const settingFieldCount = await settingFields.count();
            expect(settingFieldCount, 'フィールド設定 UI 要素が最低 5 件描画される').toBeGreaterThan(5);
    ...
    test('UC11: 他テーブル参照（複数）→文字列一行（複数）のルックアップ', async ({ page }) => {
        await test.step('623: 他テーブル参照フィールド設定ページが ISE なく描画され、設定 UI 要素が表示されること', async () => {
            // モーダルが残っていたらリロード（cascade failure防止）
            if (await page.locator(".modal.show").count() > 0) {
                await page.reload({ waitUntil: "domcontentloaded", timeout: 15000 });
                await page.waitForSelector(".overSetting, .navbar", { timeout: 5000 }).catch(() => {});
            ...
            // [flow] 23-2b. Angular SPA の本体描画完了を待機
            await page.waitForFunction(
                () => {
                    const t = document.body.innerText || '';
                    return t.length >= 100;
            ...
            // [check] 23-3. ✅ フィールド設定ページが ISE なく表示されること
            const bodyText = await page.innerText('body');
            expect(bodyText, 'フィールド設定ページが ISE なく表示').not.toContain('Internal Server Error');

            // [check] 23-4. ✅ フィールド設定特有のキーワードが含まれること
            expect(bodyText, 'フィールド設定 UI キーワード (項目名/参照/テーブル) が含まれる').toMatch(/項目名|フィールド名|タイプ|名前|参照|テーブル/);

            // [check] 23-5. ✅ フィールド設定 UI 要素が複数描画されていること
            const settingFields = page.locator('input[type="text"], select, ng-select, label');
            const settingFieldCount = await settingFields.count();
            expect(settingFieldCount, 'フィールド設定 UI 要素が最低 5 件描画される').toBeGreaterThan(5);
...
test.describe('テキスト・文字列フィールド拡張テスト', () => {
    ...
    test('UC01: リッチテキスト追加オプション', async ({ page }) => {
        await test.step('274: フィールド設定ページが ISE なく描画され、リッチテキストのフィールド設定 UI 要素が表示されること', async () => {
            // モーダルが残っていたらリロード（cascade failure防止）
            if (await page.locator(".modal.show").count() > 0) {
                await page.reload({ waitUntil: "domcontentloaded", timeout: 15000 });
                await page.waitForSelector(".overSetting, .navbar", { timeout: 5000 }).catch(() => {});
            ...
            // [flow] 74-2. 文章（複数行）フィールドをクリックして設定を開く
            ...
            if (await textAreaField.count() > 0) {
                ...
                // [flow] 74-3. 「追加オプション」ボタンをクリックして設定を展開
                const optionBtn = page.locator('button:has-text("追加オプション"), a:has-text("追加オプション")').first();
                if (await optionBtn.count() > 0) {
                    await optionBtn.click({ force: true });
                    await waitForAngular(page);
            ...
            // [flow] 74-3b. Angular SPA の本体描画完了を待機
            await page.waitForFunction(
                () => {
                    const t = document.body.innerText || '';
                    return t.length >= 100;
            ...
            // [check] 74-4. ✅ フィールド設定ページが ISE なく表示されること
            const bodyText = await page.innerText('body');
            expect(bodyText, 'フィールド設定ページが ISE なく表示').not.toContain('Internal Server Error');

            // [check] 74-5. ✅ フィールド設定特有のキーワードが含まれること
            expect(bodyText, 'フィールド設定 UI キーワード (項目名/フィールド/名前) が含まれる').toMatch(/項目名|フィールド名|タイプ|名前|表示|文章/);

            // [check] 74-6. ✅ フィールド設定 UI 要素が複数描画されていること
            const settingFields = page.locator('input[type="text"], select, ng-select, label');
            const settingFieldCount = await settingFields.count();
            expect(settingFieldCount, 'フィールド設定 UI 要素が最低 5 件描画される').toBeGreaterThan(5);
...
test.describe('ファイル・画像フィールド拡張テスト', () => {
    ...
    test('FD14: 項目', async ({ page }) => {
        await test.step('257: ファイルフィールド設定ページが ISE なく描画され、設定 UI 要素が表示されること', async () => {
            // モーダルが残っていたらリロード（cascade failure防止）
            if (await page.locator(".modal.show").count() > 0) {
                await page.reload({ waitUntil: "domcontentloaded", timeout: 15000 });
                await page.waitForSelector(".overSetting, .navbar", { timeout: 5000 }).catch(() => {});
            ...
            // [flow] 57-2. ファイルフィールドをクリックして設定を開く
            const fileField = page.locator('.pc-field-block').filter({ hasText: 'ファイル' }).first();
            if (await fileField.count() > 0) {
            ...
            // [flow] 57-2b. Angular SPA の本体描画完了を待機
            await page.waitForFunction(
                () => {
                    const t = document.body.innerText || '';
                    return t.length >= 100;
            ...
            // [check] 57-3. ✅ フィールド設定ページが ISE なく表示されること
            const bodyText = await page.innerText('body');
            expect(bodyText, 'フィールド設定ページが ISE なく表示').not.toContain('Internal Server Error');

            // [check] 57-4. ✅ フィールド設定特有のキーワードが含まれること
            expect(bodyText, 'フィールド設定 UI キーワード (項目名/ファイル/タイプ) が含まれる').toMatch(/項目名|フィールド名|タイプ|名前|ファイル|表示/);

            // [check] 57-5. ✅ フィールド設定 UI 要素が複数描画されていること
            const settingFields = page.locator('input[type="text"], select, ng-select, label');
            const settingFieldCount = await settingFields.count();
            expect(settingFieldCount, 'フィールド設定 UI 要素が最低 5 件描画される').toBeGreaterThan(5);
    ...
    test('FD16: 文字列', async ({ page }) => {
        await test.step('497: フィールド設定ページが ISE なく描画され、ファイル系フィールド設定 UI 要素が表示されること', async () => {
            // モーダルが残っていたらリロード（cascade failure防止）
            if (await page.locator(".modal.show").count() > 0) {
                await page.reload({ waitUntil: "domcontentloaded", timeout: 15000 });
                await page.waitForSelector(".overSetting, .navbar", { timeout: 5000 }).catch(() => {});
            ...
            // [flow] 97-1b. Angular SPA の本体描画完了を待機
            await page.waitForFunction(
                () => {
                    const t = document.body.innerText || '';
                    return t.length >= 100;
            ...
            // [check] 97-2. ✅ フィールド設定ページが ISE なく表示されること
            const bodyText = await page.innerText('body');
            expect(bodyText, 'フィールド設定ページが ISE なく表示').not.toContain('Internal Server Error');

            // [check] 97-3. ✅ フィールド設定特有のキーワードが含まれること
            expect(bodyText, 'フィールド設定 UI キーワード (項目名/タイプ/名前) が含まれる').toMatch(/項目名|フィールド名|タイプ|名前|ファイル|表示/);

            // [check] 97-4. ✅ フィールド設定 UI 要素が複数描画されていること
            const settingFields = page.locator('input[type="text"], select, ng-select, label');
            const settingFieldCount = await settingFields.count();
            expect(settingFieldCount, 'フィールド設定 UI 要素が最低 5 件描画される').toBeGreaterThan(5);
...
test.describe('ビュー・項目並べ替え・自動採番・その他テスト', () => {
    ...
    test('FD15: 日時', async ({ page }) => {
        await test.step('388: フィールド設定ページが ISE なく描画され、自動採番フィールド設定 UI 要素が表示されること', async () => {
            // モーダルが残っていたらリロード（cascade failure防止）
            if (await page.locator(".modal.show").count() > 0) {
                await page.reload({ waitUntil: "domcontentloaded", timeout: 15000 });
                await page.waitForSelector(".overSetting, .navbar", { timeout: 5000 }).catch(() => {});
            ...
            // [flow] 88-2. 自動採番フィールドをクリックして設定を開く
            const autoNumField = page.locator('.pc-field-block').filter({ hasText: '自動採番' }).first();
            if (await autoNumField.count() > 0) {
            ...
            // [flow] 88-2b. Angular SPA の本体描画完了を待機
            await page.waitForFunction(
                () => {
                    const t = document.body.innerText || '';
                    return t.length >= 100;
            ...
            // [check] 88-3. ✅ フィールド設定ページが ISE なく表示されること
            const bodyText = await page.innerText('body');
            expect(bodyText, 'フィールド設定ページが ISE なく表示').not.toContain('Internal Server Error');

            // [check] 88-4. ✅ フィールド設定特有のキーワードが含まれること
            expect(bodyText, 'フィールド設定 UI キーワード (項目名/タイプ/名前) が含まれる').toMatch(/項目名|フィールド名|タイプ|名前|表示|設定/);

            // [check] 88-5. ✅ フィールド設定 UI 要素が複数描画されていること
            const settingFields = page.locator('input[type="text"], select, ng-select, label');
            const settingFieldCount = await settingFields.count();
            expect(settingFieldCount, 'フィールド設定 UI 要素が最低 5 件描画される').toBeGreaterThan(5);

        });
        await test.step('414: フィールド設定ページが ISE なく描画され、ビュー設定 UI 要素が表示されること', async () => {
            // モーダルが残っていたらリロード（cascade failure防止）
            if (await page.locator(".modal.show").count() > 0) {
                await page.reload({ waitUntil: "domcontentloaded", timeout: 15000 });
                await page.waitForSelector(".overSetting, .navbar", { timeout: 5000 }).catch(() => {});
            ...
            // [flow] 14-2b. Angular SPA の本体描画完了を待機
            await page.waitForFunction(
                () => {
                    const t = document.body.innerText || '';
                    return t.length >= 100;
            ...
            // [check] 14-3. ✅ フィールド設定ページが ISE なく表示されること
            const bodyText = await page.innerText('body');
            expect(bodyText, 'フィールド設定ページが ISE なく表示').not.toContain('Internal Server Error');

            // [check] 14-4. ✅ フィールド設定特有のキーワードが含まれること
            expect(bodyText, 'フィールド設定 UI キーワード (項目名/タイプ/名前) が含まれる').toMatch(/項目名|フィールド名|タイプ|名前|表示|設定/);

            // [check] 14-5. ✅ フィールド設定 UI 要素が複数描画されていること
            const settingFields = page.locator('input[type="text"], select, ng-select, label');
            const settingFieldCount = await settingFields.count();
            expect(settingFieldCount, 'フィールド設定 UI 要素が最低 5 件描画される').toBeGreaterThan(5);
...
test.describe('フィールド関連汎用修正確認テスト', () => {
    ...
    test('UC16: 計算項目', async ({ page }) => {
        await test.step('723: フィールド設定ページが ISE なく描画され、フィールド設定 UI 要素が表示されること', async () => {
            // モーダルが残っていたらリロード（cascade failure防止）
            if (await page.locator(".modal.show").count() > 0) {
                await page.reload({ waitUntil: "domcontentloaded", timeout: 15000 });
                await page.waitForSelector(".overSetting, .navbar", { timeout: 5000 }).catch(() => {});
            ...
            // [flow] 23-1b. Angular SPA の本体描画完了を待機
            await page.waitForFunction(
                () => {
                    const t = document.body.innerText || '';
                    return t.length >= 100;
            ...
            // [check] 23-2. ✅ フィールド設定ページが ISE なく表示されること
            const bodyText = await page.innerText('body');
            expect(bodyText, 'フィールド設定ページが ISE なく表示').not.toContain('Internal Server Error');

            // [check] 23-3. ✅ フィールド設定特有のキーワードが含まれること
            expect(bodyText, 'フィールド設定 UI キーワード (項目名/タイプ/名前) が含まれる').toMatch(/項目名|フィールド名|タイプ|名前|表示|設定/);

            // [check] 23-4. ✅ フィールド設定 UI 要素が複数描画されていること
            const settingFields = page.locator('input[type="text"], select, ng-select, label');
            const settingFieldCount = await settingFields.count();
            expect(settingFieldCount, 'フィールド設定 UI 要素が最低 5 件描画される').toBeGreaterThan(5);
    ...
    test('UC20: 数値項目', async ({ page }) => {
        await test.step('794: フィールド設定ページが ISE なく描画され、フィールド設定 UI 要素が表示されること', async () => {
            // モーダルが残っていたらリロード（cascade failure防止）
            if (await page.locator(".modal.show").count() > 0) {
                await page.reload({ waitUntil: "domcontentloaded", timeout: 15000 });
                await page.waitForSelector(".overSetting, .navbar", { timeout: 5000 }).catch(() => {});
            ...
            // [flow] 94-1b. Angular SPA の本体描画完了を待機
            await page.waitForFunction(
                () => {
                    const t = document.body.innerText || '';
                    return t.length >= 100;
            ...
            // [check] 94-2. ✅ フィールド設定ページが ISE なく表示されること
            const bodyText = await page.innerText('body');
            expect(bodyText, 'フィールド設定ページが ISE なく表示').not.toContain('Internal Server Error');

            // [check] 94-3. ✅ フィールド設定特有のキーワードが含まれること
            expect(bodyText, 'フィールド設定 UI キーワード (項目名/タイプ/名前) が含まれる').toMatch(/項目名|フィールド名|タイプ|名前|表示|設定/);

            // [check] 94-4. ✅ フィールド設定 UI 要素が複数描画されていること
            const settingFields = page.locator('input[type="text"], select, ng-select, label');
            const settingFieldCount = await settingFields.count();
            expect(settingFieldCount, 'フィールド設定 UI 要素が最低 5 件描画される').toBeGreaterThan(5);
    ...
    test('UC15: 自動採番フォーマット（年度・UNIQ-ID）', async ({ page }) => {
        await test.step('702: フィールド設定ページが ISE なく描画され、フィールド設定 UI 要素が表示されること', async () => {
            // モーダルが残っていたらリロード（cascade failure防止）
            if (await page.locator(".modal.show").count() > 0) {
                await page.reload({ waitUntil: "domcontentloaded", timeout: 15000 });
                await page.waitForSelector(".overSetting, .navbar", { timeout: 5000 }).catch(() => {});
            ...
            // [flow] 02-1b. Angular SPA の本体描画完了を待機
            await page.waitForFunction(
                () => {
                    const t = document.body.innerText || '';
                    return t.length >= 100;
            ...
            // [check] 02-2. ✅ フィールド設定ページが ISE なく表示されること
            const bodyText = await page.innerText('body');
            expect(bodyText, 'フィールド設定ページが ISE なく表示').not.toContain('Internal Server Error');

            // [check] 02-3. ✅ フィールド設定特有のキーワードが含まれること
            expect(bodyText, 'フィールド設定 UI キーワード (項目名/タイプ/名前) が含まれる').toMatch(/項目名|フィールド名|タイプ|名前|表示|設定/);

            // [check] 02-4. ✅ フィールド設定 UI 要素が複数描画されていること
            const settingFields = page.locator('input[type="text"], select, ng-select, label');
            const settingFieldCount = await settingFields.count();
            expect(settingFieldCount, 'フィールド設定 UI 要素が最低 5 件描画される').toBeGreaterThan(5);
    ...
    test('UC17: 複数値の文字列一行の表示', async ({ page }) => {
        await test.step('734: フィールド設定ページが ISE なく描画され、フィールド設定 UI 要素が表示されること', async () => {
            // モーダルが残っていたらリロード（cascade failure防止）
            if (await page.locator(".modal.show").count() > 0) {
                await page.reload({ waitUntil: "domcontentloaded", timeout: 15000 });
                await page.waitForSelector(".overSetting, .navbar", { timeout: 5000 }).catch(() => {});
            ...
            // [flow] 34-1b. Angular SPA の本体描画完了を待機
            await page.waitForFunction(
                () => {
                    const t = document.body.innerText || '';
                    return t.length >= 100;
            ...
            // [check] 34-2. ✅ フィールド設定ページが ISE なく表示されること
            const bodyText = await page.innerText('body');
            expect(bodyText, 'フィールド設定ページが ISE なく表示').not.toContain('Internal Server Error');

            // [check] 34-3. ✅ フィールド設定特有のキーワードが含まれること
            expect(bodyText, 'フィールド設定 UI キーワード (項目名/タイプ/名前) が含まれる').toMatch(/項目名|フィールド名|タイプ|名前|表示|設定/);

            // [check] 34-4. ✅ フィールド設定 UI 要素が複数描画されていること
            const settingFields = page.locator('input[type="text"], select, ng-select, label');
            const settingFieldCount = await settingFields.count();
            expect(settingFieldCount, 'フィールド設定 UI 要素が最低 5 件描画される').toBeGreaterThan(5);
    ...
    test('UC12: 子テーブルのSUMIF関数', async ({ page }) => {
        await test.step('649: フィールド設定ページが ISE なく描画され、フィールド設定 UI 要素が表示されること', async () => {
            // モーダルが残っていたらリロード（cascade failure防止）
            if (await page.locator(".modal.show").count() > 0) {
                await page.reload({ waitUntil: "domcontentloaded", timeout: 15000 });
                await page.waitForSelector(".overSetting, .navbar", { timeout: 5000 }).catch(() => {});
            ...
            // [flow] 49-1b. Angular SPA の本体描画完了を待機
            await page.waitForFunction(
                () => {
                    const t = document.body.innerText || '';
                    return t.length >= 100;
            ...
            // [check] 49-2. ✅ フィールド設定ページが ISE なく表示されること
            const bodyText = await page.innerText('body');
            expect(bodyText, 'フィールド設定ページが ISE なく表示').not.toContain('Internal Server Error');

            // [check] 49-3. ✅ フィールド設定特有のキーワードが含まれること
            expect(bodyText, 'フィールド設定 UI キーワード (項目名/タイプ/名前) が含まれる').toMatch(/項目名|フィールド名|タイプ|名前|表示|設定/);

            // [check] 49-4. ✅ フィールド設定 UI 要素が複数描画されていること
            const settingFields = page.locator('input[type="text"], select, ng-select, label');
            const settingFieldCount = await settingFields.count();
            expect(settingFieldCount, 'フィールド設定 UI 要素が最低 5 件描画される').toBeGreaterThan(5);
    ...
    test('UC03: テストケース101-7バグ修正確認', async ({ page }) => {
        await test.step('365: フィールド設定ページが ISE なく描画され、日時フィールド設定 UI 要素が表示されること', async () => {
            // モーダルが残っていたらリロード（cascade failure防止）
            if (await page.locator(".modal.show").count() > 0) {
                await page.reload({ waitUntil: "domcontentloaded", timeout: 15000 });
                await page.waitForSelector(".overSetting, .navbar", { timeout: 5000 }).catch(() => {});
            ...
            // [flow] 65-2b. Angular SPA の本体描画完了を待機
            await page.waitForFunction(
                () => {
                    const t = document.body.innerText || '';
                    return t.length >= 100;
            ...
            // [check] 65-3. ✅ フィールド設定ページが ISE なく表示されること
            const bodyText = await page.innerText('body');
            expect(bodyText, 'フィールド設定ページが ISE なく表示').not.toContain('Internal Server Error');

            // [check] 65-4. ✅ フィールド設定特有のキーワードが含まれること
            expect(bodyText, 'フィールド設定 UI キーワード (項目名/タイプ/名前) が含まれる').toMatch(/項目名|フィールド名|タイプ|名前|表示|設定/);

            // [check] 65-5. ✅ フィールド設定 UI 要素が複数描画されていること
            const settingFields = page.locator('input[type="text"], select, ng-select, label');
            const settingFieldCount = await settingFields.count();
            expect(settingFieldCount, 'フィールド設定 UI 要素が最低 5 件描画される').toBeGreaterThan(5);
    ...
    test('UC06: 計算項目の入力行制限（4項目以上禁止）', async ({ page }) => {
        await test.step('524: フィールド設定ページが ISE なく描画され、フィールド設定 UI 要素が表示されること', async () => {
            // モーダルが残っていたらリロード（cascade failure防止）
            if (await page.locator(".modal.show").count() > 0) {
                await page.reload({ waitUntil: "domcontentloaded", timeout: 15000 });
                await page.waitForSelector(".overSetting, .navbar", { timeout: 5000 }).catch(() => {});
            ...
            // [flow] 24-1b. Angular SPA の本体描画完了を待機
            await page.waitForFunction(
                () => {
                    const t = document.body.innerText || '';
                    return t.length >= 100;
            ...
            // [check] 24-2. ✅ フィールド設定ページが ISE なく表示されること
            const bodyText = await page.innerText('body');
            expect(bodyText, 'フィールド設定ページが ISE なく表示').not.toContain('Internal Server Error');

            // [check] 24-3. ✅ フィールド設定特有のキーワードが含まれること
            expect(bodyText, 'フィールド設定 UI キーワード (項目名/タイプ/名前) が含まれる').toMatch(/項目名|フィールド名|タイプ|名前|表示|設定/);

            // [check] 24-4. ✅ フィールド設定 UI 要素が複数描画されていること
            const settingFields = page.locator('input[type="text"], select, ng-select, label');
            const settingFieldCount = await settingFields.count();
            expect(settingFieldCount, 'フィールド設定 UI 要素が最低 5 件描画される').toBeGreaterThan(5);
