...
test.describe('テーブル定義（テーブル管理・テーブル設定・追加オプション）', () => {
    ...
    test('25-9: テーブルのアイコン設定がエラーなく行えること', async ({ page }) => {
        ...
        await expect(activePanel.first()).toBeVisible({ timeout: 30000 });
    ...
    test('12-10: テーブル権限設定で組織・閲覧・編集・1データのみ登録可能・条件制限が設定できること', async ({ page }) => {
        ...
        // 権限設定ページが表示されることを確認
        await expect(page.locator('.navbar')).toBeVisible({ timeout: 60000 });
        await expect(page.locator('.tab-pane.active, main').first()).toBeVisible({ timeout: 30000 });
        const errorEl = page.locator('.alert-danger');
        const errorCount = await errorEl.count();
        expect(errorCount).toBe(0);
    });
    ...
    test('98-2: CSVアップロードで{NOCHANGE}を使うと指定した複数項目のみ更新されること', async ({ page }) => {
        ...
        await expect(hamburgerBtn2).toBeVisible({ timeout: 5000 });
    ...
    test('50-1: 他テーブル参照で項目名を未入力で追加するとエラーが発生すること', async ({ page }) => {
        ...
        const addFieldBtn = page.locator('button, a').filter({ hasText: /項目を追加/ }).first();
        await expect(addFieldBtn, '項目追加ボタンが存在すること').toBeVisible({ timeout: 30000 });
    ...
    test('4-2: Excelよりテーブル追加がエラーなく行えること', async ({ page }) => {
        ...
        await expect(addTableBtn).toBeVisible({ timeout: 30000 });
    ...
    test('259: テーブルの詳細画面が正常に表示されること', async ({ page }) => {
        ...
        await expect(tableHeader.first()).toBeVisible({ timeout: 30000 });
...
test.describe('テーブル定義追加テスト', () => {
    ...
    test('265: 他テーブル参照で値「0」を選択して登録できること', async ({ page }) => {
        ...
        await expect(refField).toBeVisible({ timeout: 30000 });
    ...
    test('33-1: テーブル権限設定に使用中の組織を削除しようとするとエラーになること', async ({ page }) => {
        ...
        await expect(addOrgBtn).toBeVisible({ timeout: 30000 });
