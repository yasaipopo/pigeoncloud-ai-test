...
test.describe('テーブルアーカイブ', () => {
    ...
    test('ARC-01: テーブルをアーカイブできること', async ({ page }) => {
        ...
        // アーカイブボタンはテーブルデータ一覧ページ（/admin/dataset__XXX）に表示される
        // admin.component.html: <button (click)="archiveTable()" *ngIf="grant.edit && dataset_view && !showUnarchiveButton">
        expect(archiveTableId, 'アーカイブテスト用のテーブルIDが取得できること').toBeTruthy();

        await page.goto(BASE_URL + '/admin/dataset__' + archiveTableId, { waitUntil: 'domcontentloaded', timeout: 30000 }).catch(() => {});
        await waitForAngular(page).catch(() => {});
        await expect(page.locator('.navbar')).toBeVisible({ timeout: 30000 });

        // 「アーカイブする」ボタンを探す（テーブルデータ一覧ページのヘッダー領域）
...
test.describe('自動採番リセット', () => {
    ...
    test('AUTOID-01: テーブル設定画面に自動採番リセット機能が存在すること', async ({ page }) => {
        ...
        // 「項目設定に戻る」リンクをクリックしてフィールド一覧画面に移動
        // テーブル設定の基本設定タブでは歯車アイコンは表示されない
        // 「<< 項目設定に戻る」ボタンをpage.evaluateでクリック（locatorだとテキストマッチが難しい）
        await page.evaluate(() => {
    ...
    test('AUTOID-02: 自動採番リセットを実行できること', async ({ page }) => {
        ...
        // 「項目設定に戻る」リンクをクリックしてフィールド一覧画面に移動
        const backToFieldsBtn = page.locator('button:has-text("項目設定に戻る"), a:has-text("項目設定に戻る"), :has-text("<< 項目設定に戻る")').first();
