...
test.describe('テーブル定義（テーブル管理・テーブル設定・追加オプション）', () => {
    ...
    test('98-1: CSVアップロードで{NOCHANGE}を使うと指定した1項目のみ更新されること', async ({ page }) => {
        ...
        // CSVアップロードモーダルを開く
        const csvUploadBtn = page.locator('button:has-text("CSVアップロード"), a:has-text("CSVアップロード")').first();
        await expect(csvUploadBtn).toBeVisible({ timeout: 10000 });
        await csvUploadBtn.click();
        await page.waitForTimeout(1000);
        ...
        if (hasDlBtn) {
            ...
            const csvText = await (await download.createReadStream()).read().catch(() => null);
            if (csvText) {
...
test.describe('テーブルオプション設定の動作確認（109系オプション反映）', () => {
    ...
    test('109-1: 複製ボタン非表示設定をONにするとレコード詳細画面で複製ボタンが非表示になること', async ({ page }) => {
        ...
        if (firstRecordId) {
            await page.goto(BASE_URL + `/admin/dataset__${tableId}/${firstRecordId}`);
    ...
    test('109-2: 一覧編集モードをONにするとレコード一覧でインライン編集UIが表示されること', async ({ page }) => {
        ...
        // 「一覧編集モード」トグルを探す
        const inlineEditToggle = page.locator(
            'label:has-text("一覧編集"), label:has-text("インライン編集"), input[name*="inline_edit"], input[name*="list_edit"]'
        ).filter({ visible: true }).first();
        const inlineEditCount = await inlineEditToggle.count();

        expect(inlineEditCount, '一覧編集モード設定が表示されること').toBeGreaterThan(0);

        // チェックボックスをONにする
        const toggleInput = inlineEditToggle.locator('input[type="checkbox"]').first();
        const hasCheckbox = await toggleInput.count();
        let targetInput;
        if (hasCheckbox > 0) {
            targetInput = toggleInput;
        } else {
            // labelの隣のinputを探す
        ...
        expect(targetCount, '一覧編集モードのチェックボックスが存在すること').toBeGreaterThan(0);
...
test.describe('テーブルアーカイブ', () => {
    ...
    test.beforeAll(async ({ browser }) => {
        ...
        await page.goto(BASE_URL + '/admin/dataset/create');
        await page.waitForLoadState('domcontentloaded');
        await page.waitForTimeout(2000);
    ...
    test('ARC-01: テーブルをアーカイブできること', async ({ page }) => {
        ...
        if (archiveBtnCount === 0) {
            ...
            if (archiveTableId) {
                await page.goto(BASE_URL + '/admin/dataset/edit/' + archiveTableId);
                await page.waitForLoadState('domcontentloaded');
                await page.waitForTimeout(2000);
                ...
                if (await archiveBtnInEdit.count() === 0) {
                    expect(false, 'アーカイブボタンが存在すること').toBeTruthy();
                }
    ...
    test('ARC-02: アーカイブしたテーブルを復元できること', async ({ page }) => {
        ...
        if (await restoreBtn.count() === 0) {
            // 復元ボタンが見つからない場合は、UIが存在することを確認するだけ
            console.log('ARC-02: 復元ボタンが見つからなかった（アーカイブ済みテーブルがないか、UIが異なる）');
            await expect(page.locator('.navbar')).toBeVisible();
            expect(false, '復元ボタンが存在すること').toBeTruthy();
        }
...
test.describe('自動採番リセット', () => {
    ...
    test('AUTOID-01: テーブル設定画面に自動採番リセット機能が存在すること', async ({ page }) => {
        ...
        if (elCount === 0) {
            ...
            // 見つからない場合はUIが存在しないとしてスキップ
            console.log('AUTOID-01: 自動採番リセットUIが見つからなかった（UIが存在しない可能性）');
            await expect(page.locator('.navbar')).toBeVisible();
            expect(false, '自動採番リセットUIが存在すること').toBeTruthy();
        }
    ...
    test('AUTOID-02: 自動採番リセットを実行できること', async ({ page }) => {
        ...
        if (await autoIdResetBtn.count() === 0) {
            expect(false, '自動採番リセットボタンが存在すること').toBeTruthy();
        }
