        ...
        if (archiveBtnCount === 0) {
            ...
            if (archiveTableId) {
                ...
                if (await archiveBtnInEdit.count() === 0) {
                    throw new Error('ARC-01: アーカイブボタンが見つからなかった — /admin/dataset と /admin/dataset/edit/{id} 両方で確認済み。機能のUI構造を確認してください');
                }
                await archiveBtnInEdit.click();
            } else {
                throw new Error('ARC-01: アーカイブボタンが見つからなかった — /admin/dataset ページにアーカイブUIがありません。機能のUI構造を確認してください');
            }
        } else {
...
test.describe('自動採番リセット', () => {
    ...
    test('AUTOID-01: テーブル設定画面に自動採番リセット機能が存在すること', async ({ page }) => {
        ...
        if (elCount === 0) {
            ...
            throw new Error('AUTOID-01: 自動採番リセットUIが全タブで見つからなかった — テーブル設定の「その他」「基本設定」「メニュー」「CSV」タブを確認してください');
        }
    ...
    test('AUTOID-02: 自動採番リセットを実行できること', async ({ page }) => {
        ...
        if (await autoIdResetBtn.count() === 0) {
            throw new Error('AUTOID-02: 自動採番リセットボタンが見つからなかった — テーブル設定の「その他」タブのUI構造を確認してください');
        }
