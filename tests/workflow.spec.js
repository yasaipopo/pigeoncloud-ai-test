...
async function createWorkflowTestTable(page) {
    ...
    await page.waitForURL(/\/dataset__\d+/, { timeout: 15000, waitUntil: 'domcontentloaded' });
...
async function createRecordAndSubmit(page, tableId, approverName, comment = '') {
    ...
    await page.goto(BASE_URL + `/admin/dataset__${tableId}/edit/new`, { waitUntil: "domcontentloaded", timeout: 15000 }).catch(() => {});
...
test.describe('ワークフロー設定（21系）', () => {
    ...
    test('WF03: ワークフロー', async ({ page }) => {
        ...
        await test.step('21-4: ワークフロー申請の取り下げがエラーなく完了すること', async () => {
            ...
            // 申請取り下げボタンが表示されること
            await expect(page.locator('button:has-text("申請取り下げ")')).toBeVisible();
            ...
            await expect(page.locator('button:has-text("申請取り下げ")')).not.toBeVisible().catch(() => {});
...
test.describe('ワークフロー基本動作（11系）', () => {
    ...
    test('WF03: ワークフロー', async ({ page }) => {
        ...
        await test.step('11-4: 否認された後に再申請ができること', async () => {
            ...
            await expect(page.locator('button').filter({ hasText: /^申請$/ }).first()).toBeVisible();
        ...
        await test.step('11-5: 組織による承認（一人の承認が必要）ができること', async () => {
            ...
            await page.locator('.card-footer button').filter({ hasText: /^申請$/ }).first().waitFor({ state: 'visible', timeout: 5000 });
            await page.waitForLoadState('networkidle', { timeout: 8000 }).catch(() => {});
            await page.waitForTimeout(500);
            ...
            await waitForAngular(page);
            // 組織(役職)の設定UIが表示されること
            await expect(page.locator('division-forms-field').first()).toBeVisible();
        ...
        await test.step('11-6: 承認者データ編集可能設定が申請フローに反映されること', async () => {
            ...
            expect(bodyText).not.toContain('Internal Server Error');
            // 承認ボタンが表示されていること
            await expect(page.locator('button:has-text("承認")').filter({ hasNotText: '一括' }).first()).toBeVisible();
...
test.describe('一括操作（111系）', () => {
    ...
    test('WF02: ワークフロー', async ({ page }) => {
        ...
        await test.step('111-02: 申請を複数選択して一括承認できること', async () => {
            ...
            if (rid1) {
                await page.goto(BASE_URL + `/admin/dataset__${tableId}/view/${rid1}`, { waitUntil: "domcontentloaded", timeout: 15000 }).catch(() => {});
                await waitForAngular(page);
        ...
        await test.step('111-13: 申請を1つ選択して一括取り下げできること', async () => {
            ...
            await expect(bulkWithdrawBtn).toBeVisible();
    ...
    test('WF04: ワークフロー', async ({ page }) => {
        ...
        await test.step('28-4: ワークフロー申請中に承認者組織を削除しても問題ないこと', async () => {
            ...
            if (recordId2) {
                ...
                await page.goto(BASE_URL + `/admin/dataset__${tableId}/view/${recordId2}`, { waitUntil: "domcontentloaded", timeout: 15000 }).catch(() => {});
                await waitForAngular(page);
                const bodyText = await page.innerText('body');
                expect(bodyText).not.toContain('Internal Server Error');
            } else {
...
test.describe('通知（36系）', () => {
    ...
    test('WF04: ワークフロー', async ({ page }) => {
        await test.step('36-1: ユーザーを申請者として申請し通知が届くこと', async () => {
            ...
            expect(recordId).toBeTruthy();
            // 通知一覧ページで確認
            await page.goto(BASE_URL + '/admin/notifications', { waitUntil: "domcontentloaded", timeout: 15000 }).catch(() => {});
            await waitForAngular(page);
            const bodyText = await page.innerText('body');
            expect(bodyText).not.toContain('Internal Server Error');
    ...
    test('WF05: ワークフロー', async ({ page }) => {
        ...
        await test.step('398-1: 同一ステップ内で複数の承認者を追加できること', async () => {
            ...
            if (await andOrSelect.count() > 0) {
                await expect(andOrSelect.first()).toBeVisible();
            }
...
test.describe('バグ修正確認・機能改善確認（WF関連）', () => {
    ...
    test('WF06: ワークフロー', async ({ page }) => {
        ...
        await test.step('446: ワークフロー申請ボタン押下時に申請画面が正常に表示されること', async () => {
            ...
            await page.waitForSelector('button.btn-primary:has-text("申請する")', { timeout: 20000 });
            const addFlowBtn = page.locator('button:has-text("承認フロー追加")').first();
            await expect(addFlowBtn).toBeVisible();
            const bodyText = await page.innerText('body');
            expect(bodyText).not.toContain('Internal Server Error');
            await page.locator('button.btn-secondary:has-text("キャンセル")').click().catch(() => {});
    ...
    test('WF07: ワークフロー', async ({ page }) => {
        ...
        await test.step('464: ワークフロー設定画面で閲覧権限関連のUIがエラーなく表示されること', async () => {
            ...
            const bodyText = await page.innerText('body');
            expect(bodyText).not.toContain('Internal Server Error');
            await expect(page.locator('dataset-workflow-options')).toBeVisible();
        ...
        await test.step('480: WF否認/取り下げ後の編集画面でテンプレートが正しく表示されること', async () => {
            ...
            const bodyText = await page.innerText('body');
            expect(bodyText).not.toContain('Internal Server Error');
            await expect(page.locator('.card-footer').first()).toBeVisible();
        ...
        await test.step('613: サイドメニューのWFバッジがテーブル名が長くても表示されること', async () => {
            ...
            const bodyText = await page.innerText('body');
            expect(bodyText).not.toContain('Internal Server Error');
            await expect(page.locator('.sidebar, .nav-sidebar, nav').first()).toBeVisible();
    ...
    test('UC05: ワークフロー否認/取り下げ後のデータ編集', async ({ page }) => {
