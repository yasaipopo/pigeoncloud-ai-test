...
test.describe('レコード操作（一覧・作成・編集・削除・一括編集）', () => {
    ...
    test('RC01: レコード一覧', async ({ page }) => {
        await test.step('143-01: レコード一覧にコメントアイコンが表示されること', async () => {
            ...
            // テーブルヘッダー行が存在すること
            await expect(page.locator('tr[mat-header-row]')).toBeVisible({ timeout: 60000 });
            ...
            await expect(page.locator('tr[mat-row] input[type="checkbox"]').first()).toBeVisible({ timeout: 60000 });
            ...
            await expect(commentIcon, 'コメント投稿後にコメントアイコンが一覧に表示されること').toBeVisible({ timeout: 60000 });
        ...
        await test.step('167-1: チェックボックスをクリックすると一括削除ボタンが表示されること', async () => {
            const STEP_TIME = Date.now();
        ...
        await test.step('237: レコード一覧のスクロールバーを問題なく操作できること', async () => {
            const STEP_TIME = Date.now();
            ...
            await expect(page.locator('.navbar')).toBeVisible({ timeout: 60000 });
            await expect(page.locator('table.pc-list-view, table[mat-table]')).toBeVisible({ timeout: 60000 });
    ...
    test('RC02: 一括編集', async ({ page }) => {
        await test.step('180-1: 一括編集メニューが表示され、一括編集を実行できること', async () => {
            const STEP_TIME = Date.now();
        ...
        await test.step('180-3: 一括編集モーダルにロック中データの扱いが説明されていること', async () => {
            const STEP_TIME = Date.now();
            ...
            await expect(dataRows.first(), 'テーブルにデータが存在すること').toBeVisible({ timeout: 60000 });
            ...
            // 一括編集モーダルが開くことを確認
            const modal = page.locator('.modal.show').first();
            await expect(modal, '一括編集モーダルが表示されること').toBeVisible({ timeout: 60000 });

            // モーダルにロック中データの扱いが説明されていることを確認
            const lockWarningText = modal.locator(
                ':text("編集中でロックされているデータは更新されません"), :text("ロックされているデータは更新されません")'
            ).first();
            await expect(lockWarningText, '一括編集モーダルにロック中データは更新されない旨の説明があること').toBeVisible({ timeout: 60000 });
        ...
        await test.step('180-4: フィルタ適用中にのみ一括編集がかかること', async () => {
            const STEP_TIME = Date.now();
            ...
            if (filteredRows >= totalRows) {
                ...
                await expect(hamburgerBtn2, 'ハンバーガーメニューが存在すること').toBeVisible({ timeout: 60000 });
                await hamburgerBtn2.click();
                await waitForAngular(page);
                const bulkEditItem2 = page.locator('.dropdown-menu.show .dropdown-item:has-text("一括編集")').first();
                await expect(bulkEditItem2, '「一括編集」メニューが表示されること').toBeVisible({ timeout: 60000 });
            ...
            // ------------------------------------------------------------------
            const hamburgerBtn = page.locator('button:has(.fa-bars)').first();
            await expect(hamburgerBtn, 'ハンバーガーメニューが存在すること').toBeVisible({ timeout: 60000 });
            await hamburgerBtn.click();
            await waitForAngular(page);

            const bulkEditItem = page.locator('.dropdown-menu.show .dropdown-item:has-text("一括編集")').first();
            await expect(bulkEditItem, 'フィルタ適用中でも「一括編集」メニューが表示されること').toBeVisible({ timeout: 60000 });
            await bulkEditItem.click();
            await waitForAngular(page);
            ...
            if (addItemVisible) {
                ...
                await expect(executeBtn, '一括編集実行ボタンが存在すること').toBeVisible({ timeout: 60000 });
            }
    ...
    test('RC03: 関連レコード一覧', async ({ page }) => {
        await test.step('35-1: 参照中のテーブルを削除しようとするとエラーが表示されること', async () => {
            const STEP_TIME = Date.now();
            ...
            await gearBtn.waitFor({ state: 'visible', timeout: 30000 });
        ...
        await test.step('52-1: 関連レコード一覧の項目名未入力でエラーが発生すること', async () => {
            const STEP_TIME = Date.now();
            ...
            await expect(addFieldBtn, '「項目を追加する」ボタンが存在すること').toBeVisible({ timeout: 60000 });
            await addFieldBtn.click({ force: true });
            await waitForAngular(page);

            // settingModalが開いていることを確認（型選択フェーズ）
            await page.waitForSelector('.modal.settingModal.show', { timeout: 30000 }).catch(() => {});
            ...
            await expect(errorMsg52_1, '項目名未入力でエラーメッセージが表示されること').toBeVisible({ timeout: 60000 });
        ...
        await test.step('52-2: 関連レコード一覧の対象テーブル未入力でエラーが発生すること', async () => {
            const STEP_TIME = Date.now();
            ...
            await expect(addFieldBtn52_2, '「項目を追加する」ボタンが存在すること').toBeVisible({ timeout: 60000 });
            ...
            await expect(errorMsg52_2, '対象テーブル未入力でエラーメッセージが表示されること').toBeVisible({ timeout: 60000 });
...
test.describe('レコード一括操作（チェックボックス選択・一括削除・一括編集）', () => {
    ...
    test.beforeAll(async ({ browser }) => {
        ...
        try {
            ...
            // データが実際にDBに存在することを確認（最大30秒ポーリング）
            for (let i = 0; i < 6; i++) {
                await page.waitForTimeout(5000);
                try {
                    const status = await page.evaluate(async (baseUrl) => {
    ...
    test('一括-2: 全選択チェックボックスをクリックすると一括操作ボタンが表示されること', async ({ page }) => {
        ...
        const dataRows = page.locator('tr[mat-row]');
        await expect(dataRows.first(), 'データ行が表示されること（beforeAllで作成済み）').toBeVisible({ timeout: 60000 });
    ...
    test('LOCK-01: レコード編集開始でロック状態になること', async ({ page }) => {
        ...
        if (recordId) {
            ...
            if (detailBtnVisible) {
                ...
                const firstCheckbox = page.locator('tr[mat-row] input[type="checkbox"]').first();
                await expect(firstCheckbox, 'テーブルの最初のレコードが存在すること').toBeVisible({ timeout: 60000 });
...
test.describe('レコード保存・値の永続化', () => {
    ...
    test.beforeAll(async ({ browser }) => {
        ...
        try {
            ...
            await ensureLoggedIn(page);
            tableId = await getAllTypeTableId(page);
            // __LOGIN_ERROR__ の場合は再ログインしてリトライ
            if (!tableId || tableId === '__LOGIN_ERROR__') {
                await login(page);
                tableId = await getAllTypeTableId(page);
            }
        ...
        console.log(`[レコード保存テスト] tableId=${tableId}, recordId=${recordId}`);
            ...
            console.error('[beforeAll] レコードID取得失敗:', e.message);
        }
        await page.close().catch(() => {});
        await context.close().catch(() => {});
    });
    ...
    async function goToEditPage(page) {
        ...
        await page.waitForSelector('[id^="field__"]', { timeout: 5000 });
    }
    ...
    test('SAVE-01: テキストフィールドの値を編集→保存→値が永続化されていること', async ({ page }) => {
        ...
        await expect(targetInput, 'テキストフィールドの入力欄が表示されること').toBeVisible({ timeout: 60000 });
    ...
    test('SAVE-02: テキストエリアの値を編集→保存→値が永続化されていること', async ({ page }) => {
        ...
        await expect(targetTextarea, 'テキストエリアが表示されること').toBeVisible({ timeout: 60000 });
    ...
    test('SAVE-03: 数値フィールドの値を編集→保存→値が永続化されていること', async ({ page }) => {
        ...
        await expect(numInput, '数値フィールドの入力欄が表示されること').toBeVisible({ timeout: 60000 });
...
test.describe('レコード操作 追加テスト', () => {
    ...
    test('RC01: レコード一覧', async ({ page }) => {
        await test.step('281: 一覧画面の文章(複数行)フィールドを編集モードで編集してもフリーズしないこと', async () => {
            const STEP_TIME = Date.now();
        ...
        await test.step('322: 数値項目(小数)で入力した値が編集画面で正しく表示されること', async () => {
            const STEP_TIME = Date.now();
    ...
    test('RC02: 一括編集', async ({ page }) => {
        await test.step('502: 子テーブルの複製ボタンが存在すること', async () => {
            const STEP_TIME = Date.now();
        ...
        await test.step('518: 子テーブルの一覧表示で項目値が表示されること', async () => {
            const STEP_TIME = Date.now();
        ...
        await test.step('180-2: 権限がないデータが含まれていると一括編集がされないこと', async () => {
            const STEP_TIME = Date.now();
    ...
    test('UC05: レコード一括操作', async ({ page }) => {
        await test.step('442: 一括削除後に全選択チェックが自動で外れること', async () => {
            const STEP_TIME = Date.now();
        ...
        await test.step('475: レコードのコピーボタン押下で確認ダイアログが表示されること', async () => {
            const STEP_TIME = Date.now();
    ...
    test('UC07: レコード全選択', async ({ page }) => {
        await test.step('526: 全選択時に全ページのデータが選択対象になりモーダルに件数表示されること', async () => {
            const STEP_TIME = Date.now();
    ...
    test('UC11: レコード一括操作', async ({ page }) => {
        await test.step('617: 全データ選択時の一括削除ポップアップに赤文字注意書きが表示されること', async () => {
            const STEP_TIME = Date.now();
    ...
    test('UC15: フィルタ一括削除', async ({ page }) => {
        await test.step('704: フィルタ適用中の全選択一括削除でフィルタ対象のレコードのみが削除されること', async () => {
            const STEP_TIME = Date.now();
    ...
    test('UC20: レコード編集', async ({ page }) => {
        await test.step('780: 編集保存で複数値フィールドのデータが消えないこと', async () => {
            const STEP_TIME = Date.now();
    ...
    test('UC06: 複数値の空検索と削除権限のみの削除', async ({ page }) => {
        await test.step('498: 複数値項目の空検索が正しく動作し削除権限のみのユーザーでも削除可能なこと', async () => {
            const STEP_TIME = Date.now();
    ...
    test('UC08: 一括削除の件数整合性', async ({ page }) => {
        await test.step('552: 一括削除モーダルに件数が正しく表示されること', async () => {
            const STEP_TIME = Date.now();
