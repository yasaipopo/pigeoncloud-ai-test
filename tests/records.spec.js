...
test.describe('レコード操作（一覧・作成・編集・削除・一括編集）', () => {
    ...
    test.beforeAll(async ({ browser }) => {
            ...
            await page.close();
            await context.close();
            console.log(`[records-1] 自己完結環境: ${BASE_URL}`);
        });
...
test.describe('レコード一括操作（チェックボックス選択・一括削除・一括編集）', () => {
    ...
    test.beforeAll(async ({ browser }) => {
        ...
        console.log(`[records-batch] 自己完結環境: ${BASE_URL}`);
    });
...
test.describe('編集ロック', () => {
    ...
    test.beforeAll(async ({ browser }) => {
        ...
        await page.close().catch(() => {});
        await context.close().catch(() => {});
        console.log(`[records-lock] 自己完結環境: ${BASE_URL}`);
    });
...
test.describe('レコード保存・値の永続化', () => {
    ...
    test.beforeAll(async ({ browser }) => {
        ...
        await page.close().catch(() => {});
        await context.close().catch(() => {});
        console.log(`[records-save] 自己完結環境: ${BASE_URL}`);
    });
...
test.describe('レコード操作 追加テスト', () => {
    ...
    test.beforeAll(async ({ browser }) => {
            ...
            await page.close();
            await context.close();
            console.log(`[records-extra] 自己完結環境: ${BASE_URL}`);
        });
