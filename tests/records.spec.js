...
test.describe('レコード操作（一覧・作成・編集・削除・一括編集）', () => {
    ...
    test('143-01: レコード一覧にコメントアイコンが表示されること', async ({ page }) => {
        ...
        // チェックボックスのvalue属性からrecordIdを取得（行の /record/ リンクは存在しない場合がある）
        const firstCheckbox = page.locator('tr[mat-row] input[type="checkbox"]').first();
        const commentRecordId = await firstCheckbox.getAttribute('value', { timeout: 10000 }).catch(() => null);
    ...
    test('35-1: 参照中のテーブルを削除しようとするとエラーが表示されること', async ({ page }) => {
        ...
        await page.waitForSelector('.modal.settingModal.show.relation_table', { timeout: 15000 }).catch(() => {});
        await page.waitForTimeout(2000);

        // 設定モーダルが開いていること（関連レコード一覧専用モーダル = .relation_table クラス）
        await expect(page.locator('.modal.settingModal.show.relation_table'), '関連レコード一覧設定モーダルが開くこと').toBeVisible({ timeout: 10000 });

        // 項目名を入力
        await page.evaluate(() => {
            const modal = document.querySelector('.modal.settingModal.show.relation_table');
            if (!modal) return;
    ...
    test('52-1: 関連レコード一覧の項目名未入力でエラーが発生すること', async ({ page }) => {
        ...
        await page.waitForTimeout(2000);
        // ローディング完了を待つ
        await page.waitForSelector('.modal.settingModal.show.relation_table .loading', { state: 'hidden', timeout: 5000 }).catch(() => {});

        // 設定モーダルが開いていること（項目名は空のまま = 未入力状態）
        await expect(page.locator('.modal.settingModal.show.relation_table'), '設定モーダルが開くこと').toBeVisible({ timeout: 10000 });
