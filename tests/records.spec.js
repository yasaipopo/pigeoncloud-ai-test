...
test.describe('レコード並び替え', () => {
    ...
    test('ORD-01: レコード一覧に並び替えボタンまたはドラッグハンドルが存在すること', async ({ page }) => {
        ...
        if (sortBtnCount === 0 && dragHandleCount === 0) {
            ...
            test.skip(true, '並び替えUIが存在しないためスキップ');
            return;
        }
    ...
    test('ORD-02: 並び替えモードに切り替えられること', async ({ page }) => {
        ...
        const sortBtnCount = await sortBtn.count();
        if (sortBtnCount === 0) {
            test.skip(true, '並び順ボタンが見つからないためスキップ');
            return;
        }
...
test.describe('編集ロック', () => {
    ...
    test('LOCK-01: レコード編集開始でロック状態になること', async ({ page }) => {
        ...
        if (recordId) {
            ...
            if (await firstRowLink.count() === 0) {
                test.skip(true, 'レコードが見つからないためスキップ');
                return;
            }
    ...
    test('LOCK-02: 編集キャンセルでロックが解除されること', async ({ page }) => {
        ...
        if (await cancelBtn.count() === 0) {
            test.skip(true, 'キャンセルボタンが見つからないためスキップ');
            return;
        }
    ...
    test('LOCK-03: 編集保存でロックが解除されること', async ({ page }) => {
        ...
        if (await saveBtn.count() === 0) {
            // 保存ボタンが見つからない場合はキャンセルして終了
            await page.locator('button:has-text("キャンセル")').first().click().catch(() => {});
