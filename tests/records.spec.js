...
test.describe('レコード操作（一覧・作成・編集・削除・一括編集）', () => {
    ...
    test('143-01: レコード一覧にコメントアイコンが表示されること', async ({ page }) => {
        ...
        // テーブルヘッダー行が存在すること
        await expect(page.locator('tr[mat-header-row]')).toBeVisible({ timeout: 30000 });
        ...
        await expect(page.locator('tr[mat-row] input[type="checkbox"]').first()).toBeVisible({ timeout: 30000 });
        ...
        await expect(commentIcon, 'コメント投稿後にコメントアイコンが一覧に表示されること').toBeVisible({ timeout: 30000 });
    ...
    test('180-1: 一括編集メニューが表示され、一括編集を実行できること', async ({ page }) => {
        ...
        await expect(hamburgerBtn).toBeVisible({ timeout: 30000 });
    ...
    test('180-3: 一括編集モーダルにロック中データの扱いが説明されていること', async ({ page }) => {
        ...
        await expect(dataRows.first(), 'テーブルにデータが存在すること').toBeVisible({ timeout: 30000 });
        ...
        // 一括編集モーダルが開くことを確認
        const modal = page.locator('.modal.show').first();
        await expect(modal, '一括編集モーダルが表示されること').toBeVisible({ timeout: 30000 });
    ...
    test('180-4: フィルタ適用中にのみ一括編集がかかること', async ({ page }) => {
        ...
        if (filteredRows >= totalRows) {
            ...
            await expect(hamburgerBtn2, 'ハンバーガーメニューが存在すること').toBeVisible({ timeout: 30000 });
        ...
        // ------------------------------------------------------------------
        const hamburgerBtn = page.locator('button:has(.fa-bars)').first();
        await expect(hamburgerBtn, 'ハンバーガーメニューが存在すること').toBeVisible({ timeout: 30000 });
        await hamburgerBtn.click();
        await waitForAngular(page);
    ...
    test('52-1: 関連レコード一覧の項目名未入力でエラーが発生すること', async ({ page }) => {
        ...
        await expect(addFieldBtn, '「項目を追加する」ボタンが存在すること').toBeVisible({ timeout: 30000 });
    ...
    test('52-2: 関連レコード一覧の対象テーブル未入力でエラーが発生すること', async ({ page }) => {
        ...
        await expect(addFieldBtn52_2, '「項目を追加する」ボタンが存在すること').toBeVisible({ timeout: 30000 });
...
test.describe('レコード一括操作（チェックボックス選択・一括削除・一括編集）', () => {
    ...
    test('一括-2: 全選択チェックボックスをクリックすると一括操作ボタンが表示されること', async ({ page }) => {
        ...
        const dataRows = page.locator('tr[mat-row]');
        await expect(dataRows.first(), 'データ行が表示されること（beforeAllで作成済み）').toBeVisible({ timeout: 30000 });
    ...
    test('LOCK-01: レコード編集開始でロック状態になること', async ({ page }) => {
        ...
        if (recordId) {
            ...
            if (detailBtnVisible) {
                ...
                const firstCheckbox = page.locator('tr[mat-row] input[type="checkbox"]').first();
                await expect(firstCheckbox, 'テーブルの最初のレコードが存在すること').toBeVisible({ timeout: 30000 });
...
test.describe('レコード保存・値の永続化', () => {
    ...
    test('SAVE-01: テキストフィールドの値を編集→保存→値が永続化されていること', async ({ page }) => {
        ...
        await expect(targetInput, 'テキストフィールドの入力欄が表示されること').toBeVisible({ timeout: 30000 });
    ...
    test('SAVE-02: テキストエリアの値を編集→保存→値が永続化されていること', async ({ page }) => {
        ...
        await expect(targetTextarea, 'テキストエリアが表示されること').toBeVisible({ timeout: 30000 });
    ...
    test('SAVE-03: 数値フィールドの値を編集→保存→値が永続化されていること', async ({ page }) => {
        ...
        await expect(numInput, '数値フィールドの入力欄が表示されること').toBeVisible({ timeout: 30000 });
