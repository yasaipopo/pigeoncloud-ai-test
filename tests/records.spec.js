...
// ファイルレベル: 1つの自己完結環境を全describeで共有
test.beforeAll(async ({ browser }) => {
    ...
    console.log(`[records] 自己完結環境: ${BASE_URL}, tableId: ${tableId}`);
});
...
test.describe('レコード操作（一覧・作成・編集・削除・一括編集）', () => {
    ...
    test.beforeEach(async ({ page }) => {
        ...
        await page.context().clearCookies();
            await page.goto(BASE_URL + '/admin/login', { waitUntil: 'domcontentloaded', timeout: 15000 }).catch(() => {});
        // BASE_URLドメインのlocalStorage/sessionStorageをクリア（storageStateのキャッシュ除去）
        await page.evaluate(() => { try { localStorage.clear(); sessionStorage.clear(); } catch {} });
        // storageStateのキャッシュが原因でダッシュボードにリダイレクトされた場合、再度ログインページへ
        if (!page.url().includes('/login')) {
            await page.goto(BASE_URL + '/admin/login', { waitUntil: 'domcontentloaded', timeout: 15000 }).catch(() => {});
        }
            if (page.url().includes('/login')) {
    ...
    test('RC01: レコード一覧', async ({ page }) => {
        const _testStart = Date.now();
        await test.step('rec-010: レコード一覧にコメントアイコンが表示されること', async () => {
            const STEP_TIME = Date.now();

            expect(tableId).not.toBeNull();

            // レコード一覧に移動（navigateToTableでSPA nav経由、route guard回避）
            ...
            await expect(page.locator('table.pc-list-view, table[class*="list"]')).toBeVisible();
            ...
            await expect(commentIcon, 'コメント投稿後にコメントアイコンが一覧に表示されること').toBeVisible();
            ...
            await stepScreenshot(page, 'records', 'RC01', 'rec-010-s3', _testStart);

        });
        await test.step('rec-020: チェックボックスをクリックすると一括削除ボタンが表示されること', async () => {
            const STEP_TIME = Date.now();
            ...
            await stepScreenshot(page, 'records', 'RC01', 'rec-020-s3', _testStart);

        });
        await test.step('rec-070: レコード一覧のスクロールバーを問題なく操作できること', async () => {
            const STEP_TIME = Date.now();
            ...
            await stepScreenshot(page, 'records', 'RC01', 'rec-070-s3', _testStart);

        });
    });

    test('RC02: 一括編集', async ({ page }) => {
        const _testStart = Date.now();
        await test.step('rec-030: 一括編集メニューが表示され、一括編集を実行できること', async () => {
            const STEP_TIME = Date.now();
            ...
            await stepScreenshot(page, 'records', 'RC02', 'rec-030-s3', _testStart);

        });
        await test.step('rec-050: 一括編集モーダルにロック中データの扱いが説明されていること', async () => {
            const STEP_TIME = Date.now();
            ...
            await expect(dataRows.first(), 'テーブルにデータが存在すること').toBeVisible();
            ...
            await expect(bulkEditItem, '「一括編集」メニューが表示されること').toBeVisible();
            await bulkEditItem.click();
            await waitForAngular(page);
            ...
            await expect(lockWarningText, '一括編集モーダルにロック中データは更新されない旨の説明があること').toBeVisible();
            ...
            await stepScreenshot(page, 'records', 'RC02', 'rec-050-s3', _testStart);

        });
        await test.step('rec-060: フィルタ適用中にのみ一括編集がかかること', async () => {
            const STEP_TIME = Date.now();
            ...
            if (filteredRows >= totalRows) {
                ...
                await expect(hamburgerBtn2, 'ハンバーガーメニューが存在すること').toBeVisible();
                await hamburgerBtn2.click();
                await waitForAngular(page);
                const bulkEditItem2 = page.locator('.dropdown-menu.show .dropdown-item:has-text("一括編集")').first();
                await expect(bulkEditItem2, '「一括編集」メニューが表示されること').toBeVisible();
            ...
            const bulkEditItem = page.locator('.dropdown-menu.show .dropdown-item:has-text("一括編集")').first();
            await expect(bulkEditItem, 'フィルタ適用中でも「一括編集」メニューが表示されること').toBeVisible();
            await bulkEditItem.click();
            await waitForAngular(page);
            ...
            if (addItemVisible) {
                ...
                await expect(executeBtn, '一括編集実行ボタンが存在すること').toBeVisible();
            }
            ...
            await stepScreenshot(page, 'records', 'RC02', 'rec-060-s3', _testStart);

        });
    });

    test('RC03: 関連レコード一覧', async ({ page }) => {
        const _testStart = Date.now();
        await test.step('rec-080: 参照中のテーブルを削除しようとするとエラーが表示されること', async () => {
            const STEP_TIME = Date.now();
            ...
            await stepScreenshot(page, 'records', 'RC03', 'rec-080-s3', _testStart);

        });
        await test.step('rec-090: 関連レコード一覧の項目名未入力でエラーが発生すること', async () => {
            const STEP_TIME = Date.now();

            test.setTimeout(75000); // モーダル操作に時間がかかるため延長
            ...
            await expect(addFieldBtn, '「項目を追加する」ボタンが存在すること').toBeVisible();
            await addFieldBtn.click({ force: true });
            await waitForAngular(page);

            // settingModalが開いていることを確認（型選択フェーズ）
            await page.waitForSelector('.modal.settingModal.show', { timeout: 5000 }).catch(() => {});
            ...
            await expect(errorMsg52_1, '項目名未入力でエラーメッセージが表示されること').toBeVisible();

            // スクリーンショット保存
            const reportsDir = process.env.REPORTS_DIR || 'reports/agent-1';
            await page.screenshot({ path: `${reportsDir}/screenshots/52-1-related-record-name-error.png`, fullPage: true });
            await stepScreenshot(page, 'records', 'RC03', 'rec-090-s3', _testStart);

        });
        await test.step('rec-100: 関連レコード一覧の対象テーブル未入力でエラーが発生すること', async () => {
            const STEP_TIME = Date.now();
            ...
            await expect(addFieldBtn52_2, '「項目を追加する」ボタンが存在すること').toBeVisible();
            ...
            await expect(errorMsg52_2, '対象テーブル未入力でエラーメッセージが表示されること').toBeVisible();
            ...
            await stepScreenshot(page, 'records', 'RC03', 'rec-100-s3', _testStart);
...
test.describe('レコード一括操作（チェックボックス選択・一括削除・一括編集）', () => {
    ...
    test.beforeAll(async ({ browser }) => {
        ...
        try {
            ...
            console.error('[beforeAll] 一括操作セットアップ失敗:', e.message);
        }
        await page.close().catch(() => {});
        await context.close().catch(() => {});
        console.log(`[records-batch] データ作成完了`);
    });

    test.beforeEach(async ({ page }) => {
        // 古い環境のcookieをクリアして新環境にログイン
        await page.context().clearCookies();
        // goto の後に localStorage.clear()（BASE_URLドメインのキャッシュを確実にクリア）
        await page.goto(BASE_URL + '/admin/login', { waitUntil: 'domcontentloaded', timeout: 15000 }).catch(() => {});
        await page.evaluate(() => { try { localStorage.clear(); sessionStorage.clear(); } catch {} });
        // storageStateキャッシュでダッシュボードにリダイレクトされた場合、再度ログインページへ
        if (!page.url().includes('/login')) {
            await page.goto(BASE_URL + '/admin/login', { waitUntil: 'domcontentloaded', timeout: 15000 }).catch(() => {});
        }
        if (page.url().includes('/login')) {
    ...
    test('一括-1: レコード一覧にチェックボックスと全選択UIが存在すること', async ({ page }) => {
        ...
        const dataRows = page.locator('tr[mat-row]');
        await expect(dataRows.first(), 'データ行が表示されること（beforeAllで10件作成済み）').toBeVisible();
        ...
        await stepScreenshot(page, 'records', 'RC02', 'rec-batch1-s3', _testStart);
    });
    ...
    test('一括-2: 全選択チェックボックスをクリックすると一括操作ボタンが表示されること', async ({ page }) => {
        ...
        const dataRows = page.locator('tr[mat-row]');
        await expect(dataRows.first(), 'データ行が表示されること（beforeAllで作成済み）').toBeVisible();
        ...
        await stepScreenshot(page, 'records', 'RC02', 'rec-batch2-s3', _testStart);
    });
    ...
    test('一括-3: 1件選択して一括削除を実行すると件数が減ること', async ({ page }) => {
        ...
        // 1行目のチェックボックスをクリック
        const firstCheckbox = page.locator('tr[mat-row] input[type="checkbox"]').first();
        await expect(firstCheckbox.first()).toBeVisible();
        ...
        await stepScreenshot(page, 'records', 'RC02', 'rec-batch3-s3', _testStart);
    });
    ...
    test('一括-4: 複数選択後に一括編集メニューが表示されること（UIが存在する場合）', async ({ page }) => {
        ...
        await expect(bulkEditItem, '「一括編集」メニューアイテムが表示されること').toBeVisible();
        ...
        await stepScreenshot(page, 'records', 'RC02', 'rec-batch4-s3', _testStart);
    });
});
...
test.describe('編集ロック', () => {
    ...
    test.beforeAll(async ({ browser }) => {
        ...
        try {
            ...
            // data-record-id属性からrecordIdを取得
            await page.goto(BASE_URL + `/admin/dataset__${tableId}`, { waitUntil: "domcontentloaded", timeout: 15000 }).catch(() => {});
            await waitForAngular(page);
            await page.waitForSelector('tr[mat-row]', { timeout: 5000 }).catch(() => {});
            const firstRow = page.locator('tr[mat-row]').first();
            const dataRecordId = await firstRow.getAttribute('data-record-id', { timeout: 5000 }).catch(() => null);
            if (dataRecordId) {
            ...
            console.error('[beforeAll] 編集ロックセットアップ失敗:', e.message);
        }
        await page.close().catch(() => {});
        await context.close().catch(() => {});
        console.log(`[records-lock] データ作成完了`);
    });
    ...
    test('LOCK-01: レコード編集開始でロック状態になること', async ({ page }) => {
        ...
        if (recordId) {
            ...
            // navigateToTableでSPA nav経由でテーブル一覧に遷移（route guard回避）
            await navigateToTable(page, BASE_URL, tableId, { maxRetries: 3, retryWait: 5000 });
            await waitForAngular(page);
            ...
            if (detailBtnVisible) {
                ...
                const firstCheckbox = page.locator('tr[mat-row] input[type="checkbox"]').first();
                await expect(firstCheckbox, 'テーブルの最初のレコードが存在すること').toBeVisible();
        ...
        await expect(page.locator('.navbar')).toBeVisible({ timeout: 15000 });
        await stepScreenshot(page, 'records', 'RC03', 'rec-lock1-s3', _testStart);
    });
    ...
    test('LOCK-02: 編集キャンセルでロックが解除されること', async ({ page }) => {
        ...
        await expect(editBtnAfter, 'キャンセル後に編集ボタンが再度表示されること（ロックが解除されたこと）').toBeVisible();

        console.log('LOCK-02: 編集キャンセルでロック解除確認OK');
        await stepScreenshot(page, 'records', 'RC03', 'rec-lock2-s3', _testStart);
    });
    ...
    test('LOCK-03: 編集保存でロックが解除されること', async ({ page }) => {
        ...
        await expect(editBtnAfterSave, '保存後に編集ボタンが再表示されること（ロックが解除されたこと）').toBeVisible();

        console.log('LOCK-03: 編集保存でロック解除確認OK');
        await stepScreenshot(page, 'records', 'RC03', 'rec-lock3-s3', _testStart);
    });
});
...
test.describe('レコード保存・値の永続化', () => {
    ...
    test.beforeAll(async ({ browser }) => {
        ...
        try {
            ...
            console.error('[beforeAll] レコード保存セットアップ失敗:', e.message);
        }
        await page.close().catch(() => {});
        await context.close().catch(() => {});
        console.log(`[records-save] データ作成完了`);
    });
    ...
    async function clickSaveButton(page) {
        ...
        await expect(saveBtn, '更新ボタンが表示されること').toBeVisible();
        await saveBtn.click();
        await page.waitForTimeout(1000);
        // 確認ダイアログが出る場合があるので対応
    ...
    test('SAVE-01: テキストフィールドの値を編集→保存→値が永続化されていること', async ({ page }) => {
        ...
        await stepScreenshot(page, 'records', 'RC03', 'rec-save1-s3', _testStart);
    });
    ...
    test('SAVE-02: テキストエリアの値を編集→保存→値が永続化されていること', async ({ page }) => {
        ...
        await stepScreenshot(page, 'records', 'RC03', 'rec-save2-s3', _testStart);
    });
    ...
    test('SAVE-03: 数値フィールドの値を編集→保存→値が永続化されていること', async ({ page }) => {
        ...
        await stepScreenshot(page, 'records', 'RC03', 'rec-save3-s3', _testStart);
    });
    ...
    test('SAVE-04: 複数フィールドを同時に編集→保存→全値が永続化されていること', async ({ page }) => {
        ...
        await stepScreenshot(page, 'records', 'RC03', 'rec-save4-s3', _testStart);
    });
});
...
test.describe('レコード操作 追加テスト', () => {
    ...
    test.beforeAll(async ({ browser }) => {
            ...
            await page.close();
            await context.close();
            console.log(`[records-extra] データ作成完了`);
        });
    ...
    test('RC01: レコード一覧', async ({ page }) => {
        const _testStart = Date.now();
        await test.step('rec-110: 一覧画面の文章(複数行)フィールドを編集モードで編集してもフリーズしないこと', async () => {
            const STEP_TIME = Date.now();
            ...
            await stepScreenshot(page, 'records', 'RC01', 'rec-110-s3', _testStart);

        });
        await test.step('rec-120: 数値項目(小数)で入力した値が編集画面で正しく表示されること', async () => {
            const STEP_TIME = Date.now();
            ...
            // ページエラーなし
            await expect(page.locator('.navbar')).toBeVisible({ timeout: 15000 });
            await stepScreenshot(page, 'records', 'RC01', 'rec-120-s3', _testStart);
    ...
    test('RC02: 一括編集', async ({ page }) => {
        const _testStart = Date.now();
        await test.step('rec-130: 子テーブルの複製ボタンが存在すること', async () => {
            const STEP_TIME = Date.now();
            ...
            // ページが正常であること
            await expect(page.locator('.navbar')).toBeVisible({ timeout: 15000 });
            await stepScreenshot(page, 'records', 'RC02', 'rec-130-s3', _testStart);

        });
        await test.step('rec-140: 子テーブルの一覧表示で項目値が表示されること', async () => {
            const STEP_TIME = Date.now();
            ...
            await stepScreenshot(page, 'records', 'RC02', 'rec-140-s3', _testStart);

        });
        await test.step('rec-040: 権限がないデータが含まれていると一括編集がされないこと', async () => {
            const STEP_TIME = Date.now();
            ...
            expect(bodyText).not.toContain('Internal Server Error');
            await expect(page.locator('.navbar')).toBeVisible({ timeout: 15000 });
            await stepScreenshot(page, 'records', 'RC02', 'rec-040-s3', _testStart);

        });
    });

    test('UC05: レコード一括操作', async ({ page }) => {
        const _testStart = Date.now();
        await test.step('rec-150: 一括削除後に全選択チェックが自動で外れること', async () => {
            const STEP_TIME = Date.now();
            ...
            // ページが正常であること
            await expect(page.locator('.navbar')).toBeVisible({ timeout: 15000 });
            await stepScreenshot(page, 'records', 'UC05', 'rec-150-s3', _testStart);

        });
        await test.step('rec-160: レコードのコピーボタン押下で確認ダイアログが表示されること', async () => {
            const STEP_TIME = Date.now();
            ...
            await expect(page.locator('.navbar')).toBeVisible({ timeout: 15000 });
            await stepScreenshot(page, 'records', 'UC05', 'rec-160-s3', _testStart);

        });
    });

    test('UC07: レコード全選択', async ({ page }) => {
        const _testStart = Date.now();
        await test.step('rec-170: 全選択時に全ページのデータが選択対象になりモーダルに件数表示されること', async () => {
            const STEP_TIME = Date.now();
            ...
            await expect(page.locator('.navbar')).toBeVisible({ timeout: 15000 });
            await stepScreenshot(page, 'records', 'UC07', 'rec-170-s3', _testStart);

        });
    });

    test('UC11: レコード一括操作', async ({ page }) => {
        const _testStart = Date.now();
        await test.step('rec-180: 全データ選択時の一括削除ポップアップに赤文字注意書きが表示されること', async () => {
            const STEP_TIME = Date.now();
            ...
            await expect(page.locator('.navbar')).toBeVisible({ timeout: 15000 });
            await stepScreenshot(page, 'records', 'UC11', 'rec-180-s3', _testStart);

        });
    });

    test('UC15: フィルタ一括削除', async ({ page }) => {
        const _testStart = Date.now();
        await test.step('rec-190: フィルタ適用中の全選択一括削除でフィルタ対象のレコードのみが削除されること', async () => {
            const STEP_TIME = Date.now();
            ...
            expect(bodyText).not.toContain('Internal Server Error');
            await expect(page.locator('.navbar')).toBeVisible({ timeout: 15000 });
            await stepScreenshot(page, 'records', 'UC15', 'rec-190-s3', _testStart);

        });
    });

    test('UC20: レコード編集', async ({ page }) => {
        const _testStart = Date.now();
        await test.step('rec-200: 編集保存で複数値フィールドのデータが消えないこと', async () => {
            const STEP_TIME = Date.now();
            ...
            if (editLinkCount > 0) {
                ...
                await waitForAngular(page);
                await page.waitForSelector('[id^="field__"]', { timeout: 5000 }).catch(() => {});
            ...
            // ページが正常であること
            await expect(page.locator('.navbar')).toBeVisible({ timeout: 15000 });
            await stepScreenshot(page, 'records', 'UC20', 'rec-200-s3', _testStart);

        });
    });

    test('UC06: 複数値の空検索と削除権限のみの削除', async ({ page }) => {
        const _testStart = Date.now();
        await test.step('rec-210: 複数値項目の空検索が正しく動作し削除権限のみのユーザーでも削除可能なこと', async () => {
            const STEP_TIME = Date.now();
            ...
            expect(bodyText).not.toContain('Internal Server Error');
            await expect(page.locator('.navbar')).toBeVisible({ timeout: 15000 });
            await stepScreenshot(page, 'records', 'UC06', 'rec-210-s3', _testStart);

        });
    });

    test('UC08: 一括削除の件数整合性', async ({ page }) => {
        const _testStart = Date.now();
        await test.step('rec-220: 一括削除モーダルに件数が正しく表示されること', async () => {
            const STEP_TIME = Date.now();
            ...
            await expect(page.locator('.navbar')).toBeVisible({ timeout: 15000 });
            await stepScreenshot(page, 'records', 'UC08', 'rec-220-s3', _testStart);
