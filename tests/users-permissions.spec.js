...
test.describe('ユーザー管理（作成・編集・削除・有効/無効）', () => {
    ...
    test.beforeAll(async ({ browser }) => {
            test.setTimeout(120000); // 上限解除リトライに対応するため延長
    ...
    test.beforeEach(async ({ page }) => {
            test.setTimeout(120000); // storageState利用 + サーバー応答遅延に対応
...
test.describe('権限設定・グループ権限', () => {
    ...
    test.beforeEach(async ({ page }) => {
            test.setTimeout(120000); // サーバー応答遅延に対応
            await ensureLoggedIn(page);
            await closeTemplateModal(page);
        });
    ...
    test('UP05: ユーザー管理', async ({ page }) => {
        ...
        await test.step('3-7: マスターユーザーで電話番号を変更できること', async () => {
            ...
            expect(page.url()).toContain('/admin/admin/edit/');
            // フォーム要素が存在すること
            await expect(page.locator('form, input, button.btn-ladda').first()).toBeVisible();
...
test.describe('権限設定の動作確認（テストユーザー作成・権限付与・ログイン確認）', () => {
    ...
    test('61-3: デバッグAPIで作成したテストユーザーでログインできること', async ({ browser }) => {
            ...
            try {
                ...
                await testPage.waitForSelector('#id', { timeout: 5000 });
                ...
                await testPage.waitForSelector('.navbar', { timeout: 5000 }).catch(() => {});
    ...
    test('290-1: 初回ログイン時にパスワード変更画面が表示されること', async ({ browser }) => {
            ...
            if (user && user.result === 'success' && user.email) {
                ...
                try {
                    ...
                    await userPage.waitForSelector('#id', { timeout: 5000 }).catch(() => {});
