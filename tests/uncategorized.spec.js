...
async function navigateToFieldEditPage(page, tableId) {
    ...
    await page.waitForSelector('.cdk-drag, .field-drag, .cdk-drop-list, .toggle-drag-field-list', { timeout: 60000 }).catch(() => {});
    const bodyText = await page.innerText('body');
    expect(bodyText).not.toContain('Internal Server Error');
    return bodyText;
}
...
test.describe('文字列表示設定（145系）', () => {
    ...
    test('145-01: テキストフィールドの一覧表示文字数設定が正しく動作すること', async ({ page }) => {
        ...
        await expect(page.locator('table thead th, [role="columnheader"]').first()).toBeVisible({ timeout: 30000 });
    ...
    test('145-01(B): 文字列に一覧表示文字数と全文字表示を設定した場合に折り返して全表示されること', async ({ page }) => {
        ...
        await expect(page.locator('table, [role=\"columnheader\"]').first()).toBeVisible({ timeout: 30000 });
        // テーブルが表示されること
        // テーブル構造が正常であること（データがない場合もあるため行数チェックは省略）
        const thCount2 = await page.locator('table thead th, [role="columnheader"]').count();
        expect(thCount2).toBeGreaterThanOrEqual(0);
...
test.describe('表示条件設定（250系）', () => {
    ...
    // -------------------------------------------------------------------------
    test('250: 項目削除時に確認モーダルが表示されること', async ({ page }) => {
...
test.describe('権限設定（262系）', () => {
    ...
    // -------------------------------------------------------------------------
    test('262: テーブル権限設定と項目権限設定のUIがテーブル設定ページに存在すること', async ({ page }) => {
        test.setTimeout(120000);
        const reportsDir = process.env.REPORTS_DIR || 'reports/agent-1';
        expect(tableId, 'テーブルIDが取得できること（beforeAllで作成済み）').toBeTruthy();
        // テーブル設定ページへ移動
...
test.describe('自動採番（273系）', () => {
    ...
    // -------------------------------------------------------------------------
    test('273: 自動採番フィールドの設定モーダルが開き設定欄が存在すること', async ({ page }) => {
        test.setTimeout(120000);
        const reportsDir = process.env.REPORTS_DIR || 'reports/agent-1';
        expect(tableId, 'テーブルIDが取得できること（beforeAllで作成済み）').toBeTruthy();
        // フィールド設定ページへ移動
        ...
        if (modalCount > 0) {
            await expect(settingModal, '自動採番フィールドの設定モーダルが開くこと').toBeVisible({ timeout: 30000 });
            ...
            if (await settingPanel.count() > 0) {
                await expect(settingPanel, '自動採番フィールドの設定パネルが開くこと').toBeVisible({ timeout: 30000 });
...
test.describe('リッチテキスト（274系）', () => {
    ...
    test('274: リッチテキスト項目で設定モーダルが開くこと', async ({ page }) => {
        ...
        const modalCount = await settingModal.count();
        if (modalCount > 0) {
            await expect(settingModal, 'リッチテキストフィールドの設定モーダルが開くこと').toBeVisible({ timeout: 30000 });
...
test.describe('日時フォーマット（275系）', () => {
    ...
    // -------------------------------------------------------------------------
    test('275: 日時フィールドの表示フォーマット設定モーダルが開くこと', async ({ page }) => {
        test.setTimeout(120000);
        const reportsDir = process.env.REPORTS_DIR || 'reports/agent-1';
        expect(tableId, 'テーブルIDが取得できること（beforeAllで作成済み）').toBeTruthy();
        // フィールド設定ページへ移動
        ...
        const modalCount = await settingModal.count();
        if (modalCount > 0) {
            await expect(settingModal, '日時フィールドの設定モーダルが開くこと').toBeVisible({ timeout: 30000 });
...
test.describe('循環参照エラー（291系）', () => {
    ...
    // -------------------------------------------------------------------------
    test('291: 他テーブル参照フィールドの追加モーダルが利用できること', async ({ page }) => {
        test.setTimeout(120000);
        const reportsDir = process.env.REPORTS_DIR || 'reports/agent-1';
        expect(tableId, 'テーブルIDが取得できること（beforeAllで作成済み）').toBeTruthy();
        // フィールド設定ページへ移動
        ...
        await expect(addModal, 'フィールド追加モーダルが開くこと').toBeVisible({ timeout: 30000 });
...
test.describe('テーブル削除ロック（349系）', () => {
    ...
    // 349: テーブルの削除ロック機能
    // -------------------------------------------------------------------------
    test('349: テーブル設定ページで削除ロック機能のUIが存在すること', async ({ page }) => {
        test.setTimeout(120000);
        const reportsDir = process.env.REPORTS_DIR || 'reports/agent-1';
        expect(tableId, 'テーブルIDが取得できること（beforeAllで作成済み）').toBeTruthy();
        // テーブル設定ページへ移動
...
test.describe('一覧編集モード（324系）', () => {
    ...
    test('324: 一覧編集モードで編集後に詳細画面で値が消えないこと', async ({ page }) => {
        ...
        if (hasEditModeBtn === 0) {
            ...
            await expect(page.locator('.navbar, header.app-header').first()).toBeVisible({ timeout: 30000 });
            // ナビバーが表示されること
        } else {
...
test.describe('追加実装テスト（282-593系）', () => {
    ...
    // -------------------------------------------------------------------------
    test('282: レコード編集画面で値を入力・保存し、詳細画面で正しい値が表示されること', async ({ page }) => {
    ...
    // 283: 権限設定・編集不可項目
    // -------------------------------------------------------------------------
    test('283: 「高度な設定」項目権限設定を追加しても編集不可項目の設定が外れないこと', async ({ page }) => {
        expect(tableId, 'テーブルIDが取得できること').toBeTruthy();
        // テーブル設定 > 権限設定を開く
    ...
    // -------------------------------------------------------------------------
    test('474: ワークフロー否認/取り下げ後にデータ編集してもWFが正しく切り替わること', async ({ page }) => {
        expect(tableId, 'テーブルIDが取得できること').toBeTruthy();
        // テーブル設定 > ワークフロー設定を確認
        await page.goto(BASE_URL + `/admin/dataset__${tableId}/setting`);
        await waitForAngular(page);
    ...
    // -------------------------------------------------------------------------
    test('475: レコードコピーボタン押下時に確認ダイアログが表示されOKでコピーが実行されること', async ({ page }) => {
        expect(tableId, 'テーブルIDが取得できること').toBeTruthy();
        await page.goto(BASE_URL + `/admin/dataset__${tableId}`);
        await waitForAngular(page);
        await page.waitForTimeout(1500);
    ...
    // -------------------------------------------------------------------------
    test('501: レコードの編集・保存が正しく動作すること', async ({ page }) => {
        expect(tableId, 'テーブルIDが取得できること').toBeTruthy();
        await navigateToDatasetPage(page, tableId);
    ...
    // -------------------------------------------------------------------------
    test('503: グループに所属するテーブル名がサイドメニューで正しく表示されること', async ({ page }) => {
        expect(tableId, 'テーブルIDが取得できること').toBeTruthy();
        await page.goto(BASE_URL + '/admin/dashboard', { waitUntil: 'domcontentloaded', timeout: 60000 }).catch(() => {});
        // ログインリダイレクト対策
        if (page.url().includes('/admin/login')) {
        ...
        await page.waitForSelector('.sidebar a, .app-sidebar a, aside a, .nav-item a, .sidebar-nav a', { timeout: 30000 }).catch(() => {});
    ...
    // -------------------------------------------------------------------------
    test('504: テーブル設定変更後にレコード一覧画面がエラーなく表示されること', async ({ page }) => {
        expect(tableId, 'テーブルIDが取得できること').toBeTruthy();
        // テーブル設定画面を開く
        await checkPage(page, `/admin/dataset__${tableId}/setting`);
    ...
    // -------------------------------------------------------------------------
    test('508: ルックアップが他テーブル参照でも関連レコード一覧の表示条件が正常に動作すること', async ({ page }) => {
        expect(tableId, 'テーブルIDが取得できること').toBeTruthy();
    ...
    // -------------------------------------------------------------------------
    test('509: 数値が100000000000000以上のとき桁区切りが正しく表示されること', async ({ page }) => {
        expect(tableId, 'テーブルIDが取得できること').toBeTruthy();
        await navigateToDatasetPage(page, tableId);
    ...
    // -------------------------------------------------------------------------
    test('519: テーブル設定の変更が正しく保存・反映されること', async ({ page }) => {
        expect(tableId, 'テーブルIDが取得できること').toBeTruthy();
        await checkPage(page, `/admin/dataset__${tableId}/setting`);
        const bodyText = await page.innerText('body');
        expect(bodyText).not.toContain('Internal Server Error');
    ...
    // -------------------------------------------------------------------------
    test('520: ワークフローのAND/OR並行承認で2人目以降の役職選択が正しく保存・表示されること', async ({ page }) => {
        expect(tableId, 'テーブルIDが取得できること').toBeTruthy();
        await page.goto(BASE_URL + `/admin/dataset__${tableId}/setting`);
        await waitForAngular(page);
        await page.waitForTimeout(1500);
    ...
    // -------------------------------------------------------------------------
    test('521: 組織未選択時にバリデーションエラーが表示され正しく選択後に再申請でエラーが出ないこと', async ({ page }) => {
        expect(tableId, 'テーブルIDが取得できること').toBeTruthy();
        // テーブル一覧を開く
        await page.goto(BASE_URL + `/admin/dataset__${tableId}`);
        await waitForAngular(page);
    ...
    // -------------------------------------------------------------------------
    test('525: フィールドの追加オプション設定が正しく動作すること', async ({ page }) => {
        expect(tableId, 'テーブルIDが取得できること').toBeTruthy();
        await checkPage(page, `/admin/dataset__${tableId}/setting`);
    ...
    // -------------------------------------------------------------------------
    test('526: 全選択時に全ページのデータが対象となり一括削除・一括編集モーダルに件数が正しく表示されること', async ({ page }) => {
        expect(tableId, 'テーブルIDが取得できること').toBeTruthy();
        await page.goto(BASE_URL + `/admin/dataset__${tableId}`);
        await waitForAngular(page);
        await page.waitForTimeout(1500);
    ...
    // -------------------------------------------------------------------------
    test('527: テーブル設定変更後に一覧画面とフィールドヘッダーがエラーなく正常に表示されること', async ({ page }) => {
        expect(tableId, 'テーブルIDが取得できること').toBeTruthy();
        await checkPage(page, `/admin/dataset__${tableId}/setting`);
        const errors1 = await page.locator('.alert-danger').count();
        expect(errors1).toBe(0);
    ...
    // -------------------------------------------------------------------------
    test('528: 親テーブル削除権限あり・子テーブル削除権限なしの場合に子テーブル削除が禁止されること', async ({ page }) => {
        expect(tableId, 'テーブルIDが取得できること').toBeTruthy();
        // テーブル設定 > 権限設定を確認
        await page.goto(BASE_URL + `/admin/dataset__${tableId}/setting`);
        await waitForAngular(page);
    ...
    // -------------------------------------------------------------------------
    test('532: レコードの作成・編集・削除操作が正しく動作すること', async ({ page }) => {
        expect(tableId, 'テーブルIDが取得できること').toBeTruthy();
        await navigateToDatasetPage(page, tableId);
    ...
    // -------------------------------------------------------------------------
    test('533: 未ログイン状態の公開フォームからファイル添付して送信できテーブルに反映されること', async ({ page }) => {
        expect(tableId, 'テーブルIDが取得できること').toBeTruthy();
        // テーブル設定で公開フォームURLを確認
        await page.goto(BASE_URL + `/admin/dataset__${tableId}/setting`);
        await waitForAngular(page);
    ...
    // -------------------------------------------------------------------------
    test('534: 連鎖する他テーブル参照で親カテゴリ選択に応じて子カテゴリの選択肢が絞り込まれること', async ({ page }) => {
        expect(tableId, 'テーブルIDが取得できること').toBeTruthy();
    ...
    // -------------------------------------------------------------------------
    test('541: フィールド設定の変更が一覧画面・詳細画面に正しく反映されること', async ({ page }) => {
        expect(tableId, 'テーブルIDが取得できること').toBeTruthy();
        await checkPage(page, `/admin/dataset__${tableId}/setting`);
    ...
    // -------------------------------------------------------------------------
    test('542: テーブルにアイコンを設定した場合にサイドメニューで正しく表示されること', async ({ page }) => {
        expect(tableId, 'テーブルIDが取得できること').toBeTruthy();
        // テーブル設定でアイコン設定を確認
        await checkPage(page, `/admin/dataset__${tableId}/setting`);
    ...
    // -------------------------------------------------------------------------
    test('543: テーブル設定の変更保存後にレコード一覧画面がエラーなく表示されること', async ({ page }) => {
        expect(tableId, 'テーブルIDが取得できること').toBeTruthy();
        await checkPage(page, `/admin/dataset__${tableId}/setting`);
        const errors1 = await page.locator('.alert-danger').count();
        expect(errors1).toBe(0);
    ...
    // -------------------------------------------------------------------------
    test('544: フィルタ後に一括編集した場合フィルタ対象のレコードのみが更新されること', async ({ page }) => {
        expect(tableId, 'テーブルIDが取得できること').toBeTruthy();
        await page.goto(BASE_URL + `/admin/dataset__${tableId}`);
        await waitForAngular(page);
        await page.waitForTimeout(1500);
    ...
    // -------------------------------------------------------------------------
    test('548: テーブル設定の追加オプション変更が正しく反映されること', async ({ page }) => {
        expect(tableId, 'テーブルIDが取得できること').toBeTruthy();
        await checkPage(page, `/admin/dataset__${tableId}/setting`);
    ...
    // -------------------------------------------------------------------------
    test('549: WFステータス変更通知で「申請時」トリガーが申請タイミングのみ発火すること', async ({ page }) => {
        expect(tableId, 'テーブルIDが取得できること').toBeTruthy();
        // 通知設定画面を開く
        await page.goto(BASE_URL + `/admin/dataset__${tableId}/notification`);
        await waitForAngular(page);
    ...
    // -------------------------------------------------------------------------
    test('550: 他テーブル参照の検索モーダルが正常に表示されキーワード検索でレコードを選択できること', async ({ page }) => {
        expect(tableId, 'テーブルIDが取得できること').toBeTruthy();
        // レコード作成画面を開く
        await page.goto(BASE_URL + `/admin/dataset__${tableId}/create`);
        await waitForAngular(page);
    ...
    // 551: 絞り込み後の一括削除件数表示
    // -------------------------------------------------------------------------
    test('551: フィルター後の一括削除確認メッセージに正しい件数が表示されること', async ({ page }) => {
        expect(tableId, 'テーブルIDが取得できること').toBeTruthy();
        await navigateToDatasetPage(page, tableId);
    ...
    // -------------------------------------------------------------------------
    test('553: フィールド設定が正しく保存・反映されること', async ({ page }) => {
        expect(tableId, 'テーブルIDが取得できること').toBeTruthy();
        await checkPage(page, `/admin/dataset__${tableId}/setting`);
    ...
    // -------------------------------------------------------------------------
    test('554: フィルタの「いずれかの項目」OR条件で正しく絞り込みが行われること', async ({ page }) => {
        expect(tableId, 'テーブルIDが取得できること').toBeTruthy();
        await page.goto(BASE_URL + `/admin/dataset__${tableId}`);
        await waitForAngular(page);
        await page.waitForTimeout(1500);
    ...
    // -------------------------------------------------------------------------
    test('560: フィールド設定変更後に一覧画面・編集画面でフィールドがエラーなく正常に動作すること', async ({ page }) => {
        expect(tableId, 'テーブルIDが取得できること').toBeTruthy();
        // 一覧画面
        await navigateToDatasetPage(page, tableId);
    ...
    // 561: 集計・日付型の最大最小
    // -------------------------------------------------------------------------
    test('561: 集計の最大・最小で日付フィールドが選択でき正しく集計されること', async ({ page }) => {
        expect(tableId, 'テーブルIDが取得できること').toBeTruthy();
        // テーブル設定 > 集計設定を確認
        await checkPage(page, `/admin/dataset__${tableId}/setting`);
    ...
    // -------------------------------------------------------------------------
    test('563: フィールド設定変更が正しく保存・反映されること', async ({ page }) => {
        expect(tableId, 'テーブルIDが取得できること').toBeTruthy();
        await checkPage(page, `/admin/dataset__${tableId}/setting`);
    ...
    // -------------------------------------------------------------------------
    test('565: フィールド設定変更後にテーブル一覧とフィールドヘッダーがエラーなく表示されること', async ({ page }) => {
        expect(tableId, 'テーブルIDが取得できること').toBeTruthy();
        await checkPage(page, `/admin/dataset__${tableId}/setting`);
        const errors1 = await page.locator('.alert-danger').count();
        expect(errors1).toBe(0);
    ...
    // -------------------------------------------------------------------------
    test('566: 数値フィールドの入力中は桁区切りが非表示でフォーカスアウト後に桁区切りが適用されること', async ({ page }) => {
        expect(tableId, 'テーブルIDが取得できること').toBeTruthy();
        // レコード作成画面を開く
        await page.goto(BASE_URL + `/admin/dataset__${tableId}/create`);
        await waitForAngular(page);
    ...
    // -------------------------------------------------------------------------
    test('569: レコード編集操作が正しく動作すること', async ({ page }) => {
        expect(tableId, 'テーブルIDが取得できること').toBeTruthy();
        await navigateToDatasetPage(page, tableId);
    ...
    // -------------------------------------------------------------------------
    test('571: フィールド設定変更後に一覧・詳細画面でフィールドがエラーなく正常に表示されること', async ({ page }) => {
        expect(tableId, 'テーブルIDが取得できること').toBeTruthy();
        // 一覧画面
        await navigateToDatasetPage(page, tableId);
    ...
    // -------------------------------------------------------------------------
    test('572: パスワードが7桁以上で設定可能でありハイフンも使用できること', async ({ page }) => {
        expect(tableId, 'テーブルIDが取得できること').toBeTruthy();
        // ユーザー管理画面を開く
    ...
    // -------------------------------------------------------------------------
    test('576: 関連レコード一覧の表示・操作が正常に動作すること', async ({ page }) => {
        expect(tableId, 'テーブルIDが取得できること').toBeTruthy();
        await page.goto(BASE_URL + `/admin/dataset__${tableId}`);
        await waitForAngular(page);
        await page.waitForTimeout(1500);
    ...
    // -------------------------------------------------------------------------
    test('577: WF閲覧権限で承認者=ログインユーザー設定時に後付けユーザーでもレコードを閲覧できること', async ({ page }) => {
        expect(tableId, 'テーブルIDが取得できること').toBeTruthy();
        // テーブル設定のワークフロー設定を確認
        await page.goto(BASE_URL + `/admin/dataset__${tableId}/setting`);
        await waitForAngular(page);
    ...
    // -------------------------------------------------------------------------
    test('578: Excelテンプレートの300行目までの変数式が正しく反映されてダウンロードされること', async ({ page }) => {
        expect(tableId, 'テーブルIDが取得できること').toBeTruthy();
        // テーブル設定 > 帳票設定を確認
        await page.goto(BASE_URL + `/admin/dataset__${tableId}/setting`);
        await waitForAngular(page);
    ...
    // 581: ダッシュボード正常表示
    // -------------------------------------------------------------------------
    test('581: ダッシュボードがエラーなく正常に表示されること', async ({ page }) => {
        await page.goto(BASE_URL + '/admin/dashboard');
        await waitForAngular(page);
        await page.waitForTimeout(1500);
        const bodyText = await page.innerText('body');
    ...
    // -------------------------------------------------------------------------
    test('586: レコードの作成・編集操作が正しく完了しデータが保存されること', async ({ page }) => {
        expect(tableId, 'テーブルIDが取得できること').toBeTruthy();
        await navigateToDatasetPage(page, tableId);
    ...
    // -------------------------------------------------------------------------
    test('588: WF承認者が無効/削除済みの場合に申請時にエラー表示で取り下げになること', async ({ page }) => {
        expect(tableId, 'テーブルIDが取得できること').toBeTruthy();
        // テーブル設定 > ワークフロー設定を確認
        await page.goto(BASE_URL + `/admin/dataset__${tableId}/setting`);
        await waitForAngular(page);
    ...
    // -------------------------------------------------------------------------
    test('589: モバイルでリッチテキストの拡大編集後に前の画面に正常に戻れ編集内容が保存されること', async ({ page }) => {
        expect(tableId, 'テーブルIDが取得できること').toBeTruthy();
        // モバイルビューポートに変更
        await page.setViewportSize({ width: 375, height: 812 });
        await page.goto(BASE_URL + `/admin/dataset__${tableId}`);
    ...
    // -------------------------------------------------------------------------
    test('591: 帳票の一括ダウンロードZIPが正常に動作し選択レコードの帳票がダウンロードできること', async ({ page }) => {
        expect(tableId, 'テーブルIDが取得できること').toBeTruthy();
        await navigateToDatasetPage(page, tableId);
    ...
    // -------------------------------------------------------------------------
    test('593: 申請中レコードを削除した場合にワークフローバッジが正しく更新されること', async ({ page }) => {
        expect(tableId, 'テーブルIDが取得できること').toBeTruthy();
        await navigateToDatasetPage(page, tableId);
    ...
    // -------------------------------------------------------------------------
    test('594: WF無効テーブルのレコード詳細画面でWF履歴が非表示であること', async ({ page }) => {
        expect(tableId, 'テーブルIDが取得できること').toBeTruthy();
        await page.goto(BASE_URL + `/admin/dataset__${tableId}`);
        await waitForAngular(page);
        await page.waitForTimeout(1500);
    ...
    // -------------------------------------------------------------------------
    test('595: 関連レコードのSUM計算結果が親レコードに正しく表示されること', async ({ page }) => {
        expect(tableId, 'テーブルIDが取得できること').toBeTruthy();
        // テーブル設定画面で計算項目の存在を確認
        await page.goto(BASE_URL + `/admin/dataset/edit/${tableId}`);
        await waitForAngular(page);
    ...
    // 598: 一覧カラム幅の最小値制限
    // -------------------------------------------------------------------------
    test('598: 一覧画面のカラム幅を最小にしても項目名が表示されること', async ({ page }) => {
        expect(tableId, 'テーブルIDが取得できること').toBeTruthy();
        await page.goto(BASE_URL + `/admin/dataset__${tableId}`);
        await waitForAngular(page);
        await page.waitForTimeout(1500);
    ...
    // -------------------------------------------------------------------------
    test('602: 計算フィールドの先頭ゼロ付き数値がCSVダウンロードで保持されること', async ({ page }) => {
        expect(tableId, 'テーブルIDが取得できること').toBeTruthy();
        await page.goto(BASE_URL + `/admin/dataset__${tableId}`);
        await waitForAngular(page);
        await page.waitForTimeout(1500);
    ...
    // -------------------------------------------------------------------------
    test('603: コネクトのWF完了トリガー時に重複禁止エラーが適切に表示されること', async ({ page }) => {
        expect(tableId, 'テーブルIDが取得できること').toBeTruthy();
        // コネクト設定画面を確認
    ...
    // -------------------------------------------------------------------------
    test('604: テーブル設定変更後にレコード一覧画面が正常に表示されること', async ({ page }) => {
        expect(tableId, 'テーブルIDが取得できること').toBeTruthy();
        // テーブル設定画面を開く
        await page.goto(BASE_URL + `/admin/dataset/edit/${tableId}`);
        await waitForAngular(page);
    ...
    // 606: 子テーブル・最終更新者表示
    // -------------------------------------------------------------------------
    test('606: 子テーブルレコード作成直後に最終更新者が表示されること', async ({ page }) => {
        expect(tableId, 'テーブルIDが取得できること').toBeTruthy();
        await page.goto(BASE_URL + `/admin/dataset__${tableId}`);
        await waitForAngular(page);
        await page.waitForTimeout(1500);
    ...
    // -------------------------------------------------------------------------
    test('608: 複数許可の他テーブル参照フィールドがCSVで表示値出力されること', async ({ page }) => {
        expect(tableId, 'テーブルIDが取得できること').toBeTruthy();
        await navigateToDatasetPage(page, tableId);
    ...
    // -------------------------------------------------------------------------
    test('609: FTP連携エラー通知にテーブル名が含まれること', async ({ page }) => {
        expect(tableId, 'テーブルIDが取得できること').toBeTruthy();
        // FTP設定画面を確認
    ...
    // -------------------------------------------------------------------------
    test('610: 他テーブル参照の表示項目が空欄の場合バリデーションエラーが表示されること', async ({ page }) => {
        expect(tableId, 'テーブルIDが取得できること').toBeTruthy();
        // テーブル設定画面を開く
        await page.goto(BASE_URL + `/admin/dataset/edit/${tableId}`);
        await waitForAngular(page);
    ...
    // 612: ビュー権限デフォルト変更
    // -------------------------------------------------------------------------
    test('612: ビュー設定タブの権限デフォルト設定が変更・保持できること', async ({ page }) => {
        expect(tableId, 'テーブルIDが取得できること').toBeTruthy();
        // テーブルのビュー設定ページを開く
        await page.goto(BASE_URL + `/admin/dataset__${tableId}/setting`);
        await waitForAngular(page);
    ...
    // -------------------------------------------------------------------------
    test('615: チャートの凡例6個以上の場合の表示/非表示設定が機能すること', async ({ page }) => {
        expect(tableId, 'テーブルIDが取得できること').toBeTruthy();
        // チャート設定ページを確認
    ...
    // -------------------------------------------------------------------------
    test('617: 全データ選択時の一括削除に赤文字の注意書きが表示されること', async ({ page }) => {
        expect(tableId, 'テーブルIDが取得できること').toBeTruthy();
        await page.goto(BASE_URL + `/admin/dataset__${tableId}`);
        await waitForAngular(page);
        await page.waitForTimeout(1500);
    ...
    // -------------------------------------------------------------------------
    test('619: 子テーブルCSVダウンロードでテーブル名と親テーブルIDが正しく出力されること', async ({ page }) => {
        expect(tableId, 'テーブルIDが取得できること').toBeTruthy();
        await navigateToDatasetPage(page, tableId);
        const errors = await page.locator('.alert-danger').count();
        expect(errors).toBe(0);
    ...
    // -------------------------------------------------------------------------
    test('622: 他テーブル参照の値の絞り込みが全ユーザーで正常に動作すること', async ({ page }) => {
        expect(tableId, 'テーブルIDが取得できること').toBeTruthy();
    ...
    // -------------------------------------------------------------------------
    test('625: 数値フィールドの小数デフォルト値が正しく表示・保存されること', async ({ page }) => {
        expect(tableId, 'テーブルIDが取得できること').toBeTruthy();
        // テーブル設定画面を開く
        await page.goto(BASE_URL + `/admin/dataset/edit/${tableId}`);
        await waitForAngular(page);
    ...
    // -------------------------------------------------------------------------
    test('626: ユーザーテーブルCSVダウンロードで組織が正しく出力されること', async ({ page }) => {
        // ユーザー管理テーブルを開く
        await page.goto(BASE_URL + '/admin/user');
        await waitForAngular(page);
        await page.waitForTimeout(1500);
    ...
    // -------------------------------------------------------------------------
    test('632: テーブル設定変更後にレコード一覧がエラーなく表示されること', async ({ page }) => {
        expect(tableId, 'テーブルIDが取得できること').toBeTruthy();
        // テーブル設定画面
        await page.goto(BASE_URL + `/admin/dataset/edit/${tableId}`);
        await waitForAngular(page);
    ...
    // -------------------------------------------------------------------------
    test('633: スペースのみの項目名でもテーブル設定画面で項目が表示されること', async ({ page }) => {
        expect(tableId, 'テーブルIDが取得できること').toBeTruthy();
        await page.goto(BASE_URL + `/admin/dataset/edit/${tableId}`);
        await waitForAngular(page);
        await page.waitForTimeout(1500);
    ...
    // -------------------------------------------------------------------------
    test('637: 数字のみの項目名でも計算フィールドで正しく参照・計算できること', async ({ page }) => {
        expect(tableId, 'テーブルIDが取得できること').toBeTruthy();
        await page.goto(BASE_URL + `/admin/dataset/edit/${tableId}`);
        await waitForAngular(page);
        await page.waitForTimeout(1500);
    ...
    // -------------------------------------------------------------------------
    test('638: WF申請フローNo.変更時にステップ番号が正しく切り替わること', async ({ page }) => {
        expect(tableId, 'テーブルIDが取得できること').toBeTruthy();
        // ワークフロー設定ページを確認
        await page.goto(BASE_URL + `/admin/dataset__${tableId}/setting`);
        await waitForAngular(page);
    ...
    // -------------------------------------------------------------------------
    test('639: ブラウザタブのタイトルにテーブル名が正しく表示されること', async ({ page }) => {
        expect(tableId, 'テーブルIDが取得できること').toBeTruthy();
        // レコード一覧画面を開く
        await page.goto(BASE_URL + `/admin/dataset__${tableId}`);
        await waitForAngular(page);
    ...
    // -------------------------------------------------------------------------
    test('641: システムログページにAPIログチャートが表示されること', async ({ page }) => {
        // システム設定のログページを開く
    ...
    // -------------------------------------------------------------------------
    test('643: 主キー設定で5項目以上を設定でき正しく保存・動作すること', async ({ page }) => {
        expect(tableId, 'テーブルIDが取得できること').toBeTruthy();
        await page.goto(BASE_URL + `/admin/dataset/edit/${tableId}`);
        await waitForAngular(page);
        await page.waitForTimeout(1500);
    ...
    // -------------------------------------------------------------------------
    test('644: テーブルコピーやJSONインポートで行の設定が正しくコピーされること', async ({ page }) => {
        expect(tableId, 'テーブルIDが取得できること').toBeTruthy();
        // テーブル管理画面を開く
        await page.goto(BASE_URL + '/admin/dataset');
        await waitForAngular(page);
    ...
    // -------------------------------------------------------------------------
    test('647: 月末日でも相対値「今月」フィルタが正しく動作すること', async ({ page }) => {
        expect(tableId, 'テーブルIDが取得できること').toBeTruthy();
        await page.goto(BASE_URL + `/admin/dataset__${tableId}`);
        await waitForAngular(page);
        await page.waitForTimeout(1500);
    ...
    // -------------------------------------------------------------------------
    test('648: パスワードリセットメールにリンクとURL文字列の両方が含まれること', async ({ page }) => {
    ...
    // -------------------------------------------------------------------------
    test('650: 特定項目更新時通知が手動編集・CSVアップロード両方で送信されること', async ({ page }) => {
        expect(tableId, 'テーブルIDが取得できること').toBeTruthy();
        // 通知設定ページを開く
        await page.goto(BASE_URL + `/admin/dataset__${tableId}/notification`);
        await waitForAngular(page);
    ...
    // 651: SMTP設定 — テストメール送信
    // -------------------------------------------------------------------------
    test('651: SMTP設定画面でテストメール送信が成功すること', async ({ page }) => {
        // SMTP設定ページを開く
    ...
    // -------------------------------------------------------------------------
    test('652: 関連レコードの表示条件が他テーブル参照でもSUM計算が正しく動作すること', async ({ page }) => {
        expect(tableId, 'テーブルIDが取得できること').toBeTruthy();
        await page.goto(BASE_URL + `/admin/dataset__${tableId}`);
        await waitForAngular(page);
        await page.waitForTimeout(1500);
    ...
    // -------------------------------------------------------------------------
    test('655: 計算項目の日付フォーマットが帳票出力で正しく反映されること', async ({ page }) => {
        expect(tableId, 'テーブルIDが取得できること').toBeTruthy();
        await navigateToDatasetPage(page, tableId);
    ...
    // -------------------------------------------------------------------------
    test('657: 親テーブルの階層参照を子テーブルの計算フィールドで参照できること', async ({ page }) => {
        expect(tableId, 'テーブルIDが取得できること').toBeTruthy();
        await page.goto(BASE_URL + `/admin/dataset/edit/${tableId}`);
        await waitForAngular(page);
        await page.waitForTimeout(1500);
    ...
    // -------------------------------------------------------------------------
    test('658: 通知設定の通知先に「ログインユーザーのメールアドレス」が選択可能なこと', async ({ page }) => {
        expect(tableId, 'テーブルIDが取得できること').toBeTruthy();
        await page.goto(BASE_URL + `/admin/dataset__${tableId}/notification`);
        await waitForAngular(page);
        await page.waitForTimeout(1500);
    ...
    // -------------------------------------------------------------------------
    test('659: 相対値「来年度」フィルタが年度開始月に基づいて正しく絞り込まれること', async ({ page }) => {
        expect(tableId, 'テーブルIDが取得できること').toBeTruthy();
        await page.goto(BASE_URL + `/admin/dataset__${tableId}`);
        await waitForAngular(page);
        await page.waitForTimeout(1500);
    ...
    // -------------------------------------------------------------------------
    test('661: チャートプレビューの前後切り替えボタンで期間表示が正しく更新されること', async ({ page }) => {
        expect(tableId, 'テーブルIDが取得できること').toBeTruthy();
        await page.goto(BASE_URL + `/admin/dataset__${tableId}/chart`);
        await waitForAngular(page);
        await page.waitForTimeout(1500);
    ...
    // -------------------------------------------------------------------------
    test('663: 一括否認・一括削除ボタンの非表示設定が正しく反映されること', async ({ page }) => {
        expect(tableId, 'テーブルIDが取得できること').toBeTruthy();
        // テーブル設定画面を開く
        await page.goto(BASE_URL + `/admin/dataset/edit/${tableId}`);
        await waitForAngular(page);
    ...
    // -------------------------------------------------------------------------
    test('664: ルックアップで表示した日付がルックアップ先のフォーマットで表示されること', async ({ page }) => {
        expect(tableId, 'テーブルIDが取得できること').toBeTruthy();
        await navigateToDatasetPage(page, tableId);
        const errors = await page.locator('.alert-danger').count();
        expect(errors).toBe(0);
    ...
    // -------------------------------------------------------------------------
    test('665: 他テーブル参照の日時項目で一桁月日のCSV再アップロードがエラーなく完了すること', async ({ page }) => {
        expect(tableId, 'テーブルIDが取得できること').toBeTruthy();
        await navigateToDatasetPage(page, tableId);
        const errors = await page.locator('.alert-danger').count();
        expect(errors).toBe(0);
    ...
    // -------------------------------------------------------------------------
    test('666: 子テーブルを含むCSVアップロードで他テーブル参照値が正しく反映されること', async ({ page }) => {
        expect(tableId, 'テーブルIDが取得できること').toBeTruthy();
        await navigateToDatasetPage(page, tableId);
        const errors = await page.locator('.alert-danger').count();
        expect(errors).toBe(0);
    ...
    // -------------------------------------------------------------------------
    test('668: 他テーブル参照の絞り込みで他テーブル参照タイプの表示項目を選択できること', async ({ page }) => {
        expect(tableId, 'テーブルIDが取得できること').toBeTruthy();
        await page.goto(BASE_URL + `/admin/dataset/edit/${tableId}`);
        await waitForAngular(page);
        await page.waitForTimeout(1500);
    ...
    // -------------------------------------------------------------------------
    test('669: 画像フィールドでファイルサイズと画素数が正しく表示されること', async ({ page }) => {
        expect(tableId, 'テーブルIDが取得できること').toBeTruthy();
        // レコード詳細画面で画像フィールドを確認
        await page.goto(BASE_URL + `/admin/dataset__${tableId}`);
        await waitForAngular(page);
    ...
    // -------------------------------------------------------------------------
    test('670: メール受信取込機能の設定画面が正常に表示され保存できること', async ({ page }) => {
        // メール受信取込設定ページを開く
    ...
    // -------------------------------------------------------------------------
    test('671: 大量レコードのCSVダウンロードで重複が発生しないこと', async ({ page }) => {
        expect(tableId, 'テーブルIDが取得できること').toBeTruthy();
        await navigateToDatasetPage(page, tableId);
    ...
    // -------------------------------------------------------------------------
    test('673: WF承認済みスキップが組織・項目条件で正しく動作すること', async ({ page }) => {
        expect(tableId, 'テーブルIDが取得できること').toBeTruthy();
        await page.goto(BASE_URL + `/admin/dataset__${tableId}/setting`);
        await waitForAngular(page);
        await page.waitForTimeout(1500);
    ...
    // -------------------------------------------------------------------------
    test('674: Yes/Noフィールドのラベル空白時もCSVダウンロードで値が正しく出力されること', async ({ page }) => {
        expect(tableId, 'テーブルIDが取得できること').toBeTruthy();
        await navigateToDatasetPage(page, tableId);
        const errors = await page.locator('.alert-danger').count();
        expect(errors).toBe(0);
    ...
    // -------------------------------------------------------------------------
    test('675: 関連レコード削除時に確認ダイアログが表示され正常に閉じること', async ({ page }) => {
        expect(tableId, 'テーブルIDが取得できること').toBeTruthy();
        await page.goto(BASE_URL + `/admin/dataset__${tableId}`);
        await waitForAngular(page);
        await page.waitForTimeout(1500);
    ...
    // -------------------------------------------------------------------------
    test('678: チャートプレビューのページ移動ボタンを押してもエラーが発生しないこと', async ({ page }) => {
        expect(tableId, 'テーブルIDが取得できること').toBeTruthy();
        await page.goto(BASE_URL + `/admin/dataset__${tableId}/chart`);
        await waitForAngular(page);
        await page.waitForTimeout(1500);
    ...
    // -------------------------------------------------------------------------
    test('680: フロー戻し時にスキップ承認者ではなく直前の承認者に戻ること', async ({ page }) => {
        expect(tableId, 'テーブルIDが取得できること').toBeTruthy();
        await page.goto(BASE_URL + `/admin/dataset__${tableId}/setting`);
        await waitForAngular(page);
        await page.waitForTimeout(1500);
    ...
    // -------------------------------------------------------------------------
    test('681: 関連レコード一覧の表示条件で他テーブル参照の他テーブル参照が正しく反映されること', async ({ page }) => {
        expect(tableId, 'テーブルIDが取得できること').toBeTruthy();
        await page.goto(BASE_URL + `/admin/dataset__${tableId}`);
        await waitForAngular(page);
        await page.waitForTimeout(1500);
    ...
    // -------------------------------------------------------------------------
    test('682: 子テーブルCSVアップロードで必須項目空欄時にエラーが表示されること', async ({ page }) => {
        expect(tableId, 'テーブルIDが取得できること').toBeTruthy();
        await navigateToDatasetPage(page, tableId);
