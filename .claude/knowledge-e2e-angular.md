...
### yaml の `includes` フィールド
中カテゴリに統合された元ケースの case_no を `includes` に記載。
これにより「#248 のバグ修正はどのテストで確認されるか」がトレースできる。

### 中カテゴリの分け方
同じページ/同じ操作文脈でチェックできるものをまとめる:
- 同じURL（テーブル一覧、レコード編集、設定画面等）
- 同じ前提操作（カレンダー表示に切り替え、フィルタを設定等）
- 1動画として見て意味がある単位
