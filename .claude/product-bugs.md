...
### 🚨 auth-260: マルチテナント分離欠陥（重大セキュリティ脆弱性）(2026-04-21)

- **症状**: テナント A でログインして取得した cookie を、テナント B サブドメインに送るとテナント B の管理画面にアクセスできてしまう（= なりすまし可能）
- **該当プロダクトコード**:
  - `/Users/yasaipopo/PhpStormProjects/PopoframeworkSlim/Application/Controllers/LoginController.php:269-273` `_login()` - セッションに `admin.id` のみ保存、発行元テナント識別子 `ALIAS_NAME` を記録していない
  - `/Users/yasaipopo/PhpStormProjects/PopoframeworkSlim/routes/admin/admin.php:1105-1109` - 認証ミドルウェアが `$_SESSION['admin_id']` をそのまま現テナント DB に投げて lookup。session 発行テナントと `HTTP_HOST` 由来の `ALIAS_NAME` を照合していない
- **攻撃シナリオ**:
  1. 攻撃者が自テナントにログイン（セッション Cookie 取得）
  2. Cookie ドメインを他テナントに書き換えて送信
  3. 同じ admin_id を持つ他テナントのユーザーとしてアクセス成立（Redis/file セッションが全テナント共有のため）
- **影響**: 重大 — 全テナント間での情報漏洩・操作可能
- **判定**: 🔴 **PRODUCT バグ（セキュリティ重大）**
- **対応方針**:
  1. `LoginController._login()` で `$_SESSION['admin']['tenant'] = ALIAS_NAME` を保存
  2. 認証ミドルウェアで `$_SESSION['admin']['tenant'] !== ALIAS_NAME` なら `session_destroy()` + 401
  3. `ApiRequest.php` にも同等ガード追加
  4. 修正 PR 時に Integration テスト (全体セッション分離) 追加必須
- **既存 PHPUnit**: OpenSearch 層のみ分離テストあり。アプリ全体のセッション分離テストは未実装
- **テスト対応**: 修正不要（攻撃経路を正しく再現している）。fail のまま保持
- **調査記録**: `.claude/auth-260-investigation-20260421.md` 参照
