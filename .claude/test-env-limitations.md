...
#### SS05: 契約設定 (PayPal/Stripe 外部決済連携) (2026-05-09 追加)
- **対象 spec**: `tests/system-settings.spec.js` SS05 (130-01 PayPal / 131-02 デビット・クレジット / 284-1 Stripe)
- **スキップ理由**: PigeonCloud 側の契約・課金フロー機能廃止 + 外部決済サービス (PayPal/Stripe) 連携のため E2E 自動テスト不可。手動確認も外部サービス依存で実用性低い
- **対応**: test 関数レベルで `test.skip(true, ...)` に統合。各 step は削除済み (旧コード履歴は git log SS05 検索)
- **解消条件**: PigeonCloud 内部の Stripe 連携 API のみで完結する形に変更された場合、または社内 mock 決済サービスが整備された場合に再有効化
- **見直し予定日**: 機能変更時 (現状予定なし)
